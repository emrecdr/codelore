...
- **🚦 Quality gate inside the agent loop.** `codelore gate` — and its MCP twin `gate_changes` — evaluate your **uncommitted working tree** against HEAD *before you commit*: projected per-file code-health delta (with a floor on each newly added file's own score), newly-introduced import cycles, duplicated functions introduced only in the working tree, and coupling blast radius, gated by the same `.codelore-thresholds.toml` and optional defect-calibrated weights. The companion `change_context` tool briefs an AI coding agent on a file's ownership, co-change partners, and calibrated risk *before* it writes. The behavioral, temporal signal a static pre-commit gate can't see — fully local, no account, in the loop.
