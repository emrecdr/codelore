...
- **The narrative-evidence doc gained its first model study: a pinned 3B local model narrated every code-health-scored file in this repository, three times each.** 110 files × 3 generations against `llama3.2-t0-s42` — an ollama model card that bakes temperature 0 and a fixed seed into the model id itself — produced a 65.5% `grounded ✓` rate on the first generation (Wilson 95% [0.562, 0.737]), published with per-item records committed beside the doc so every aggregate can be recomputed. Two findings matter beyond the headline. First, the flagged stamps validate the labelled corpus's predictions in the wild: the dominant uncited tokens are function-span line numbers quoted from the `functions` section's identifiers (a newly observed over-flagging route, now the corpus's next growth candidate) and verbatim quotes of the `corpus_percentile_ci` en-dash composite (the corpus's `fp-ci-bound` class occurring in reality) — so the measured grounded rate is a lower bound on faithful generations, not a hallucination rate. Second, temperature-0-plus-seed determinism split cleanly in two: repeat generations of an identical prompt were byte-identical in 110/110 items, while the first generation diverged from that stable pair in 35/110 and carried all 13 verdict flips — the serving-state sensitivity the protocol pre-registered flip-rate measurement for, and the reason the doc reports the first generation (the realistic single-run user path, ~12% verdict instability) as primary. The paired frontier-model leg stays pending an endpoint and must reuse the recorded commit and file list; live models remain out of gating CI.
