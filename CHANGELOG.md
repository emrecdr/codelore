...
- **`codelore profile` no longer advertises a C++ grammar the binary cannot parse, and `codelore-rca`'s crates.io page no longer pins four grammars it does not have.** The unreachable-grammar excision removed C++, Kotlin and the two Mozilla helper grammars, and its own narrative is accurate — what it did not do was sweep for other text asserting the state it had just changed. Six claims survived it, and the two that matter most are not comments. `codelore profile` is the command a user runs to learn what the tool is built from, and it listed C++ among the pinned tree-sitter grammars; the product dispatches six parsers over five languages and none of them is C++. `UPSTREAM.md` is set as `readme` in `codelore-rca`'s manifest, so its grammar-version table *is* the published package page, and it still carried rows for `tree-sitter-kotlin-ng` and the three `bca-tree-sitter-*` grammars plus a paragraph routing C++ through a `get_language!` macro arm that was deleted — inside a section written as present-tense guidance for the next person upgrading a grammar. The remaining four: a layout note describing a `macros.rs` reference to the mozcpp grammar that is gone, and promising work "to be resolved" that has been; `import_graph.rs` explaining that it avoids `petgraph` because of a version conflict that ceased to exist when `petgraph` left the workspace entirely; the roadmap asserting the `leiden-rs` `petgraph` feature is off *solely* over that conflict, contradicting the manifest comment the same commit wrote saying the opposite; and a manifest comment introducing a `dirs` declaration that a later commit deleted from directly beneath it. Every one of these survives a build, which is the point — the removals were verified by building, and that is the right method, but a compiler cannot see prose. Two further instances inside the vendored fork are recorded rather than fixed: a `SpaceKind` variant documented as a C/C++ namespace whose only producer left with the C++ getter, and a crate rustdoc listing six languages that were never vendored. Removing a public enum variant is a breaking change and correcting upstream's rustdoc means diverging from it for cosmetic gain, so both are a decision about the fork's divergence budget rather than defect fixes.
