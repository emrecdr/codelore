...
- **`diff_gate_verdict` and `change_set_gate_verdict` now share one internal `skipped`/`passed`/`failed` classification helper instead of repeating the same three-way branch — no output change.** A new regression test also asserts bot filtering never regresses to the retired canonical-level `BOOL_OR(is_bot)` collapse (see `[Unreleased]`'s pair-granular bot-filtering fix below) outside its one legitimate mention in `analyses/query.rs`'s explanatory doc comment.
