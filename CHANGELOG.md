...
- **`defect-validation` analysis.** New analysis (`--analysis defect-validation`) that reads a defect-calibration artifact and flattens its evidence into `(metric, value)` rows: the band table (share of defect-introducing changes that landed in files red/yellow/green at the time, each with its count/total), AUC and precision@k of HEAD structural risk against the defect labels, mining tallies, the weight-tuning decision (`weights_source`), and both validation AUCs — surfaced whenever present, so a reader can see when tuning was applied yet the validation AUCs sit below 0.5. Presentation follows the project's honesty framing: association, not causation; every count carries its `n`; absent metrics render as explicit `n/a (<why>)`. The analysis reads the artifact only (never mines); without a configured artifact it returns zero rows and prints a one-line hint pointing at `codelore calibrate-defects`. Outputs: csv, json, markdown.
