...
- **A repository could point CodeLore's calibration reader at any file on the host.** `[calibration] defect_artifact` is read out of the *analysed* repository's own `.codelore-thresholds.toml`, so it is attacker-controlled whenever that repository is untrusted, and it is consumed on the plain `analyze --repo <clone>` path as well as by `check`, `gate`, `explain` and the MCP server at startup. An absolute value passed through verbatim and a relative one was joined to the repo root with no containment check, so a hostile repository could name any path on the machine: measured against a throwaway clone, `../outside/secret.txt` and `/etc/hosts` were both opened and parsed, and the two failure modes split cleanly by exit code — 4 once a file was read, 2 when it was absent — which is an existence oracle over the host filesystem for as many paths as the attacker cares to try. The distinction that fixes this is the path's *source*, not its shape. `--defect-calibration` is typed by the operator, stays trusted, and is unrestricted; the repository-declared value is now confined to the tree that declared it, with absolute paths and `..` components both refused and the offending key named. `..` is rejected outright rather than normalised and re-checked, because the lexical normaliser used for import resolution pops on an empty stack — `../secrets` collapses to `secrets`, which would pass a starts-with test while naming a file outside the tree. The three sites that resolved this path had drifted into three copies of the same join; they now share one function, so the containment cannot be present in one command and missing in another. An artifact that genuinely lives outside the repository is still reachable, by passing the flag.
