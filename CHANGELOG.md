...
- **The `diff` base-cache digest folds the whole base-rev `Options`, not just `min_revs` + `exclude`** — so changing any other measured-value-affecting option (grouping, window, language filters) no longer silently reuses a baseline computed under different settings.
