...
- **The advisory layer's request timeout is a documented number instead of an unnamed bound.** §8's failure-postures section said requests use "a single bounded timeout and no retries" without saying what the bound is or that nothing moves it. It is 120 seconds, covering connect through body read, and it is compiled in — unlike the provider, base URL, API key and model, which are all read from the environment. Someone meeting the ceiling saw an aborted `--llm` run and had no way to learn a ceiling existed, let alone where it sat; an independent evaluation of the layer measured a maximum request of 117.7s against it on a model whose mean was 23.6s, close enough that a slower model or a loaded gateway crosses it. Documentation only — the constant is unchanged, and whether it should become configurable is a recorded finding rather than a decision taken here.
