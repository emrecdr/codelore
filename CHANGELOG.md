...
- **Zero-config factor header with XmR attention** — a four-tile dashboard header (Code, Architecture, Knowledge, Delivery) above the KPI grid. Each tile shows a 0–100 headline score and an XmR-gated attention badge that fires only when a signal is statistically unlikely to be noise (last point outside Shewhart limits OR eight consecutive points same side of mean). No configuration required.
...
- **`codelore mcp` subcommand** — starts a Model Context Protocol server over stdio, exposing two tools: `repo_overview` (repository summary: commit count, authors, files, date range) and `hotspots` (top hotspot files by revision count, with optional `limit` parameter). Each tool call opens its own `FactsDb` via the warm-cache path so the `!Send + !Sync` DuckDB connection never crosses thread boundaries. Warm-cache calls are fast; the first call on a cold cache triggers ingest. Read-only — no network, no account, no telemetry. Uses `rmcp 2.2` with stdio transport.
