...
- **`cargo-deny` no longer fails on a dependency that has no unyanked version to move to.** Every published version of `bisync` was yanked upstream, and it reaches this workspace transitively as `gix` → `gix-protocol` → `bisync`. The newest `gix-protocol` still requires `^0.3.0`, so `cargo update -p bisync` has nowhere to go and no version of anything this repository controls resolves it; the lowest fixable layer is upstream. The gate stays `yanked = "deny"` and the exemption is scoped to that one crate at that one version, so a yank in any other dependency still fails the build, and the entry stops matching — and should be deleted — once `gix` drops the dependency. Relaxing the check to `warn` globally was the alternative and was rejected: it would trade one upstream event for a permanently weaker gate.
