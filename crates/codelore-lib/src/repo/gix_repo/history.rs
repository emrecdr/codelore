//! The gix history-reading engine behind `GixRepo`'s walk/diff methods:
//! per-commit tree diffing, blob loc/hunk counting, commit-event
//! construction, and the order-preserving chunked walker stream.

use gix::diff::tree_with_rewrites::Change as GixChange;

use crate::{ChangeType, CodeLoreError, CommitEvent, FileChange, Hunk, Result};
...
pub(super) fn compute_changed_files(
...
pub(super) fn count_loc_and_hunks(
    repo: &gix::Repository,
    old_oid: Option<gix::ObjectId>,
    new_oid: Option<gix::ObjectId>,
...
pub(super) fn process_commit_oid(
    oid: gix::ObjectId,
    inner: &gix::ThreadSafeRepository,
...
pub(super) struct WalkerStream<I: Iterator<Item = Result<CommitEvent>>> {
    pub(super) inner: I,
