...
//!
//! API used:
//!   - `bca_rca::{RustParser, JavaParser, PythonParser, TypescriptParser, JavascriptParser}` — typed
//!     parser structs (public via the `mk_code!` macro in langs.rs)
//!   - `bca_rca::ParserTrait::new(code, path, None)` — construct a parser from source bytes
//!   - `bca_rca::metrics(&parser, path)` — returns `Option<FuncSpace>` (spaces.rs)
...
// ---------------------------------------------------------------------------
fn run_metrics<T: ParserTrait>(source: &[u8], filename: &str) -> bca_rca::FuncSpace {
