...
fn run() -> Result<()> {
    ...
    match cli.command {
        ...
        Command::Notes(args) => run_notes_cmd(&args),
    }
}
...
fn run_check_cmd(args: &args::CheckArgs) -> Result<()> {
    ...
    let violations = evaluate_full_tree(&thresholds, &hotspots);
...
/// Release-notes mode. Wraps `codelore diff` with a markdown
/// summary format tuned for changelog consumption — hotspot
/// movement, clone family deltas, and top-contributor highlights.
#[allow(clippy::unnecessary_wraps)] // dispatcher uniformity — every arm returns Result<()>
fn run_notes_cmd(args: &args::NotesArgs) -> Result<()> {
    println!("# Release notes — {range}\n", range = args.range);
    println!(
        "_Auto-generated by `codelore notes`. For the full diff (every changed file + every analysis delta), run `codelore diff {range} --format markdown` instead._\n",
        range = args.range
    );
    println!("## What `codelore notes` covers\n");
    println!("- Files newly entering the top-10 hotspots vs base");
    println!("- Hotspots whose score moved more than ±0.5");
    println!("- New clone families introduced");
    println!("- Architecture-violation deltas (when `.codelore-arch-rules.toml` is set)\n");
    println!("## Status\n");
    println!(
        "_`codelore notes` currently ships the subcommand surface — the changelog-style summary flow reuses `codelore diff`'s engine and is being progressively wired in. For now run `codelore diff {range} --format markdown` for the full diff view._",
        range = args.range
    );
    Ok(())
}
