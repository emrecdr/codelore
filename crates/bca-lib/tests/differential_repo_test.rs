...
use bca_lib::test_support::differential_repo::DifferentialRepo;
...
fn fixture_path() -> &'static PathBuf {
    let sf = FIXTURE.get_or_init(|| {
        let repo = bca_lib::test_support::differential_repo::build();
...
fn commit_metadata_match() {
    ...
    for rev in &revs {
        ...
        assert_eq!(
            gix_md.signed, cli_md.signed,
            "signed mismatch at {rev}"
        );
        assert_eq!(
