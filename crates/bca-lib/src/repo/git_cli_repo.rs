...
impl GitCliRepo {
    pub fn open(root: &Path) -> Result<Self> {
        ...
        if !output.status.success() {
            return Err(BcaError::Repo(format!(
                "not a git repo: {}",
                root.display()
            )));
        }
    ...
    fn run_git(&self, args: &[&str]) -> Result<std::process::Output> {
        Command::new("git")
            ...
            .stdin(Stdio::null())
            .output()
            .map_err(|e| BcaError::Repo(format!("git {}: {e}", args.join(" "))))
    }
}
...
impl Repo for GitCliRepo {
    fn walk_commits<'a>(
        ...
        let raw = String::from_utf8(output.stdout)
            .map_err(|e| BcaError::Repo(format!("git log output not utf-8: {e}")))?;
    ...
    fn changed_files(&self, rev: &str) -> Result<Vec<FileChange>> {
        ...
        let raw = String::from_utf8(output.stdout)
            .map_err(|e| BcaError::Repo(format!("git show output not utf-8: {e}")))?;
    ...
    fn diff_hunks(&self, rev: &str, path: &str) -> Result<Vec<Hunk>> {
        ...
        let raw = String::from_utf8(output.stdout)
            .map_err(|e| BcaError::Repo(format!("git show -p output not utf-8: {e}")))?;
    ...
    fn commit_metadata(&self, rev: &str) -> Result<CommitMetadata> {
        ...
        let raw = String::from_utf8(output.stdout)
            .map_err(|e| BcaError::Repo(format!("git show metadata not utf-8: {e}")))?;
