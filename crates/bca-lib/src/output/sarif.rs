...
//!
//! Rule `BCA-HOTSPOT`: properties.tags includes "behavioral" and "hotspot"
...
const RULE_ID: &str = "BCA-HOTSPOT";
const AUTOMATION_ID: &str = "bca/hotspots/run";
...
pub fn write_hotspots_sarif<W: Write>(
    ...
    serde_json::to_writer_pretty(w, &doc).map_err(|e| BcaError::Output(format!("sarif: {e}")))?;
    Ok(())
}
...
fn build_result(row: &HotspotRow, repo_root: &str) -> serde_json::Value {
    ...
    json!({
        ...
        "properties": {
            ...
            "bca/revs": row.revisions,
            "bca/cognitive": row.cognitive,
            "bca/codehealth": row.code_health,
            "bca/score": row.hotspot_score,
            "tags": ["behavioral", "hotspot"]
        }
    })
}
