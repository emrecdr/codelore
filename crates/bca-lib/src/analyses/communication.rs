...
pub fn run_communication(db: &FactsDb, opts: &Options) -> Result<Vec<CommunicationRow>> {
    ...
    let sql = format!(
        "WITH author_files AS (
         ...
         ORDER BY strength DESC, p.author_a ASC, p.author_b ASC{limit}",
    ...
    let mut stmt = db
        .conn()
        .prepare(&sql)
        .map_err(|e| BcaError::Analysis(format!("prepare communication: {e}")))?;
    let rows = stmt
        .query_map([], |r| {
            Ok(CommunicationRow {
        ...
        .map_err(|e| BcaError::Analysis(format!("query communication: {e}")))?;
    rows.collect::<std::result::Result<Vec<_>, _>>()
        .map_err(|e| BcaError::Analysis(format!("collect communication: {e}")))
}
