...
pub fn run_code_age(db: &FactsDb, opts: &Options) -> Result<Vec<CodeAgeRow>> {
    ...
    let mut stmt = db
        .conn()
        .prepare(&sql)
        .map_err(|e| BcaError::Analysis(format!("prepare code-age: {e}")))?;
    let rows = stmt
        .query_map([], |r| {
            Ok(CodeAgeRow {
        ...
        .map_err(|e| BcaError::Analysis(format!("query code-age: {e}")))?;
    rows.collect::<std::result::Result<Vec<_>, _>>()
        .map_err(|e| BcaError::Analysis(format!("collect code-age: {e}")))
}
