...
codelore analyze [OPTIONS]
  -a, --analysis NAME           Which analysis [default: revisions]
                                (any of the analyses above; passing an unknown
...
`[calibration]` is not a gate — it's a config *selector* that declares the repo's default defect-calibration artifact once, so `analyze`, `check`, `explain <path>`, and `codelore mcp` all pick it up without repeating `--defect-calibration` on every invocation. Precedence: an explicit `--defect-calibration` flag (or the MCP server's startup flag) always wins; otherwise the `[calibration] defect_artifact` path is used, resolved relative to the repo root **and confined to it** — an absolute path, or one containing `..`, is refused with an exit-2 configuration error naming the key. That value is read out of the analysed repository rather than typed by you, so it is not allowed to name a file outside the tree that declared it; a committed absolute path could not resolve on anyone else's machine in any case. The confinement follows the *section*, not the file it arrived in: a thresholds file you name with `--thresholds-file` is confined to `--repo` exactly as a discovered one is, because naming a file's location is not the same as authoring its contents — point that flag inside a repository you do not trust and the value is as attacker-controlled as any other. `--defect-calibration` is the operator's own input and stays unrestricted, so it remains the way to point at an artifact that genuinely lives elsewhere. Otherwise the run is uncalibrated. A thresholds file containing only `[calibration]` still leaves `check` vacuously passing — see [Defect calibration](#defect-calibration-does-the-health-score-predict-where-defects-land-here) for what the artifact does once applied.
...
Each tool call opens its own `FactsDb` connection via the warm-cache path. This is intentional: `duckdb::Connection` is `!Sync` — and its prepared statements and appenders borrow it, making them `!Send` — so each call runs entirely on a dedicated blocking thread (`tokio::task::spawn_blocking`) from connection open to result serialization. The connection is dropped before the future resolves. No tool modifies tracked repository content or the fact store; the writes any tool performs are best-effort sidecar caches — `explain_file` persisting its advisory narrative, and `gate_changes` memoising its measured change-set report — plus `delta_health`, which creates and then removes a temporary detached git worktree per revision it compares (registered and torn down via an RAII guard).
