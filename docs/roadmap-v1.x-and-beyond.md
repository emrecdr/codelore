...
| Re-add `x86_64-unknown-linux-musl` release target | Dropped because Ubuntu's `musl-tools` ships `musl-gcc` but not `musl-g++`, and two things needed a C++ cross-toolchain. One is gone: excising the unreachable grammars removed `bca-tree-sitter-preproc`'s and `bca-tree-sitter-ccomment`'s `scanner.cc`, leaving `codelore-rca` pure C. Bundled DuckDB's .cpp files are now the only blocker, and have their own mitigation two rows above. Two routes: a musl-targeted build job inside a `messense/rust-musl-cross`-style Docker image, or a `musl-cross-make`-built toolchain on the existing runner. |
