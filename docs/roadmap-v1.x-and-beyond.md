...
| ~~`cargo-machete` / `cargo-udeps` as a CI gate~~ | **Evaluated and rejected on measurement.** Proposed to automate the unused-declared-dependency class this repo hit twice. `cargo machete` reports six findings here and **one is real** (`dirs`, since removed); the other five name load-bearing code — `num-traits`, required via `#[derive(FromPrimitive)]` expansion, and four of the five grammars the product dispatches. The tool's own output proves the mechanism: `tree-sitter-typescript` is the only grammar it does *not* flag, and the only one named outside the `mk_langs!` invocation. It is a text scanner; macro invocations are opaque to it (`cargo-shear` concedes the same limit, liftable only with a nightly `--expand`). Reaching green would take five ignore entries — suppressing more than it reports, and failing the bar the `zizmor` adoption set: a check that is red for the wrong reason teaches people to ignore red checks. The class is still worth catching; the method that works is removing the declaration and building. |
| ~~`petgraph` 0.6 → 0.8 bump~~ | **Closed by the unreachable-grammar excision.** Its single consumer was `codelore-rca/src/preproc.rs`, which went with the `preproc` grammar, so `petgraph` left the workspace entirely and its `dependabot.yml` pin is retired. The follow-on this unblocks: `leiden-rs`'s `petgraph` feature had two reasons to stay off, and only one survives. It is no longer constrained by a version collision — that pin left the workspace with the grammars — so what remains is simply that we feed edges directly as `Vec<(u32, u32, f64)>` and do not need the graph type. It can now be evaluated on its merits rather than held off by a conflict. |
