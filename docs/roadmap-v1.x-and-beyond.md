...
| **CHM-B1** | **Leiden community detection on the behavioral coupling graph** — NOT on a static dep graph. New `communities` analysis over the Fisher-significant coupling output (`run_coupling`). Built on the `leiden-rs` crate (MIT/Apache, 3 transitive deps with `default-features = false`); ships Leiden instead of Louvain per "modernize, don't migrate" (the refinement phase guarantees internally-connected communities — Louvain doesn't). Output: per-file community ID + community size + intra/inter strength; partition-level modularity Q. Edges weighted by `-log10(fisher_p)` (same enrichment vector as `centrality::weighted_degree`). | **Genuinely different signal** vs CHM (they run modularity on static imports; we run it on git-co-change). Same algorithm family, philosophically different feature. | shipped on `feat/leiden-communities` (commit pending) — implementation done, remaining work is SPA wire-up + KPI tile. |
| **CHM-B2** | Promote `coupling_centrality_v1` from internal TEMP TABLE to first-class analysis output. Adds degree + weighted_degree (sum of -log10 p across partners) as queryable columns on a `centrality` analysis. In-/out-degree skipped — the Fisher 2×2 contingency table is symmetric so the coupling graph is undirected. | CHM has degree centrality on static graph; we expose it on behavioral graph + statistical-strength enrichment — richer signal. Already 90% there; the new analysis exposes the previously-hidden temp-table computation. | shipped on `feat/centrality-analysis` (commit 144fd11). |
...
| **Behavioral Leiden community detection** (Traag et al. 2019 on our coupling graph) | Novel signal vs CHM (they Louvain on static dep graph; we Leiden on git-co-change). | v0.5.x (CHM-B1) | implementation shipped on `feat/leiden-communities`; SPA wire-up pending |
