...
| Bundled DuckDB: the compile dominator **and** the musl blocker (one decision) | `libduckdb-sys` `bundled` compiles ~6000 .cpp files via `cc-rs` every run (~5–7 min), and it is now the only C++-compiling dependency that builds on any target we ship — so it is simultaneously the slowest thing in CI and the sole remaining blocker on `x86_64-unknown-linux-musl`. These were separate problems until the unreachable-grammar excision removed `bca-tree-sitter-preproc`'s and `bca-tree-sitter-ccomment`'s `scanner.cc` and left `codelore-rca` pure C; one dependency and one `bundled` feature now govern both outcomes, which is why they are a single row. **The three obvious options are not interchangeable once read that way:** improving the sccache C++ object-cache hit rate helps the build time and does nothing for musl; switching to `dynamic` + shipping pre-built DuckDB helps the build time but *forfeits the reason musl is wanted*, since the point is a **static** binary for Alpine, distroless-static and air-gapped installs; splitting a "build DuckDB once, cache the artifact" job is the **only option that closes both**, provided the cached artifact is produced with a musl C++ toolchain — at which point one piece of work delivers the CI win and the release target together. Route for the toolchain itself: a `messense/rust-musl-cross`-style Docker image, or `musl-cross-make` on the existing runner. |
...
| ~~`cargo-machete` / `cargo-udeps` as a CI gate~~ | **Evaluated and rejected on measurement.** Proposed to automate the unused-declared-dependency class this repo hit twice. `cargo machete` reports six findings here and **one is real** (`dirs`, since removed); the other five name load-bearing code — `num-traits`, required via `#[derive(FromPrimitive)]` expansion, and four of the five grammars the product dispatches. The tool's own output proves the mechanism: `tree-sitter-typescript` is the only grammar it does *not* flag, and the only one named outside the `mk_langs!` invocation. It is a text scanner; macro invocations are opaque to it (`cargo-shear` concedes the same limit, liftable only with a nightly `--expand`). Reaching green would take five ignore entries — suppressing more than it reports, and failing the bar the `zizmor` adoption set: a check that is red on every pull request teaches people to ignore red checks. The class is still worth catching; the method that works is removing the declaration and building. |
| ~~`petgraph` 0.6 → 0.8 bump~~ | **Closed by the unreachable-grammar excision.** Its single consumer was `codelore-rca/src/preproc.rs`, which went with the `preproc` grammar, so `petgraph` left the workspace entirely and its `dependabot.yml` pin is retired. The follow-on this unblocks: `leiden-rs`'s `petgraph` feature is no longer constrained by a version collision, so it can be evaluated on its merits rather than held off by one. The reason of record for keeping it off otherwise lives beside the dependency in `codelore-lib/Cargo.toml`. |
| `tree-sitter*` coordinated grammar sweep | Single-grammar bumps either ABI-mismatch at load or silently produce wrong complexity metrics (renumbered node IDs). Sweep recipe: lockstep-bump core + every `tree-sitter-<lang>` in **both** manifests (`codelore-rca`'s pins and `codelore-lib`'s ABI-compat mirror) → regenerate every `language_*.rs` → update `provenance::grammar_pins()` and `UPSTREAM.md`'s version table, the two sites that restate the pins outside a manifest → re-run complexity golden fixtures. ~1-day focused sprint. |
