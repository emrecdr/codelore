...
| ~~`petgraph` 0.6 → 0.8 bump~~ | **Closed by the unreachable-grammar excision.** Its single consumer was `codelore-rca/src/preproc.rs`, which went with the `preproc` grammar, so `petgraph` left the workspace entirely and its `dependabot.yml` pin is retired. The follow-on this unblocks: `leiden-rs`'s `petgraph` feature is currently disabled in `codelore-lib/Cargo.toml` solely to avoid conflicting with the pinned 0.6, and can now be evaluated on its merits. |
