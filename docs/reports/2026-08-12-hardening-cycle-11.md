...
**Still open, unchanged:** M8 MCP cancellation (0 `RequestContext`; reframed per cycle-9 E9 as a design question — the ingest is under non-abortable `spawn_blocking`); `outputSchema` at **1 of 11** tools (one `Json<GateSummary>` against eleven `#[tool]` declarations; per-tool design work per E8); the **gitlink differential fixture** — gitlink *handling* exists in both backends, so what is absent is specifically a fixture in the differential bundle. A correction to an earlier draft of this line: the five unit tests carrying `160000` fixtures are **all on the `GitCliRepo` side**; `gix_repo`'s `is_symlink_or_gitlink` has none. The deferral rationale ("both backends already agree by design") is therefore weaker than stated, because the untested side is the production walker and the tested side is the oracle; `zizmor` (0 references under `.github/`) and crates.io Trusted Publishing, both unadopted; Rust pin `1.96.0` with 1.97.1 available (1.98 ~2026-08-27).
...
- **Retracted: "draft fields that do not exist."** This line previously said `last_author`, `top_contributor`, `file_a`, `file_b` and `touched_file` "do not appear anywhere in the crate." That is false. All five are `pub …: String` on analysis row types — `authors.rs`, `bus_factor.rs`, `clone_coupling.rs`, `coupling.rs`. The draft's characterisation was correct: they are real string fields on analyses the dashboard does not embed today, which is exactly what it claimed. The error was mine, and it was made the same way the errors this report criticises were made — a `grep` scoped to `src/output/` whose empty result was generalised to "the crate." A narrow probe reported as a broad conclusion is the failure mode this report is about; producing one in the act of correcting one is worth recording rather than quietly fixing. `main_author` is real and rendered; `src_path` is real and its single use is plumbing into `modulePath()`, not a sink.
