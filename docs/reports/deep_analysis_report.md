...
*   **Fix**: (1) rank FILES not functions — aggregate to the file first (`MAX` per file), then `PERCENT_RANK` across files, so each smell's intensity is uniformly spread; (2) replace the probabilistic-OR with a bounded weighted sum (per-smell weights summing to 1.0, absent smells contribute 0, co-occurrence implicit). Band thresholds retuned to `0.50 / 0.25` (red/yellow). Empirical result on this repo: `structural_risk` spreads `0.01–0.82`, band split **8 red / 29 yellow / 32 green**. Added an anti-saturation regression test (`code_health_structural_risk_not_saturated`). Residual (Phase-2): small per-language cohorts (e.g. a handful of JS files) make `PERCENT_RANK` coarse — the cross-repo corpus percentile addresses this.
