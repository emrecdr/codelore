...
1. **`.github/workflows/release.yml`** (the release pipeline):
   ...
   - `build` — matrix of 5 targets (aarch64-darwin, x86_64-darwin, x86_64-linux-gnu, aarch64-linux-gnu, x86_64-windows-msvc), each running `cargo build --locked --release -p codelore --target $TARGET --features spa` and packaging the artifact into a versioned tarball/zip. `--locked` builds from the audited lockfile rather than re-resolving, and `--features spa` is why released binaries carry the single-page report that a default `cargo install` does not. This job holds **no** signing permissions: it runs `build.rs`, so under SLSA Build L3 the sigstore token must be unreachable from it
   - `attest` — matrix over the same 5 targets, calling the reusable trusted signer at `.github/workflows/attest-artifact.yml`. Each instance downloads one artifact, hashes it, and signs via `actions/attest`; each sigstore bundle is then uploaded as a workflow artifact for the release job to publish. This is the only job permitted to hold `id-token`/`attestations`, and `release` depends on it so a failed attestation blocks publication
   - `release` — downloads all 5 build artifacts and publishes the GitHub Release via `softprops/action-gh-release` (SHA-pinned — see `release.yml`), with `generate_release_notes: true` pulling the CHANGELOG section automatically. Alongside the archives it stages one `<archive>.sigstore.json` attestation bundle per artifact (cardinality-checked; excluded from SHA256SUMS) and publishes them as release assets for offline verification
