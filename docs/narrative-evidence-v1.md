...
**Status:** deterministic characterization of the citation check on a labelled corpus, plus three model-study legs — a pinned 3B local model over this repository, and two contributed frontier-model runs over six external repositories, the second re-running the first's corpus under the current prompt with sampling pinned. Legs 2 and 3 are paired with each other on the prompt axis; leg 1 shares no file with either, so the model axis is still unpaired and that leg is pending.
...
**This is not a model-quality comparison, and reading it as one inverts the finding.** Leg 1's 65.5% and this leg's 6.9% differ by a factor of nine, but model, corpus, host and prompt-time all move together between them, and the largest identifiable driver is **numeric verbosity, not faithfulness**. Counting the uncited tokens each run-1 narrative actually produced — the stamp-named tokens plus the truncated residue, on the recovered basis the next paragraph explains — `deepseek-v4-pro` emits **11.69× more per narrative** than `llama3.2-t0-s42` (9.03 vs 0.77). A checker that flags a narrative when *any* single quoted number fails to match is close to a per-token race: a model that writes many more numbers loses it far more often at equal per-token faithfulness. An independent observation points the same way: §Blind-spot incidence instrumentation measures these same two models at 17.20 and 3.00 exempt tokens per corpus entry — a different corpus, a different metric, and a ratio of the same order arrived at without reference to either leg. None of this is proof: the clean test is the paired leg, where corpus is held fixed and model is the only variable. It is enough to say that the nine-fold gap should not be attributed to the frontier model being nine times less faithful.

**Per-repository, the rate varies more than the pooled figure suggests** — bat 5/34, express 4/34, ripgrep 3/34, hono 1/34, typer 1/34, gson 0/34. The subject repositories span five languages — Rust (ripgrep, bat), Python, TypeScript, Java and JavaScript — and the balanced 34-file design makes the strata directly comparable; the zero for gson is a real cell, not a missing one.
...
## Third model study — the same corpus under the current prompt

**Per-item records and provenance:** [`narrative-evidence/leg3-deepseek-v4-pro-six-repos-p2-t0-s42.json`](narrative-evidence/leg3-deepseek-v4-pro-six-repos-p2-t0-s42.json), contributed by GitHub `@Xxx91n`. It re-runs the second study's corpus — same six repositories, same 204 files, same model — under `PROMPT_VERSION` 2, with `temperature 0` and `seed 42` transmitted this time, and with the gateway API key rotated per generation so a per-key response cache cannot make repeated prompts look deterministic.

**Its provenance is weaker than leg 2's, and the difference is worth stating.** That leg shipped a raw run log and every figure below it was reconciled against that log independently. This leg shipped no equivalent, so its numbers are reconciled only against the record itself. The record is internally consistent and structurally identical to leg 1's shape, and every figure here was recomputed from it — but nothing outside it corroborates that the runs occurred as described. It is contributed point-in-time evidence, not a measurement this repository can reproduce.

| Measure | k / n | Rate | Wilson 95% |
|---|---:|---:|---|
| `grounded ✓` — first generation | 17/204 | 8.3% | [0.053, 0.129] |
| `grounded ✓` — majority of 3 | 13/204 | 6.4% | [0.038, 0.106] |
| Verdict flipped across the 3 runs | 17/204 | 8.3% | [0.053, 0.129] |

**This is the study's first paired per-item difference.** Legs 1 and 2 share no file, so nothing could be subtracted between them; legs 2 and 3 share all 204. Pairing first-generation verdicts gives 10 grounded in both, 183 in neither, and **11 discordant — 7 gained, 4 lost** — an exact McNemar **p = 0.549**. The prompt hardening did not move groundedness detectably. The instrument the protocol asks for finally exists, though on the *prompt* axis rather than the model axis it was designed for.

Two variables moved together here — prompt version and sampling pins — so a difference, had one appeared, would not have been attributable. What limits that confound is the leg's own determinism result below: the pins are shown not to change behaviour, which leaves the prompt as the only moving part that could have.

**Pinning `temperature 0` and a seed did not buy determinism.** Run-pair verdict agreement is **92.6% / 96.1% / 94.6%** here against **96.6% / 93.6% / 93.1%** in the unpinned leg 2 — indistinguishable, and marginally worse at the low end. This directly contradicts what leg 1 found, where repeat generations were byte-identical in 110/110 items under the same nominal pins. The pins are the same; the serving path is not. Leg 1 ran against a local process, these legs against a hosted model behind a gateway, and determinism turns out to be a property of that path rather than of the temperature setting. The practical consequence is recorded here because it shapes any future leg: three generations per item remain necessary, and a design that assumes `temperature 0` plus a seed makes one generation sufficient would be measuring its own assumption.

**Token volume is unchanged**, 9.03 → 8.94 uncited tokens per narrative on the all-files basis this document uses throughout. The contributed report quotes 9.70 → 9.74 for the same runs; that is the same quantity over run-1 *flagged* files only. Both reproduce, but conditioning on "was flagged" selects for high token counts — being flagged is caused by emitting an unmatched number — so that basis tracks the flag rate rather than the model, and is not the one used here.
...
- **Model-sensitivity data covers two models, and the pairing that exists is not between them** (leg status and the pairing requirement: see §First model study, "Limits of this leg"). All three legs meet the same bar — a per-item record committed here, with the prose recomputed from it — but leg 1 shares no file with legs 2 and 3, so the **model** axis has still never been paired. What legs 2 and 3 do supply is a paired per-item difference on the **prompt** axis, over the same 204 files; the protocol's central instrument therefore exists, measuring the wrong variable. Two of the protocol's pins remain unmet by leg 2 (temperature and seed never sent, model ID undated); leg 3 closes the first but not the second. Their rates stay published as separate legs rather than being compared against leg 1's as though one number followed the other. One asymmetry between the two contributed legs is worth carrying: leg 2 shipped a raw run log its figures were reconciled against, leg 3 did not, so leg 3 is reconciled only against itself. The study protocol, fixed in advance so the numbers can't be shaped after the fact: the same ≥ 100 fact sheets to every model (paired design, per the error-bars guidance above); one pinned prompt template with its hash recorded (`PROMPT_VERSION` already exists for this); temperature 0 everywhere, with the local model pinned by model tag, quantization, runtime version, and seed, and the hosted model by dated model ID; 3 runs per item to measure and report the flip rate (temperature-0 nondeterminism is documented for hosted APIs); grounded rate per model with Wilson 95% CIs plus the paired per-item difference; and the checker's own FP/FN from this document stated alongside, since an imperfect judge bounds what the study can claim. Live models stay out of per-PR CI — a scheduled or offline refresh only.
