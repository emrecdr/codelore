...
**Date:** 2026-09-01 (numbers). **Per-item records:** [`narrative-evidence/leg1-llama3.2-3b-q4km-t0-s42.json`](narrative-evidence/leg1-llama3.2-3b-q4km-t0-s42.json) — path, three verdicts, uncited tokens, and a narrative hash per run, so every aggregate below can be recomputed from the committed record.

**Setup, pinned per the protocol below:** ollama 0.33.0 on macOS/Apple M4 Pro serving `llama3.2-t0-s42` — a model card deriving from `llama3.2:latest` (3.2B parameters, Q4_K_M quantization, base digest `a80c4f17`) with `temperature 0` and `seed 42` baked into its parameters, so the model id itself records the pinning. Default local OpenAI-compatible endpoint; the versioned prompt template (`PROMPT_VERSION` 1). Subject: this repository itself at commit `86cecc4e0439254fe467da103ec655ec7eee4573` — every file with a code-health row (n = 110), each narrated 3 times via `--llm-refresh`, 330 generations total, zero invocation failures in the final data.
...
**What the grounded rate means — and does not.** A 3B local model produces a `grounded ✓` narrative for roughly two files in three on this repository. Because the over-flagging routes above fire on faithful text, `⚠` is not a hallucination rate: inspection of the flagged stamps shows a substantial share are the checker's known safe-direction failures on faithful narratives, so 65.5% is a **lower bound** on faithful generations. Of the 82 uncited tokens on first-generation stamps (plus 3 beyond the stamp's 5-token preview): 49 integers — dominated by function-span line numbers quoted from the `functions` section's `name@start-end` identifiers, an over-flagging route this study observed in the wild that the labelled corpus does not yet contain (its next growth candidate); 26 decimals — dominated by verbatim quotes of the `corpus_percentile_ci` en-dash composite, the `fp-ci-bound` route the corpus predicted, now confirmed in real model output; and 7 percents, a mix of unit conversions and genuine fabrications. One stamp illustrates both directions at once: the model quoted the CI bounds `0.978549` and `0.979442` verbatim from the sheet (faithful, flagged — user-level false positive) *and* invented a "95.9% confidence interval" whose level appears nowhere on the sheet (fabrication, correctly flagged) — the same `⚠` covering a false alarm and a true catch.

**Determinism: temperature 0 + a fixed seed is not one number, it is two.** Repeat generations of an identical prompt against the warm server were byte-identical in **110/110** items — but the *first* generation of each prompt differed from that stable pair in 35/110, and all 13 verdict flips follow exactly this cold-vs-warm pattern. This is consistent with documented serving-state sensitivity in llama.cpp-family servers (prompt-cache and batching effects on first evaluation), and it is why the protocol pre-registered flip-rate measurement instead of assuming temp-0 determinism: the realistic user path is one generation per dossier, and that path carries ~12% verdict instability on this setup. Numbers above report the first generation as primary — the honest analogue of what a user sees.

**Limits of this leg.** One model, one repository, and a grounded rate that conflates model faithfulness with checker over-flagging (separating the two per-stamp requires the labelling pass the growth plan describes). The paired frontier-model leg is pending an API key; to preserve the paired design it must reuse the same commit (`86cecc4…`) and file list, both recorded in the per-item JSON.
