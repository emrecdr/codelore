...
On this corpus: **20 of 59 entries carry exempt tokens (28 tokens total)**, and **8 entries carry fallback-only groundings (8 tokens: 3 benign unit conversions of a genuine sheet quantity, 5 collisions with an unrelated fraction)**. The benign/collision split is exactly why the fallback exists and exactly what it risks — the same mechanism serves both. These are corpus numbers; the model study below measures stamp-level route incidence on real narratives, but the diagnostics' own counts are not yet surfaced through any output channel, so their in-the-wild incidence remains open.
