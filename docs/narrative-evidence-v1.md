...
- **The corpus makes no prevalence claims.** It is authored-first; class frequencies are chosen. In-the-wild frequency is what §First model study measures, for one model and one repository so far. A contributed cross-model replication reports the same dominant route on other repositories and supplied five entries here, but its per-item record lives outside this repository, so it informs the corpus without contributing rates to this cut.
