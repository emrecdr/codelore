...
|---|---|---|---|---|
| 1 | **Hotspot circle-pack** | `run_hotspots` + filesystem hierarchy | `custom` + `d3.pack()` layout | Signature CodeScene look. Supports a bivariate health×activity default plus seven single-signal lenses (cognitive, code health, tech-debt friction, knowledge map, AI attribution, knowledge loss, clones) plus a ring overlay and coupling-arc overlay (Fisher p-value-encoded opacity + degree-encoded width). |
...
`docs/bi-integration.md` (planned) will document this with example queries that reproduce every analysis in the registry. This path is complementary to the SPA emitter — users who want raw exploration can stay here; users who want the curated CodeLore narrative get the SPA.
