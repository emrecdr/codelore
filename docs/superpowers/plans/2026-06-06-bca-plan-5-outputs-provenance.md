# bca Plan 5: SARIF + Markdown + Parquet + SQLite Outputs + Provenance Manifest

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task.

**Plan 5 of 6 for v1 Spine release.** Builds on Plans 1–4.

**Goal:** Add the 5 remaining output formats (SARIF 2.1.0, JSON, Markdown for `$GITHUB_STEP_SUMMARY`, Parquet, SQLite) and emit a provenance manifest alongside every analysis. The Behavioral SARIF output is bca's published wedge differentiator (no other tool emits SARIF for organizational signals; lands in GitHub Code Scanning UI alongside CodeQL alerts).
...
- `bca analyze --format sarif` emits SARIF 2.1.0 compatible JSON with hotspot findings
- `bca analyze --format json` emits structured JSON
- `bca analyze --format markdown` emits Markdown summary suitable for `$GITHUB_STEP_SUMMARY`
- `bca analyze --format parquet --output file.parquet` works
- `bca analyze --format sqlite --output file.db` works
...
**Files:**
- Create: `crates/bca-lib/src/provenance/mod.rs`
- Modify: `crates/bca-lib/src/facts/mod.rs` (add `provenance()` accessor)
- Modify: `crates/bca-lib/src/lib.rs`
- Create: `crates/bca-lib/tests/provenance_test.rs`
...
impl Manifest {
    pub fn capture(db: &FactsDb, opts: &Options, analysis: &str) -> Result<Self> {
        ...
        // ... query db.query_provenance("bca_version") etc.
        Ok(Self {
            bca_version: env!("CARGO_PKG_VERSION").to_string(),
    ...
    pub fn to_json(&self) -> Result<String> {
        serde_json::to_string_pretty(self)
            .map_err(|e| BcaError::Output(format!("manifest json: {e}")))
    }
}
```
...
**Files:**
- Create: `crates/bca-lib/src/output/json.rs`
- Modify: `crates/bca-lib/src/output/mod.rs`
- Create: `crates/bca-lib/tests/output_json_test.rs`
...
pub fn write_hotspots_json<W: Write>(rows: &[HotspotRow], w: &mut W) -> Result<()> {
    serde_json::to_writer_pretty(w, rows)
        .map_err(|e| BcaError::Output(format!("json: {e}")))
}
```
...
**Files:**
- Create: `crates/bca-lib/src/output/sarif.rs`
- Create: `crates/bca-lib/tests/output_sarif_test.rs`
...
pub fn write_hotspots_sarif<W: std::io::Write>(
    ...
    let rules = vec![
        SarifRule {
            ...
            help_uri: "https://github.com/.../bca#hotspots".into(),
    ...
    let results: Vec<SarifResult> = rows.iter().map(|r| {
        ...
        props.insert("bca/revs".into(), serde_json::json!(r.revisions));
        props.insert("bca/cognitive".into(), serde_json::json!(r.cognitive));
        props.insert("bca/codehealth".into(), serde_json::json!(r.code_health));
        props.insert("bca/score".into(), serde_json::json!(r.hotspot_score));
        ...
        runs: vec![SarifRun {
            tool: SarifTool {
                driver: SarifDriver {
                    ...
                    information_uri: "https://github.com/.../bca".into(),
                    rules,
                },
            },
            automation_details: SarifAutomation { id: "bca/hotspots/run".into() },
            results,
        }],
    };

    serde_json::to_writer_pretty(w, &log)
        .map_err(|e| BcaError::Output(format!("sarif: {e}")))
}
...
You'll need to add `sha2 = "0.10"` and `hex = "0.4"` deps to bca-lib.

Test: emit a SARIF blob, parse it back via serde_json, assert `runs[0].results[0].ruleId == "BCA-HOTSPOT"`.

Plan 5 ships SARIF for `hotspots` only. Other rules (BCA-COUPLING, BCA-OWNERSHIP-RISK, BCA-CODE-HEALTH) defer to Plan 5.x or v1.5.
...
**Files:**
- Create: `crates/bca-lib/src/output/markdown.rs`
- Create: `crates/bca-lib/tests/output_markdown_test.rs`
...
pub fn write_hotspots_markdown<W: Write>(rows: &[HotspotRow], w: &mut W) -> Result<()> {
    writeln!(w, "# bca hotspots").map_err(BcaError::Io)?;
    writeln!(w).map_err(BcaError::Io)?;
    writeln!(w, "| Entity | Revisions | Cognitive | Code Health | Score |").map_err(BcaError::Io)?;
    writeln!(w, "|---|---|---|---|---|").map_err(BcaError::Io)?;
    for row in rows {
        writeln!(w, "| `{}` | {} | {:.2} | {:.2} | {:.4} |",
...
**Files:**
- Create: `crates/bca-lib/src/output/parquet.rs`
- Create: `crates/bca-lib/src/output/sqlite.rs`
- Tests
...
- Modify: `crates/bca-cli/src/args.rs` (already accepts --format as String)
...
Test: assert sidecar exists after `bca analyze --output /tmp/x.csv` and contains expected fields.
...
- [ ] `bca analyze --format sarif` emits SARIF 2.1.0 with hotspot findings
- [ ] `bca analyze --format json` emits structured JSON
- [ ] `bca analyze --format markdown` emits Markdown summary
- [ ] `bca analyze --format parquet --output file.parquet` works
- [ ] `bca analyze --format sqlite --output file.db` works
