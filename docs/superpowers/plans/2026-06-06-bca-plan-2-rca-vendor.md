# bca Plan 2: RCA Vendor + Plan 1 Carry-Over

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Plan 2 of 6 for v1 Spine release.** Builds on Plan 1 (`docs/superpowers/plans/2026-06-06-bca-phase-0-walking-skeleton.md`).

**Goal:** Add the third workspace crate `bca-rca/` (vendored fork of [mozilla/rust-code-analysis](https://github.com/mozilla/rust-code-analysis)), validate it produces working metrics for Tier-1 languages (Rust, TypeScript/JavaScript, Python, Java), gate the buggy JS/TS Halstead+MI metrics behind a `metrics-experimental` feature flag, and clean up the small carry-over items from Plan 1's final review.

**Architecture:** `bca-rca` is a self-contained vendored crate under dual SPDX license `MPL-2.0 AND GPL-3.0-only` (original RCA files retain MPL-2.0; new files are GPL-3.0). It is NOT integrated with `bca-lib` in Plan 2 — that's Plan 3 (complexity integration). Plan 2 ships the raw metric library; Plan 3 wires it into hotspots and Code Health.
...
- bca-lib integration of complexity metrics (Plan 3)
...
- `crates/bca-rca/` exists in the workspace with vendored RCA code
- Cargo.toml SPDX: `MPL-2.0 AND GPL-3.0-only`
- `cargo test -p bca-rca` is green (RCA's own snapshot tests preserved for the 4 Tier-1 languages)
- A new integration test in `bca-rca/tests/` proves each Tier-1 language produces non-zero Cyclomatic + Cognitive metrics for a representative sample
...
### Task 1: Wire `BcaError::exit_code()` into `main()`

**Files:**
- Modify: `crates/bca-cli/src/main.rs`

- [ ] **Step 1: Update main() to map BcaError to its exit code**
...
fn invalid_repo_exits_with_code_3() {
    let status = Command::cargo_bin("bca")
        .unwrap()
        .args([
            "analyze",
            "--analysis", "revisions",
            "--repo", "/tmp/definitely-does-not-exist-bca-test",
        ])
        .status()
        .unwrap();
...
git commit -m "fix(cli): wire BcaError::exit_code into main per spec §6.6"
```
...
- Modify: `crates/bca-lib/tests/facts_test.rs` (move under test-support gating)
- Modify: `crates/bca-lib/tests/ingest_test.rs` (move under test-support gating)
- Modify: `crates/bca-lib/tests/revisions_test.rs` (move under test-support gating)

- [ ] **Step 1: Change `query_one_value` visibility**

In `crates/bca-lib/src/facts/mod.rs`, change:
...
Around `crates/bca-lib/src/repo/gix_repo.rs:35` the `NOTE(Plan 11)` comment is a typo. Update to:
...
Run: `cargo test -p bca-lib --all-features`
...
**Files:**
- Modify: `crates/bca-lib/tests/facts_test.rs`

- [ ] **Step 1: Add roundtrip test**

Append to `crates/bca-lib/tests/facts_test.rs`:
...
### Task 4: Clone and vendor `mozilla/rust-code-analysis` as `crates/bca-rca/`

**Files:**
- Create: `crates/bca-rca/` (entire vendored crate)
- Modify: `Cargo.toml` (workspace) — add `crates/bca-rca` to `members`
...
Note the commit SHA and date — record it in `crates/bca-rca/UPSTREAM.md` (created in Step 5).

- [ ] **Step 2: Copy the `src/` tree into `crates/bca-rca/`**

```bash
cd /Users/emrec/Projects/playground/codescene
mkdir -p crates/bca-rca
cp -r /tmp/rust-code-analysis-upstream/src crates/bca-rca/src
cp /tmp/rust-code-analysis-upstream/build.rs crates/bca-rca/build.rs 2>/dev/null || true
cp /tmp/rust-code-analysis-upstream/LICENSE crates/bca-rca/LICENSE-MPL
```
...
rm -rf crates/bca-rca/src/languages/language_mozcpp.rs
rm -rf crates/bca-rca/src/languages/language_mozjs.rs

# Drop unused metric impls (ABC, WMC, NPA, NPM — Java-only specializations not in our spec)
rm -f crates/bca-rca/src/metrics/abc.rs
rm -f crates/bca-rca/src/metrics/wmc.rs
rm -f crates/bca-rca/src/metrics/npa.rs
rm -f crates/bca-rca/src/metrics/npm.rs
```
...
In `crates/bca-rca/src/lib.rs` (or `src/metrics/mod.rs` and `src/languages/mod.rs`), find and remove the `pub mod abc;` / `pub mod wmc;` / `pub mod npa;` / `pub mod npm;` / `pub mod language_mozcpp;` / `pub mod language_mozjs;` lines.

Similarly, remove any references to the deleted types in `src/lib.rs`'s public API surface.

- [ ] **Step 5: Create `crates/bca-rca/UPSTREAM.md`**
...
- `src/web/` — REMOVED (actix-web; unused in bca)
...
- [ ] **Step 6: Add `crates/bca-rca/` to workspace members**
...
```toml
members = ["crates/bca-lib", "crates/bca-cli", "crates/bca-rca"]
```

- [ ] **Step 7: Commit (no build yet — Cargo.toml comes in Task 5)**

```bash
git add crates/bca-rca/ Cargo.toml
git commit -m "feat(bca-rca): vendor mozilla/rust-code-analysis (drop -web, mozcpp, mozjs, ABC/WMC/NPA/NPM)"
```

---

### Task 5: Configure `crates/bca-rca/Cargo.toml`

**Files:**
- Create: `crates/bca-rca/Cargo.toml`
...
description = "Vendored fork of Mozilla rust-code-analysis for bca"
...
```bash
git add crates/bca-rca/Cargo.toml crates/bca-rca/UPSTREAM.md
git commit -m "feat(bca-rca): Cargo.toml with MPL-2.0 AND GPL-3.0-only SPDX + Tier-1 grammars"
```
...
**Files:**
- Create: `crates/bca-rca/tests/tier1_languages_smoke.rs`
...
";
    ...
    let metrics = bca_rca::compute_metrics_for_language(src, bca_rca::Language::Rust)
...
def complex(x):
    ...
    return 0
";
    let metrics = bca_rca::compute_metrics_for_language(src, bca_rca::Language::Python)
        .expect("compute metrics");
...
";
    let metrics = bca_rca::compute_metrics_for_language(src, bca_rca::Language::TypeScript)
        .expect("compute metrics");
    assert!(metrics.cyclomatic > 1);
    assert!(metrics.cognitive > 0);
}
...
";
    let metrics = bca_rca::compute_metrics_for_language(src, bca_rca::Language::Java)
        .expect("compute metrics");
    assert!(metrics.cyclomatic > 1);
    assert!(metrics.cognitive > 0);
}
...
";
    let metrics = bca_rca::compute_metrics_for_language(src, bca_rca::Language::JavaScript)
        .expect("compute metrics");
    assert!(metrics.cyclomatic > 1);
    assert!(metrics.cognitive > 0);
}
...
**Adapt the API surface** (`bca_rca::compute_metrics_for_language`, `bca_rca::Language`, the `metrics` struct shape) based on what RCA actually exposes after the vendor + cleanup. RCA's actual public API is in `src/lib.rs` — read it and write the test to match.

- [ ] **Step 2: Run tests**

```bash
cargo test -p bca-rca --test tier1_languages_smoke
```

Expected: 5 tests pass.

```bash
cargo test -p bca-rca --test tier1_languages_smoke --features metrics-experimental
```
...
```bash
git add crates/bca-rca/tests/
git commit -m "test(bca-rca): tier-1 language smoke tests (Cyclomatic + Cognitive)"
```
...
All should be clean. If clippy complains about RCA's upstream code, the vendor crate's `[lints]` section in Cargo.toml is empty (no workspace lints applied) — that's intentional. Clippy on `bca-lib` and `bca-cli` should still be strict.

Expected test counts:
- bca-lib: 18 (previous 17 + new file-backed FactsDb test)
- bca-cli: 4 (previous 3 + new exit code test)
- bca-rca: 5 base + 1 conditional = 6 with --all-features
- **Total: 28**
...
git commit -m "ci: ensure full workspace green after bca-rca addition" || true
```
...
- `BcaError::exit_code()` now wired into `bca` CLI per spec §6.6
...
```markdown
- Per-language complexity metrics (Cyclomatic, Cognitive) for Rust, TypeScript/JavaScript, Python, Java via vendored `bca-rca/`
```
...
- [ ] `crates/bca-rca/` exists with vendored RCA source
- [ ] `Cargo.toml` has `license = "MPL-2.0 AND GPL-3.0-only"`
- [ ] `crates/bca-rca/UPSTREAM.md` documents the source SHA + modifications
...
After Plan 2 ships: author **Plan 3** (complexity integration into bca-lib + hotspot ranking + Code Health composite).
...
|---|---|
| §1.1 v1 in scope — `crates/bca-rca` slot | ✓ Tasks 4–7 |
...
| §1.1 BcaError exit code wiring | ✓ Task 1 (carry-over) |
...
- `bca_lib::BcaError::exit_code` — usage in Task 1 matches the signature from Plan 1.
