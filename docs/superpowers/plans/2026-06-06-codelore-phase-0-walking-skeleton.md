# CodeLore Phase 0 + Walking Skeleton Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Plan 1 of 6 for v1 Spine release.** Companion to spec `docs/superpowers/specs/2026-06-06-codelore-design.md`. Subsequent plans (RCA vendor, complexity integration, full analyses, SARIF/provenance, hardening) authored after this phase ships.

**Goal:** Bootstrap the Cargo workspace, prove the architectural spine (gix → DuckDB → CSV), and ship a runnable `codelore analyze --analysis revisions --repo <path> --format csv` binary that walks a real git repo's history and emits a code-maat-compatible CSV.

**Architecture:** Three-crate workspace (`codelore-lib`, `codelore-cli`, `codelore-rca` stub — RCA vendor is Plan 2). `gix` reads `.git` directly. Commits stream through a bounded `crossbeam_channel<RecordBatch>` into a single DuckDB Appender thread. DuckDB holds the fact store; one SQL view computes revisions-per-entity; DuckDB's `COPY ... TO ...` emits CSV. No complexity, no Fisher, no SARIF, no provenance manifest yet — those land in later plans. **What walking-skeleton proves: the spine works.**

**Tech Stack:**
- Rust edition 2024, MSRV 1.87 (stable - 2)
- `gix 0.84.0` with `features = ["max-performance"]`
- `arrow 58.3.0` (re-exported via `codelore-lib::arrow_facade`)
...
- SARIF, Markdown, Parquet, SQLite outputs, provenance manifest, `codelore diff` (Plan 5)
...
$ codelore analyze --analysis revisions --format csv
entity,n-revs
src/main.rs,42
src/lib.rs,38
...
...
- Create: `rustfmt.toml`
- Create: `clippy.toml`
- Create: `crates/codelore-lib/Cargo.toml`
- Create: `crates/codelore-lib/src/lib.rs`
- Create: `crates/codelore-cli/Cargo.toml`
- Create: `crates/codelore-cli/src/main.rs`
...
repository = "https://github.com/<owner>/codelore"
...
- [ ] **Step 7: Create `crates/codelore-lib/Cargo.toml`**
...
- [ ] **Step 8: Create `crates/codelore-lib/src/lib.rs`**

```rust
//! codelore-lib — Behavioral Code Analyzer library.
//!
//! See `docs/superpowers/specs/2026-06-06-codelore-design.md` for the full design.

#![doc(html_no_source)]
```

- [ ] **Step 9: Create `crates/codelore-cli/Cargo.toml`**
...
[dependencies]
codelore-lib = { path = "../codelore-lib" }
# Added incrementally in later tasks
```

- [ ] **Step 10: Create `crates/codelore-cli/src/main.rs`**

```rust
fn main() {
    println!("codelore v0.1.0-alpha.1");
}
```
...
Expected: clean build, produces `target/debug/codelore`.

- [ ] **Step 12: Run the binary and verify output**

Run: `./target/debug/codelore`

Expected output: `codelore v0.1.0-alpha.1`
...
git commit -m "feat: initialize codelore cargo workspace"
```
...
# Run the binary
codelore *ARGS:
    cargo run --release -p codelore-cli -- {{ARGS}}
```
...
## Phase 0.B: codelore-lib core types

### Task 4: Add core types

**Files:**
- Modify: `crates/codelore-lib/Cargo.toml` (add `time`, `serde`, `thiserror`)
- Create: `crates/codelore-lib/src/types.rs`
- Modify: `crates/codelore-lib/src/lib.rs` (re-export types)
- Create: `crates/codelore-lib/src/error.rs`
- Create: `crates/codelore-lib/tests/types_test.rs`

- [ ] **Step 1: Update `crates/codelore-lib/Cargo.toml`**
...
Run: `cargo test -p codelore-lib --test types_test`

Expected: FAIL with "unresolved import `codelore_lib::types`".

- [ ] **Step 4: Create `crates/codelore-lib/src/types.rs`**
...
Update `crates/codelore-lib/src/lib.rs`:
...
- [ ] **Step 6: Create `crates/codelore-lib/src/error.rs`**
...
git commit -m "feat(lib): add core types — CommitEvent, FileChange, Hunk, ChangeType, KameiFeatures, CodeLoreError"
```
...
**Files:**
- Create: `crates/codelore-lib/src/analysis.rs`
- Create: `crates/codelore-lib/src/options.rs`
- Modify: `crates/codelore-lib/src/lib.rs`
- Modify: `crates/codelore-lib/tests/types_test.rs` (add tests)

- [ ] **Step 1: Write failing test for AnalysisName parse/display**

Append to `crates/codelore-lib/tests/types_test.rs`:
...
- [ ] **Step 3: Create `crates/codelore-lib/src/analysis.rs`**
...
- [ ] **Step 4: Create `crates/codelore-lib/src/options.rs`**
...
- Create: `crates/codelore-lib/src/arrow_facade.rs`
- Modify: `crates/codelore-lib/src/lib.rs`
...
- [ ] **Step 2: Create `crates/codelore-lib/src/arrow_facade.rs`**
...
Run: `cargo build -p codelore-lib`
...
**Files:**
- Create: `crates/codelore-lib/src/repo/mod.rs`
- Create: `crates/codelore-lib/src/repo/types.rs`
- Modify: `crates/codelore-lib/src/lib.rs`
- Create: `crates/codelore-lib/tests/repo_trait_test.rs`
...
use codelore_lib::repo::{Repo, CommitMetadata};
...
Run: `cargo test -p codelore-lib --test repo_trait_test`

Expected: compile error — `Repo` not found.

- [ ] **Step 3: Create `crates/codelore-lib/src/repo/types.rs`**
...
- [ ] **Step 4: Create `crates/codelore-lib/src/repo/mod.rs`**
...
Run: `cargo test -p codelore-lib --test repo_trait_test --no-run`
...
**Files:**
- Modify: `crates/codelore-lib/Cargo.toml` (add `gix`)
- Create: `crates/codelore-lib/src/repo/gix_repo.rs`
- Modify: `crates/codelore-lib/src/repo/mod.rs`

- [ ] **Step 1: Add gix dependency**

Append to `crates/codelore-lib/Cargo.toml` `[dependencies]`:

```toml
gix = { version = "0.84", features = ["max-performance"] }
```

- [ ] **Step 2: Create `crates/codelore-lib/src/repo/gix_repo.rs`** with stub methods returning the right error variants
    ...
    fn walk_commits<'a>(
        ...
        Ok(Box::new(revwalk.filter_map(move |info| {
            ...
            let commit = match repo.find_commit(info.id) {
                Ok(c) => c,
                Err(e) => return Some(Err(CodeLoreError::Repo(format!("find_commit: {e}")))),
            };
...
Append to `crates/codelore-lib/src/repo/mod.rs`:
...
- Create: `crates/codelore-lib/tests/gix_repo_test.rs`
- Create: `crates/codelore-lib/tests/fixtures/build_tiny_repo.rs` (test helper)

- [ ] **Step 1: Add `gix-diff` feature to gix dependency**

Update `crates/codelore-lib/Cargo.toml`:
...
Create `crates/codelore-lib/src/test_support/mod.rs`:
...
Update `crates/codelore-lib/Cargo.toml` to add the feature:
...
And in `crates/codelore-lib/src/lib.rs`:
...
Run: `cargo test -p codelore-lib --test gix_repo_test --features test-support`

Expected: `changed_files_for_modify_commit` fails — Vec is empty.

- [ ] **Step 5: Implement `changed_files` via gix-diff**

Replace the stub in `crates/codelore-lib/src/repo/gix_repo.rs`:
...
fn changed_files(&self, rev: &str) -> Result<Vec<FileChange>> {
    ...
    let oid = gix::ObjectId::from_hex(rev.as_bytes())
        .map_err(|e| CodeLoreError::Repo(format!("parse oid {rev}: {e}")))?;
    let commit = oid.attach(&repo)
        .object()
        .map_err(|e| CodeLoreError::Repo(format!("find object {rev}: {e}")))?
        .try_into_commit()
        .map_err(|e| CodeLoreError::Repo(format!("not a commit {rev}: {e}")))?;

    let tree = commit.tree()
        .map_err(|e| CodeLoreError::Repo(format!("tree {rev}: {e}")))?;
    ...
    let parent_tree = commit
        ...
        .map(|pid| -> Result<gix::Tree<'_>> {
            pid.object()
                .map_err(|e| CodeLoreError::Repo(format!("parent obj: {e}")))?
                .try_into_commit()
                .map_err(|e| CodeLoreError::Repo(format!("parent not commit: {e}")))?
                .tree()
                .map_err(|e| CodeLoreError::Repo(format!("parent tree: {e}")))
        })
        .transpose()?;
    ...
    let platform = repo
        ...
        .map_err(|e| CodeLoreError::Repo(format!("diff cache: {e}")))?;

    let from = match from_tree {
        Some(id) => id.attach(&repo).object()
            .map_err(|e| CodeLoreError::Repo(format!("from obj: {e}")))?
            .try_into_tree()
            .map_err(|e| CodeLoreError::Repo(format!("from not tree: {e}")))?,
        None => repo
            .empty_tree()
            .map_err(|e| CodeLoreError::Repo(format!("empty tree: {e}")))?,
    };

    from.changes()
        .map_err(|e| CodeLoreError::Repo(format!("changes: {e}")))?
        .for_each_to_obtain_tree(
            &tree,
            |change| -> std::result::Result<gix::object::tree::diff::Action, CodeLoreError> {
        ...
        .map_err(|e| CodeLoreError::Repo(format!("walk diff: {e}")))?;
...
**Files:**
- Modify: `crates/codelore-lib/Cargo.toml`
- Create: `crates/codelore-lib/src/facts/mod.rs`
- Create: `crates/codelore-lib/src/facts/schema.rs`
- Modify: `crates/codelore-lib/src/lib.rs`
- Create: `crates/codelore-lib/tests/facts_test.rs`
...
Run `cargo build -p codelore-lib` — expect ~4–6 minute first build (DuckDB C++ compile).
...
Run: `cargo test -p codelore-lib --test facts_test`

Expected: compile error — `facts` module not found.

- [ ] **Step 4: Create `crates/codelore-lib/src/facts/schema.rs`**
...
- [ ] **Step 5: Create `crates/codelore-lib/src/facts/schema_v1.sql`**
...
- [ ] **Step 6: Create `crates/codelore-lib/src/facts/mod.rs`**
...
**Files:**
- Modify: `crates/codelore-lib/Cargo.toml` (add `crossbeam-channel`)
- Create: `crates/codelore-lib/src/facts/ingest.rs`
- Modify: `crates/codelore-lib/src/facts/mod.rs`
- Create: `crates/codelore-lib/tests/ingest_test.rs`
...
Run: `cargo test -p codelore-lib --test ingest_test --features test-support`

Expected: compile error — `ingest` method missing.

- [ ] **Step 4: Create `crates/codelore-lib/src/facts/ingest.rs`**
...
impl FactsDb {
    pub fn ingest<R: Repo>(&self, repo: &R, opts: &Options) -> Result<IngestStats> {
        ...
        let producer = std::thread::scope(|s| -> Result<IngestStats> {
            ...
            for event in walk {
                let event = event?;
                tx.send(event).map_err(|e| CodeLoreError::Analysis(format!("send: {e}")))?;
            }
            drop(tx);
...
fn ingest_loop(db: &FactsDb, rx: crossbeam_channel::Receiver<CommitEvent>) -> Result<IngestStats> {
    ...
    commits_app.flush().map_err(|e| CodeLoreError::Analysis(format!("flush commits: {e}")))?;
    changes_app.flush().map_err(|e| CodeLoreError::Analysis(format!("flush changes: {e}")))?;
    Ok(stats)
}
...
fn append_commit(app: &mut Appender<'_>, e: &CommitEvent) -> Result<()> {
    ...
    let date_str = e.date.format(&Iso8601::DEFAULT)
        .map_err(|err| CodeLoreError::Analysis(format!("format date: {err}")))?;
...
**Files:**
- Create: `crates/codelore-lib/src/analyses/mod.rs`
- Create: `crates/codelore-lib/src/analyses/revisions.rs`
- Modify: `crates/codelore-lib/src/lib.rs`
- Create: `crates/codelore-lib/tests/revisions_test.rs`
...
Run: `cargo test -p codelore-lib --test revisions_test --features test-support`

Expected: compile error — `run_revisions` not found.

- [ ] **Step 3: Create `crates/codelore-lib/src/analyses/revisions.rs`**
...
- [ ] **Step 4: Create `crates/codelore-lib/src/analyses/mod.rs`**
...
**Files:**
- Create: `crates/codelore-lib/src/output/mod.rs`
- Create: `crates/codelore-lib/src/output/csv.rs`
- Modify: `crates/codelore-lib/src/lib.rs`
- Create: `crates/codelore-lib/tests/output_csv_test.rs`
...
Run: `cargo test -p codelore-lib --test output_csv_test`

Expected: compile error.

- [ ] **Step 3: Create `crates/codelore-lib/src/output/csv.rs`**
...
pub fn write_revisions_csv<W: Write>(rows: &[(String, u32)], w: &mut W) -> Result<()> {
    ...
    for (entity, n) in rows {
        ...
        if entity.contains(',') || entity.contains('"') || entity.contains('\n') {
            ...
            writeln!(w, "\"{escaped}\",{n}").map_err(CodeLoreError::Io)?;
        } else {
            writeln!(w, "{entity},{n}").map_err(CodeLoreError::Io)?;
        }
    }
    Ok(())
}
...
- [ ] **Step 4: Create `crates/codelore-lib/src/output/mod.rs`**
...
**Files:**
- Modify: `crates/codelore-cli/Cargo.toml`
- Modify: `crates/codelore-cli/src/main.rs`
- Create: `crates/codelore-cli/src/args.rs`
- Create: `crates/codelore-cli/tests/cli_test.rs`

- [ ] **Step 1: Update `crates/codelore-cli/Cargo.toml`**
...
assert_cmd = "2"
predicates = "3"
codelore-lib = { path = "../codelore-lib", features = ["test-support"] }
```
...
- [ ] **Step 4: Create `crates/codelore-cli/src/args.rs`**
...
#[derive(Parser, Debug)]
#[command(name = "codelore", version, about = "Behavioral Code Analyzer", long_about = None)]
pub struct Cli {
    #[command(subcommand)]
    pub command: Command,
...
- [ ] **Step 5: Replace `crates/codelore-cli/src/main.rs`**
...
cd /tmp && rm -rf codelore-smoke && git clone --depth 100 https://github.com/BurntSushi/ripgrep codelore-smoke
./target/release/codelore analyze --analysis revisions --repo /tmp/codelore-smoke --rows 10
```
...
```bash
cargo install --path crates/codelore-cli  # or `cargo binstall codelore` once released
cd ~/my-repo
codelore analyze --analysis revisions --format csv
```
...
See [`docs/superpowers/specs/2026-06-06-codelore-design.md`](docs/superpowers/specs/2026-06-06-codelore-design.md).

## License

GPL-3.0-only. Includes a vendored fork of Mozilla's `rust-code-analysis` under MPL-2.0 (Plan 2+) — see `crates/codelore-rca/LICENSE-MPL`.
```
...
```bash
cd /tmp && rm -rf codelore-smoke
git clone --depth 200 https://github.com/BurntSushi/ripgrep codelore-smoke
./target/release/codelore analyze --analysis revisions --repo /tmp/codelore-smoke --rows 5 -v
```
...
- [ ] `codelore analyze --analysis revisions --repo <real-repo>` produces a CSV that matches what code-maat would produce for the same threshold (eyeball only — formal differential test lands in Plan 6)
