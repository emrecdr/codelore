...
- Plan 1–5 stack carried forward (gix 0.84, duckdb 1.10503.1, arrow 58.3.0, codelore-rca, tree-sitter 0.25.3)
...
**Files:**
- Create: `crates/codelore-lib/src/repo/git_cli_repo.rs`
- Modify: `crates/codelore-lib/src/repo/mod.rs`
- Test: `crates/codelore-lib/tests/git_cli_repo_test.rs`
...
The trait surface (verified from `crates/codelore-lib/src/repo/mod.rs`):
...
- `commit_metadata` calls `git show --no-patch --format='%G?%n%(trailers:key=Signed-off-by,valueonly)' <rev>` and parses the result into `CommitMetadata`. Read `crates/codelore-lib/src/repo/types.rs` to confirm `CommitMetadata` field names + types before implementing.
...
```bash
cat crates/codelore-lib/src/repo/mod.rs
cat crates/codelore-lib/src/repo/types.rs    # CommitMetadata fields
ls crates/codelore-lib/src/repo/
```
...
```rust
// crates/codelore-lib/tests/git_cli_repo_test.rs
...
Run: `PATH="$HOME/.rustup/toolchains/1.89.0-aarch64-apple-darwin/bin:$PATH" RUSTUP_HOME="$HOME/.rustup" cargo test -p codelore-lib --test git_cli_repo_test --all-features`
Expected: FAIL with "no `GitCliRepo` in repo" or similar.

- [ ] **Step 3: Implement `GitCliRepo`**

```rust
// crates/codelore-lib/src/repo/git_cli_repo.rs
...
impl GitCliRepo {
    pub fn open(root: &Path) -> Result<Self> {
        ...
        if !output.status.success() {
            return Err(CodeLoreError::Repo(format!("not a git repo: {}", root.display())));
        }
...
```bash
PATH="$HOME/.rustup/toolchains/1.89.0-aarch64-apple-darwin/bin:$PATH" RUSTUP_HOME="$HOME/.rustup" cargo test -p codelore-lib --test git_cli_repo_test --all-features
```
...
**Files:**
- Create: `crates/codelore-lib/src/test_support/differential_repo.rs`
- Modify: `crates/codelore-lib/src/test_support/mod.rs` (re-export)
- Create: `crates/codelore-lib/tests/differential_repo_test.rs`
- Modify: `crates/codelore-lib/Cargo.toml` — add `proptest = "1"` to dev-deps (only if writing proptest macros; otherwise skip — most differential tests are plain `#[test]` over a generated fixture)
...
Also create `codelore_lib::test_support::differential_repo` — pattern it after `tiny_repo::build()`. Read `crates/codelore-lib/src/test_support/tiny_repo.rs` first to match the existing builder shape (typically returns a `Fixture { dir: TempDir }`).
...
- Create: `crates/codelore-lib/tests/code_maat_golden_test.rs`

Run code-maat against our fixture repos, capture the output as the golden CSVs. Plan 6 ships the goldens (committed to git) plus the regenerator script. Tests assert byte-equivalence (modulo header-only column reordering) between `codelore` output and the goldens for the metrics that code-maat actually computes the same way.

**Methodology honesty caveat:** Some CodeLore outputs *deliberately* differ from code-maat (Fisher exact filter on coupling, Code Health composite, hotspot ranking). For those, document the divergence in `fixtures/golden/code-maat/README.md` and skip the test. Match goldens on:
...
- `hotspots` (CodeLore uses published formula; code-maat doesn't have this)
- `code-health` (CodeLore-original)
...
for FIXTURE in tiny differential bot-noisy; do
  cargo run --quiet --bin codelore-fixture-dump -- "$FIXTURE" /tmp/logfile.log
...
(`codelore-fixture-dump` is a tiny new binary that takes a fixture name + output path and writes a `git2` format log. Either ship it as a bin in `codelore-cli` or as a `dev-bin` helper.)

The test:

```rust
// crates/codelore-lib/tests/code_maat_golden_test.rs
...
fn revisions_matches_code_maat_on_tiny_repo() {
    ...
    let mut buf = Vec::new();
    codelore_lib::output::csv::write_revisions_csv(&rows, &mut buf).unwrap();
...
**Files:**
- Create: `crates/codelore-lib/src/test_support/medium_repo.rs` (500-commit generated fixture)
- Modify: `crates/codelore-lib/src/test_support/mod.rs` — re-export `medium_repo`
- Create: `crates/codelore-lib/benches/end_to_end.rs` (or `benches/end_to_end.rs` if workspace-bench dir preferred — match the existing layout convention)
- Modify: `crates/codelore-lib/Cargo.toml` — add `criterion = "0.7"` to dev-deps, add `[[bench]] name = "end_to_end" harness = false required-features = ["test-support"]`
...
fn ingest_medium(c: &mut Criterion) {
    let medium = codelore_lib::test_support::medium_repo::build(); // Build a 500-commit fixture
...
**Files:**
- Modify: `docs/superpowers/specs/2026-06-06-codelore-design.md` — append §11 (release-blocker performance evidence)
...
/usr/bin/time -v ./target/release/codelore analyze --analysis hotspots \
...
- Modify: `crates/codelore-cli/Cargo.toml` — set `publish = false` (binary only, not on crates.io for v1.0)
...
cargo dist init \
  ...
  --installer msi \
  --installer homebrew \
  --tap your-org/homebrew-codelore \
  --hosting github
```
...
WORKDIR /src
COPY . .
RUN cargo build --release -p codelore-cli --target x86_64-unknown-linux-gnu

FROM gcr.io/distroless/cc-debian12:nonroot
COPY --from=builder /src/target/x86_64-unknown-linux-gnu/release/codelore /usr/local/bin/codelore
USER nonroot
...
cargo pgo build -- --bin codelore
# Run the training workload
./target/x86_64-unknown-linux-gnu/release-pgo/codelore analyze \
  --analysis hotspots --repo /tmp/linux-snapshot --output /tmp/training.parquet
cargo pgo optimize build -- --bin codelore
```
...
- Modify: `CHANGELOG.md`
- Modify: `README.md`
- Create: `docs/superpowers/specs/2026-06-06-codelore-design.md` — flip §1 status from "Spine v1 (draft for sign-off)" to "Spine v1 (released)"
...
- **Distroless container image** (`ghcr.io/.../codelore`) — <30 MB compressed, nonroot, signed
...
This triggers `release.yml` (cargo-dist) and `container.yml`. Wait for both to succeed; verify artifacts on the GitHub release page; verify `cargo binstall codelore` works.
...
- [ ] `GitCliRepo` lives at `crates/codelore-lib/src/repo/git_cli_repo.rs` and implements every `Repo` method
