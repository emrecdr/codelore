# bca Plan 3: Complexity Integration + Hotspots + Code Health

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Plan 3 of 6 for v1 Spine release.** Builds on Plan 1 (`docs/superpowers/plans/2026-06-06-bca-phase-0-walking-skeleton.md`) and Plan 2 (`docs/superpowers/plans/2026-06-06-bca-plan-2-rca-vendor.md`).

**Goal:** Wire `bca-rca`'s complexity metrics into `bca-lib`, populate the `entities` and `complexity_metrics` tables during ingest at HEAD, ship two new analyses (`hotspots` with the spec §1.1 published formula, and `code-health` composite per spec §4.6), and surface them through the CLI.

**Architecture:** Plan 3 adds a `complexity` module to `bca-lib` that wraps `bca-rca`. Complexity is computed **only at HEAD** in Plan 3 (per spec §4.4 `--complexity-sample head` default); Plan 4 adds adaptive sampling. Entity granularity is **function-level** (spec §1.1: "Function-level entity baseline via tree-sitter; file-level rollups derive trivially"). Each function in a Tier-1 file becomes a row in `entities`; metrics live in `complexity_metrics`. `hotspots` joins `changes` × `entities` × `complexity_metrics`. `code-health` uses the §4.6 formula with weights configurable in `Options` (Plan 4 ingredients — fragmentation, coupling — set to weight zero in Plan 3 until those analyses exist).

**Tech Stack:**
- All Plan 1+2 stack
- Internal: `bca_rca::{RustParser, PythonParser, TypescriptParser, JavascriptParser, JavaParser, metrics, FuncSpace}` (public API discovered in Plan 2 Task 6)
...
- `bca-lib::complexity` module exists, wraps `bca-rca` for the 5 Tier-1 languages
- `FactsDb::ingest()` populates `entities` and `complexity_metrics` for HEAD files
- `bca_lib::analyses::hotspots::run_hotspots(&db, &opts) -> Result<Vec<HotspotRow>>` computes per-entity hotspot score per the §1.1 formula
- `bca_lib::analyses::code_health::run_code_health(&db, &opts) -> Result<Vec<CodeHealthRow>>` computes the composite per §4.6 (with v1.5 inputs zeroed)
- `bca analyze --analysis hotspots --format csv` and `bca analyze --analysis code-health --format csv` work end-to-end
...
Record both numbers. If `target/` is over 10 GiB AND disk free < 5 GiB, run `cargo clean -p bca-lib -p bca-cli` (NOT `bca-rca` — its incremental cache is small and the C++ DuckDB is what's bloating us, which is in bca-lib's build deps).
...
## §2 — Wire `bca-rca` into `bca-lib` (Phase 3.B)

### Task 2: Add `bca-rca` dependency + language detection

**Files:**
- Modify: `crates/bca-lib/Cargo.toml`
- Create: `crates/bca-lib/src/complexity/mod.rs`
- Create: `crates/bca-lib/src/complexity/language.rs`
- Modify: `crates/bca-lib/src/lib.rs`

- [ ] **Step 1: Add bca-rca dep to bca-lib**

In `crates/bca-lib/Cargo.toml`, append to `[dependencies]`:

```toml
bca-rca = { path = "../bca-rca", version = "0.1.0-alpha.1" }
```

- [ ] **Step 2: Create `crates/bca-lib/src/complexity/language.rs`**
...
- [ ] **Step 3: Create `crates/bca-lib/src/complexity/mod.rs`**
...
In `crates/bca-lib/src/lib.rs`, add `pub mod complexity;` in alphabetical position (between `pub mod arrow_facade;` and `pub mod error;`). No re-export at crate root.

- [ ] **Step 5: Verify build**

```bash
cargo build -p bca-lib
cargo test -p bca-lib --all-features
```
...
### Task 3: Implement `compute_for_file` via bca-rca

**Files:**
- Modify: `crates/bca-lib/src/complexity/mod.rs`
- Create: `crates/bca-lib/tests/complexity_test.rs`
...
```rust
use bca_lib::complexity::{compute_for_file, Tier1Language};
use std::path::Path;
...
Add `[[test]] name = "complexity_test"` stanza to `bca-lib/Cargo.toml` (no required-features needed).

- [ ] **Step 2: Run and confirm fail**

```bash
cargo test -p bca-lib --test complexity_test
```
...
Replace the stub in `crates/bca-lib/src/complexity/mod.rs` with:
...
pub fn compute_for_file(
    ...
    // bca-rca's parser types are concrete; dispatch on language.
...
fn flatten_func_space(
    space: &bca_rca::FuncSpace,
    path: &str,
    out: &mut Vec<ComplexityEntity>,
) {
    let kind = match space.kind {
        bca_rca::SpaceKind::Unit => "file",
        bca_rca::SpaceKind::Function => "function",
        bca_rca::SpaceKind::Class => "class",
        bca_rca::SpaceKind::Struct => "struct",
        bca_rca::SpaceKind::Trait => "trait",
        bca_rca::SpaceKind::Impl => "impl",
        bca_rca::SpaceKind::Namespace => "namespace",
        bca_rca::SpaceKind::Interface => "interface",
        bca_rca::SpaceKind::Unknown => "unknown",
    };
    ...
    out.push(ComplexityEntity {
        ...
        sd_nesting: 0.0,   // bca-rca doesn't expose SD; Plan 4 can compute
...
**Adapt as needed**: the exact field/method names on `bca_rca::FuncSpace.metrics.*` may differ (e.g. `.cyclomatic_sum()` vs `.value()`). Read `bca-rca`'s source if any field is wrong. Plan 2 Task 6's implementer confirmed:
- `func_space.metrics.cyclomatic.cyclomatic_sum()` exists
- `func_space.metrics.cognitive.cognitive_sum()` exists

For Halstead/MI/NOM/NEXITS/LOC/Nesting, check the `bca-rca` source or upstream RCA docs. If a metric isn't available on your bca-rca version, set the field to `None` or `0` and document.

- [ ] **Step 4: Run tests, confirm pass**

```bash
cargo test -p bca-lib --test complexity_test --all-features
```
...
```bash
git add crates/bca-lib/
git commit -m "feat(lib): complexity::compute_for_file via bca-rca for Tier-1 languages"
```
...
**Files:**
- Modify: `crates/bca-lib/src/facts/ingest.rs`
- Modify: `crates/bca-lib/src/facts/mod.rs` (re-export new types if needed)
- Modify: `crates/bca-lib/tests/ingest_test.rs`

- [ ] **Step 1: Add HEAD-file complexity walk to `ingest()`**

After the existing commit/changes ingest loop in `crates/bca-lib/src/facts/ingest.rs`, add a post-pass that:
...
Append to `crates/bca-lib/tests/ingest_test.rs`:
...
```bash
cargo test -p bca-lib --test ingest_test --all-features
```
...
**Files:**
- Create: `crates/bca-lib/src/analyses/hotspots.rs`
- Modify: `crates/bca-lib/src/analyses/mod.rs`
- Create: `crates/bca-lib/tests/hotspots_test.rs`
...
```rust
use bca_lib::analyses::hotspots::{run_hotspots, HotspotRow};
...
Add `[[test]] name = "hotspots_test" required-features = ["test-support"]` to `bca-lib/Cargo.toml`.
...
```bash
cargo test -p bca-lib --test hotspots_test --all-features
```
...
**Files:**
- Create: `crates/bca-lib/src/analyses/code_health.rs`
- Modify: `crates/bca-lib/src/analyses/mod.rs`
- Modify: `crates/bca-lib/src/options.rs` (add Code Health weight overrides — though optional for Plan 3)
- Create: `crates/bca-lib/tests/code_health_test.rs`
...
```rust
use bca_lib::analyses::code_health::{run_code_health, CodeHealthRow};
use bca_lib::facts::FactsDb;
use bca_lib::repo::GixRepo;
use bca_lib::Options;
...
```bash
cargo test -p bca-lib --test code_health_test --all-features
```
...
- Create: `crates/bca-lib/src/output/csv.rs` — add `write_hotspots_csv` and `write_code_health_csv` helpers
- Modify: `crates/bca-cli/tests/cli_test.rs`

- [ ] **Step 1: Add new CSV emitters to bca-lib output module**

In `crates/bca-lib/src/output/csv.rs`, append:
...
In `crates/bca-cli/src/main.rs`, update the `analyze()` function. Currently it bails for any analysis other than `Revisions`. Update to dispatch:
...
Append to `crates/bca-cli/tests/cli_test.rs`:
...
```bash
cargo test -p bca-cli --all-features
cargo test --workspace --all-features 2>&1 | tail -3
cargo clippy --workspace --all-targets --all-features -- -D warnings
cargo fmt --all --check
```
...
./target/release/bca analyze --analysis hotspots --repo . --rows 5 --min-revs 1
./target/release/bca analyze --analysis code-health --repo . --rows 5 --min-revs 1
```

- [ ] **Step 6: Commit**

```bash
git add crates/bca-lib/ crates/bca-cli/
...
- Function-level entity extraction via `bca-rca::FuncSpace` traversal
...
- CLI: `bca analyze --analysis hotspots | code-health --format csv`
...
```markdown
- `bca analyze --analysis hotspots --format csv` — file-level hotspot ranking with the published formula
- `bca analyze --analysis code-health --format csv` — Code Health composite (cognitive-only in Plan 3; full formula in Plan 4)
...
- [ ] `bca-lib::complexity::compute_for_file` works for all 5 Tier-1 languages
...
- [ ] `bca analyze --analysis hotspots --format csv` and `--analysis code-health --format csv` work
- [ ] All previous tests pass (22 lib + 4 cli + 205 bca-rca + new Plan 3 tests = ~240)
...
- **bca-rca API exact field names**: Task 3's `space.metrics.cyclomatic.cyclomatic_sum()` etc. were confirmed working in Plan 2 Task 6; Halstead/MI/etc. field names need verification when wiring up. Implementer should adapt.
