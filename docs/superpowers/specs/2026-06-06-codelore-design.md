...
- **Rust** (edition 2024, MSRV = stable-2), Cargo workspace, **3 crates from day one**: `codelore-lib` + `codelore-cli` + `codelore-rca` (vendored RCA fork). Split `codelore-lib` further (into `codelore-core` / `codelore-repo` / `codelore-facts` / `codelore-complexity` / `codelore-analyses`) when seams emerge — likely v1.5 when RefactoringMiner + co-change entropy land.
...
- **Vendored fork of Mozilla's `rust-code-analysis`** in `crates/codelore-rca/` for complexity (Cyclomatic, Cognitive, Halstead, MI, NOM, NEXITS, LOC variants) across supported languages.
...
- **`authors` and `revisions`** are addressable standalone via `codelore analyze --analysis authors` and `codelore analyze --analysis revisions` (each is a SQL view over the fact store), in addition to being inputs to `hotspots`. Code-maat parity preserved.
...
- **`dist`** for releases → `cargo binstall codelore` works day one.
...
├── crates/
│   ├── codelore-lib/              # types, pipeline, analyses, complexity, outputs
│   ├── codelore-cli/              # clap + glue → single binary
│   └── codelore-rca/              # vendored fork of Mozilla rust-code-analysis
...
### 2.1 Module layout inside `codelore-lib`
...
├── complexity/     # wraps codelore-rca; tree-sitter visitors; nesting-depth fallback
...
| `arrow` (re-exported via `codelore-lib::arrow_facade`) | **58.3.0** | **bumped in lockstep with `duckdb` releases** — see §2.6 | Columnar in-flight format |
...
- `duckdb` is **exact-pinned** because it locks `arrow` to a specific minor (currently 58.x). If `arrow = "59"` ships and `duckdb` still pins 58, naive consumers get a hard build error. The `codelore-lib::arrow_facade` module re-exports all Arrow types so we update in one place.
...
```rust
// codelore-lib/src/arrow_facade/mod.rs
...
All `use arrow::*` in workspace crates becomes `use codelore_lib::arrow_facade::*`. CI lint rule (custom clippy) forbids direct `arrow::*` imports outside the facade module.

### 2.3 Feature flags

- `lang-{rust,typescript,python,java,go}` — default-on per-language tree-sitter grammars (Go support is codelore-implemented; rest are RCA-vendored)
...
1. Copy `master` into `crates/codelore-rca/`. Last-good commit: 2026-01-20 `37e5d83` (or successor as of Phase 0).
2. **Drop from vendor**:
...
- `crates/codelore-rca/Cargo.toml`: `license = "MPL-2.0 AND GPL-3.0-only"` (NOT `OR` — we distribute under BOTH simultaneously, not as a downstream choice).
...
- `codelore-lib/Cargo.toml` and `codelore-cli/Cargo.toml`: `license = "GPL-3.0-only"`.
- Add `crates/codelore-rca/LICENSE-MPL` (verbatim MPL-2.0 text).
...
| v1 Tier-2 | C | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ | RCA |
| v1 Tier-2 | C++ | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ | RCA |
| v1 Tier-2 | Ruby | codelore-impl | codelore-impl | codelore-impl | codelore-impl | codelore-impl | codelore-impl | codelore-impl | codelore-rca additions (v1.5 stretch) |
...
`normalize` maps to `[0, 1]` via the repo's empirical 95th percentile of each metric. Weights configurable in `codelore.toml`. Stored in provenance per run.
...
```
codelore init                    # write a default codelore.toml
codelore analyze [opts]          # run analysis, emit outputs
codelore diff <base> <head>      # PR-mode analysis wrapper
codelore query <sql>             # SQL escape hatch against the fact store (read-only)
codelore facts                   # inspect / dump the fact store
codelore explain <analysis>      # print algorithm + provenance impact
codelore config [show|validate]
codelore doctor                  # diagnose common issues
codelore version                 # human + --format json
```
...
- `CODELORE-HOTSPOT` — Hotspot (high revisions × complexity)
- `CODELORE-COUPLING` — Significant logical coupling pair
- `CODELORE-OWNERSHIP-RISK` — Ownership fragmentation high
- `CODELORE-CODE-HEALTH` — CodeHealth below threshold

Properties: `tags: ["behavioral", "<rule-tag>"]`, `security-severity: min(10, codehealth_below_threshold × 10)`, plus `codelore/*` metric keys.

### 5.5 TOML configuration

`codelore.toml` per industry convention. See §5 inline draft in the dialogue history; full schema documented in `docs/config.md` at implementation time.
...
└── codelore/                 # outputs from codelore (current)
```
...
- `thiserror` 2.0 in `codelore-lib`
- `anyhow` 1.x in `codelore-cli` (anyhow remains on 1.x; no 2.0 release exists)
- `CodeLoreError` enum at lib/cli boundary drives exit codes (provenance violation = 2, repo = 3, analysis = 4, output = 5, IO = 5)
...
- Pre-commit hook: `codelore analyze --quick` on changed files (dogfooding)
...
| `codelore refactor-targets` ranked-list subcommand | Composes codehealth + churn + ownership into actionable refactor-ROI ranking | DDD Europe 2019 talk premise |
...
| `codelore codehealth-calibrate` subcommand (gradient search for weights) | Turns CodeHealth from opinion into evidence-driven score | §4 improvement c |
...
|---|---|---|
| `codelore diff <base> <head>` PR-mode command | **v1** | ~50 LOC wrapper; unlocks review-bot use cases day one |
| Pre-commit hook (`codelore analyze --quick`) | **v1** | Dogfooding; free marketing if it works |
| `codelore doctor` subcommand | **v1** | Diagnoses .mailmap issues, weird changesets, squash-merge workflows |
| `codelore query` SQL escape hatch (read-only) | **v1** | Power-user feature, near-zero cost |
...
| `codelore codehealth-calibrate` | v1.5 | Needs stable v1 first |
| `codelore serve --mcp` MCP server mode | v2 | Tracked in §8.2 |
| `codelore watch` (live hotspot updates as commits land) | v3 | Needs Differential Dataflow per §8.3 |
...
| CodeQL workflow on `codelore` repo | **v1** | Security best practice; ~15 minutes to wire |
...
|---|---|---|
| Final project name (`codelore` vs `codetide` vs `mnemo` vs `repomaat` vs other) | Before crates.io publish | `codelore` |
