...
- `delta_code_health_min` — a floor on the **whole-repo median** delta (projected − baseline), the same semantics the key carries on `codelore diff`: one key, one meaning on both surfaces. The median is computable in working-tree mode because both scoped scoring runs return full row sets. (The populations differ honestly: the gate scores with `min_revs = 1` over all scoreable files, while `diff` medians over its min-revs-filtered hotspot rows.) **Read this as a portfolio signal, not a per-file one, and treat it as sign-unstable in a way `codelore diff`'s committed-range median is not.** §3.3 notes that `structural_risk`'s seven structural smells are population-relative `PERCENT_RANK`s: substituting a changed file's new value into the scored population can nudge *unchanged* files' percentile-derived scores in either direction, so the whole-repo median blends the edited file's real delta with this second-order ripple across files nobody touched. A genuine single-file regression can therefore still read as a *positive* whole-repo median if enough unchanged files drift the other way — the "exact recomputation" guarantee in §3.3 covers each file's own score, not the isolation of other files' scores from it. Don't rely on this key alone to catch a single-file regression.
- `delta_code_health_min_per_file` — the change-scoped sharp tool, and the recommended floor for catching a single-file regression precisely because it sidesteps the sign-instability above: a floor on each changed file's own projected − baseline score, one violation per offending file. A whole-repo median dilutes a single file's regression across every unchanged file (and, per §3.3, can be perturbed by them); the per-file floor compares each edited file only against its own baseline and catches exactly that. Evaluated only by `codelore gate` / `gate_changes`; `codelore diff` does not evaluate this key.
...
Both tools emit compact fixed-order structured text — never JSON blobs, never file contents. Hard caps with `(+n more)` disclosure: top-3 co-change partners per file (` (+n more)` for the rest), the 20-file `change_context` limit, and two **render-level** caps on `gate_changes` / `codelore gate` — the ten largest-`|delta|` rows with a `(+n more files)` tail, and the first ten advisory findings with a `(+n more findings)` tail — while the JSON report carries every row and every finding. Both caps are rendering only: the engine analyzes and gates every changed file and never drops a finding from `ChangeSetReport`, so no gate ever degrades for change-set size and no verdict is ever computed from a truncated view. Budget targets, pinned by a token-counting test on a fixture (whitespace-split proxy measure, asserted with headroom):
